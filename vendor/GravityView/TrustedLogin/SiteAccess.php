...
class SiteAccess {
	...
	/**
	 * @var string The unique identifier of the site in GravityView\TrustedLogin
	 */
	private $identifier;
	...
	/**
	 * @param string $identifier The unique identifier of the site in GravityView\TrustedLogin
	 */
	...
	/**
	 * Handles the syncing of newly generated support access to the GravityView\TrustedLogin servers.
	 *
	 * @param string $secret_id The unique identifier for this GravityView\TrustedLogin authorization. {@see Endpoint::generate_secret_id}
	 ...
	 *
	 * @return true|WP_Error True if successfully created secret on GravityView\TrustedLogin servers; WP_Error if failed.
	 */
	public function sync_secret( $secret_id, $identifier, $action = 'create' ) {
		...
		if ( empty( $response_json['success'] ) ) {
			return new WP_Error( 'sync_error', __( 'Could not sync to GravityView\TrustedLogin server', 'trustedlogin' ) );
		}
	...
	/**
	 * Generates an accessKey that can be copy-pasted to support to give them access via GravityView\TrustedLogin
	 *
	 * Access Keys can only be used by authenticated support agents to request logged access to a site via their GravityView\TrustedLogin plugin.
	 *
	 * @since 0.9.2
	 *
	...
	/**
	 * Revoke a site in GravityView\TrustedLogin
	 *
	 * @param Remote $remote
	 *
	 * @return true|\WP_Error Was the sync to GravityView\TrustedLogin successful
	 */
	public function revoke( Remote $remote ) {

		if ( ! $this->config->meets_ssl_requirement() ) {
			$this->logging->log( 'Not notifying GravityView\TrustedLogin about revoked site due to SSL requirements.', __METHOD__, 'info' );

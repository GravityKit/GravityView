...
/**
 * The GravityView\TrustedLogin all-in-one drop-in class.
 */
final class Remote {

	/**
	 * @var string The API url for the GravityView\TrustedLogin SaaS Platform (with trailing slash)
	 * @since 0.4.0
	 */
	...
	public function maybe_send_webhook( $data ) {
		...
		if ( ! wp_http_validate_url( $webhook_url ) ) {
			...
			$error = new WP_Error( 'invalid_webhook_url', 'An invalid `webhook_url` setting was passed to the GravityView\TrustedLogin Client: ' . esc_attr( $webhook_url ) );
	...
	private function build_api_url( $endpoint = '' ) {
		...
		/**
		 * Modifies the endpoint URL for the GravityView\TrustedLogin service.
		 *
		 * @param string $url URL to GravityView\TrustedLogin API
		 *
	...
	public function handle_response( $api_response, $required_keys = array() ) {
		...
		switch ( wp_remote_retrieve_response_code( $api_response ) ) {
			...
			// Unauthenticated
			case 402:
				return new WP_Error( 'account_error', __( 'GravityView\TrustedLogin Account issue.', 'trustedlogin' ), $response_body );
				break;
			...
			case 404:
				return new WP_Error( 'not_found', __( 'The GravityView\TrustedLogin vendor was not found.', 'trustedlogin' ), $response_body );
				break;

			// Server issue
			case 500:
				return new WP_Error( 'unavailable', __( 'The GravityView\TrustedLogin site is not currently available.', 'trustedlogin' ), $response_body );
				break;

			case 501:
				return new WP_Error( 'server_error', __( 'The GravityView\TrustedLogin site is not currently available.', 'trustedlogin' ), $response_body );
				break;

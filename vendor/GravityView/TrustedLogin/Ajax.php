...
final class Ajax {
	...
	/**
	 * @var null|\GravityView\TrustedLogin\Logging $logging
	 */
	private $logging;
	...
	public function ajax_generate_support() {
		...
		if ( empty( $_POST['vendor'] ) ) {
			...
			$this->logging->log( 'Vendor not defined in GravityView\TrustedLogin configuration.', __METHOD__, 'critical' );

			wp_send_json_error( array( 'message' => 'Vendor not defined in GravityView\TrustedLogin configuration.' ) );
		}

		// There are multiple GravityView\TrustedLogin instances, and this is not the one being called.
		// This should not occur, since the AJAX action is namespaced.
		if ( $this->config->ns() !== $_POST['vendor'] ) {

			$this->logging->log( 'Vendor does not match GravityView\TrustedLogin configuration.', __METHOD__, 'critical' );

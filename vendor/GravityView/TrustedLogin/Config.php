...
final class Config {
	...
	public function __construct( array $settings = array() ) {
		...
		if ( empty( $settings ) ) {
			throw new Exception( 'Developer: GravityView\TrustedLogin requires a configuration array. See https://trustedlogin.com/configuration/ for more information.', 1 );
		}
	...
	public function validate() {
		...
		if ( in_array( __NAMESPACE__, array( 'ReplaceMe', 'ReplaceMe\TrustedLogin' ) ) && ! defined('TL_DOING_TESTS') ) {
			throw new Exception( 'Developer: make sure to change the namespace for the GravityView\TrustedLogin class. See https://trustedlogin.com/configuration/ for more information.', 2 );
		}
		...
		foreach ( array( 'webhook_url', 'vendor/support_url', 'vendor/website' ) as $settings_key ) {
			...
			if ( $value && ! filter_var( $url, FILTER_VALIDATE_URL ) ) {
				$errors[] = new WP_Error(
					...
					sprintf( 'An invalid `%s` setting was passed to the GravityView\TrustedLogin Client: %s',
						$settings_key,
		...
		foreach ( SupportRole::$prevented_caps as $invalid_cap ) {
			if ( array_key_exists( $invalid_cap, $added_caps ) ) {
				$errors[] = new WP_Error( 'invalid_configuration', 'GravityView\TrustedLogin users cannot be allowed to: ' . $invalid_cap );
			}
		}
		...
		if ( $errors ) {
			...
			$exception_text = 'Invalid GravityView\TrustedLogin Configuration. Learn more at https://www.trustedlogin.com/configuration/';
			$exception_text .= "\n- " . implode( "\n- ", $error_text );

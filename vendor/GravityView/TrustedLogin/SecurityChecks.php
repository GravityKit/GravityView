...
final class SecurityChecks {
	...
	/**
	 * @var string GravityView\TrustedLogin endpoint to notify brute-force activity
	 */
	const BRUTE_FORCE_ENDPOINT = 'report-brute-force';

	/**
	 * @var string GravityView\TrustedLogin endpoint to verify valid support activity
	 */
	...
	public function verify( $identifier ) {
		...
		if ( $this->in_lockdown() ){
			...
			return new WP_Error( 'in-lockdown', __( 'GravityView\TrustedLogin temporarily disabled.' , 'trustedlogin') );
		}
	...
	/**
	 * Makes double-y sure the GravityView\TrustedLogin Server approves this support-agent login.
	 *
	 * This function sends server variables to the GravityView\TrustedLogin server to help prevent a number of attack vectors.
	...
	private function check_approved_identifier( $identifier ) {
		 ...
		 *
		 * No site user/visitor/admin data is sent back to GravityView\TrustedLogin server.
		 */
		$body = array(
	...
	/**
	 * Notifies the GravityView\TrustedLogin server that a site may be under a possible brute-force attack.
	 *
	 * @since  1.0.0
	 *
	...
	/**
	 * Locks down the site to new access by GravityView\TrustedLogin identifiers, reports lockdown to GravityView\TrustedLogin
	 */
	private function do_lockdown() {
		...
		if ( is_wp_error( $notified ) ){
			$this->logging->log( sprintf( 'Could not notify GravityView\TrustedLogin (%s)', $notified->get_error_message() ), __METHOD__, 'error' );
		}
	...
	/**
	 * Checks if GravityView\TrustedLogin is currently in lockdown
	 *

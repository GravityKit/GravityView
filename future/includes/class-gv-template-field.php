...
abstract class Field_Template extends Template {
	...
	public function render() {
		 ...
		 *
		 * @hook gravityview/template/field/context
		...
		if ( empty( $output ) ) {
			...
			/**
			 * @hook gravityview/field/value/empty What to display when this field is empty.
		...
		$pre_link_compat_callback = function( $output, $context ) use ( $field_compat ) {
			...
			/**
			 * @hook gravityview_field_entry_value_{$field_type}_pre_link Modify the field value output for a field type before Show As Link setting is applied. Example: `gravityview_field_entry_value_number_pre_link`
			 * @since 1.16
			 * @param string $output HTML value output
			...
			if ( ! empty( $field->show_as_link ) && ! \gv_empty( $output, false, false ) ) {
				...
				/**
				 * @hook gravityview/template/field/entry_link Modify the link HTML
				 * @since 2.0
				 * @param string $link HTML output of the link
				 * @param string $href URL of the link
		...
		$post_link_compat_callback = function( $output, $context ) use ( $field_compat ) {
			...
			/**
			 * @hook gravityview_field_entry_value_{$field_type} Modify the field value output for a field type. Example: `gravityview_field_entry_value_number`
			 * @since 1.6
			 * @param string $output HTML value output
			 * @param array  $entry The GF entry array
			 * @param  array $field_settings Settings for the particular GV field
			...
			/**
			 * @hook gravityview_field_entry_value Modify the field value output for all field types
			 * @param string $output HTML value output
			 * @param array  $entry The GF entry array
			 * @param  array $field_settings Settings for the particular GV field
			...
			/**
			 * @hook gravityview/template/field/{$field_type}/output Modify the field output for a field type.
			 *
			 * @since 2.0
			 *
		...
		/**
		 * @hook gravityview/template/field/output Modify the field output for a field.
		 *
		 * @since 2.0
		 *
		 * @param string $output The current output.

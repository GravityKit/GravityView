...
class Entry_Renderer extends Renderer
{
    ...
    public function render(Entry $entry, View $view, Request $request = null)
    {
        ...
        if (!$request->is_renderable()) {
            gravityview()->log->error('Renderer unable to render Entry in {request_class} context', ['request_class' => get_class($request)]);
        ...
        if ($view->joins) {
            ...
            foreach ($entry->entries as $e) {
                if (!in_array($e['form_id'], $form_ids)) {
                    gravityview()->log->error('The requested entry does not belong to this View. Entry #{entry_id}, #View {view_id}', ['entry_id' => $e->ID, 'view_id' => $view->ID]);

                    return null;
                }
            }
        } elseif ($view->form && $view->form->ID != $entry['form_id']) {
            gravityview()->log->error('The requested entry does not belong to this View. Entry #{entry_id}, #View {view_id}', ['entry_id' => $entry->ID, 'view_id' => $view->ID]);
        ...
        foreach (['single'] as $part) {
            if (($path = $override->get_template_part($template_slug, $part)) && strpos($path, '/deprecated') === false) {
                 ...
                 */
                gravityview()->log->notice('Legacy templates detected in theme {path}', ['path' => $path]);
        ...
        if (!$class || !class_exists($class)) {
            gravityview()->log->notice('{template_class} not found, falling back to legacy', ['template_class' => $class]);

...
class gvlogic extends \GV\Shortcode
{
    ...
    public function callback($atts, $content = '', $tag = '')
    {
        ...
        if (is_null(\GV\Utils::get($atts, 'logged_in', null)) && false === \GV\Utils::get($atts, 'if', false)) {
            gravityview()->log->error('$atts->if/logged_in is empty.', ['data' => $atts]);
        ...
        if (false === $operator && is_null($value)) {
            if (false !== $atts['if']) { // Only-if test
                $match = $authed && !in_array(strtolower($atts['if']), ['', '0', 'false', 'no']);
            } else {
            ...
            if (sizeof($and_values) > 1) {
                ...
                foreach ($or_values as $or_value) {
                    ...
                    if (($authed && $match) || ($authed && (!$match && in_array($operator, ['isnot', 'not_contains', 'not_in'])))) {
                        break;
                    }
                }
            } else {
        ...
        $output = \GFCommon::replace_variables($output, [], [], false, true, false);
    ...
    private function get_output($match, $atts, $content)
    {
        ...
        gravityview()->log->debug('[gvlogic] output parsing:', [
            'data' => [
                'if'   => $if,
                'else' => $else,
            ],

...
class License_Handler
{
    ...
    private function __construct($settings)
    {
        ...
        add_action('init', [$this, 'setup_edd'], 0);
        add_action('wp_ajax_gravityview_license', [$this, 'license_call']);
        add_action('admin_init', [$this, 'refresh_license_status']);
        add_action('admin_init', [$this, 'check_license']);
        add_action('update_option_active_plugins', [$this, 'flush_related_plugins_transient']);
        add_action('update_option_active_sitewide_plugins', [$this, 'flush_related_plugins_transient']);
    }
     ...
     * @var string $edd_action The EDD action to perform, like `check_license`
     * @var string $field_id The ID of the field to check
     * @var bool   $update Whether to update plugin settings. Prevent updating the data by setting an `update` key to false
     * @var string $format If `object`, return the object of the license data. Else, return the JSON-encoded object
     *             }
     *
     * @param bool $cap_check Require `gravityview_edit_settings` capability from current user.
     *
     * @return mixed|string|void
     */
    public function license_call($array = [], $cap_check = true)
    {
        $is_ajax = (defined('DOING_AJAX') && DOING_AJAX);
        $data = empty($array) ? Utils::_POST('data', []) : $array;
        ...
        if ($cap_check && !\GVCommon::has_cap('gravityview_edit_settings')) {
            ...
            } elseif ($license_data->license !== 'failed' && $update_license) {
     ...
     */
    public function license_details($response = [])
    {
        ...
        if (!empty($response['license_key'])) {
            ...
            if (in_array(Utils::get($response, 'license'), ['invalid', 'deactivated'])) {
    ...
    private function get_upgrade_html($upgrades)
    {
        ...
        if (!empty($upgrades)) {
            ...
            foreach ($upgrades as $upgrade_id => $upgrade) {
                ...
                $output .= sprintf('<li><a href="%s">%s</a><span class="howto">%s</span></li>', esc_url(add_query_arg(['utm_source' => 'settings', 'utm_medium' => 'admin', 'utm_content' => 'license-details', 'utm_campaign' => 'Upgrades'], $upgrade->url)), $anchor_text, $message);
            }
            $output .= '</ul>';
        }
    ...
    private function _license_get_remote_response($data, $license = '')
    {
        ...
        if (is_wp_error($response)) {
            gravityview()->log->error('WP_Error response from license check. API params:', ['data' => $api_params]);
        ...
        // Not JSON
        if (empty($license_data)) {
            gravityview()->log->error('Empty license data response from license check', ['data' => compact('response', 'url', 'api_params', 'data')]);
    ...
    public function settings_edd_license_activation($field, $echo)
    {
        ...
        wp_enqueue_script('gv-admin-edd-license', GRAVITYVIEW_URL.'assets/js/admin-edd-license'.$script_debug.'.js', ['jquery']);
        ...
        wp_localize_script('gv-admin-edd-license', 'GVGlobals', [
    ...
    public function refresh_license_status()
    {
        ...
        gravityview()->log->debug('Refreshed the license.', ['data' => $license_call]);
    }
    ...
    public function get_site_data()
    {
        ...
        $data['active_plugins'] = get_option('active_plugins', []);
     ...
     */
    public static function get_related_plugins_and_extensions($implode = '<br />')
    {

...
class Renderer
{
    ...
    {
        if (!has_action('gravityview/template/before', [__CLASS__, 'maybe_print_notices'])) {
            add_action('gravityview/template/before', [__CLASS__, 'maybe_print_notices']);
        }
    }
    ...
    private static function maybe_print_entry_approval_notice($gravityview)
    {
        ...
        $count = \GFAPI::count_entries($gravityview->view->form->ID, [
            'status'        => 'active',
            'field_filters' => [
                [
                    'key'      => 'is_approved',
        ...
        $hide_link = sprintf('<a href="%s" style="float: '.$float_dir.'; font-size: 1rem" role="button">%s</a>', esc_url(wp_nonce_url(add_query_arg(['notice' => 'no_entries_'.$gravityview->view->ID]), 'dismiss', 'gv-dismiss')), esc_html__('Hide this notice', 'gravityview'));
    ...
    private static function maybe_print_reserved_slugs_notice($gravityview)
    {
        ...
        gravityview()->log->error('{slug} page URL is reserved.', ['slug' => $wp->request]);

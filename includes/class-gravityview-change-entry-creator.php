...
class GravityView_Change_Entry_Creator
{
    ...
    public function __construct()
    {
         ...
         * @since  1.5.1
         */
        add_action('gform_user_registered', [$this, 'assign_new_user_to_lead'], 10, 4);
         ...
         * @since  1.7.4
         *
         * @param bool $disable Disable the Change Entry Creator functionality. Default: false.
         */
         ...
         */
        add_action('init', [$this, 'load'], 100);

        add_action('plugins_loaded', [$this, 'prevent_conflicts']);

        // Enqueue and allow selectWoo UI assets
        add_action('admin_enqueue_scripts', [$this, 'enqueue_selectwoo_assets']);
        add_filter('gform_noconflict_scripts', [$this, 'register_gform_noconflict']);
        add_filter('gform_noconflict_styles', [$this, 'register_gform_noconflict']);

        // Ajax callback to get users to change entry creator.
        add_action('wp_ajax_entry_creator_get_users', [$this, 'entry_creator_get_users']);
    }
    ...
    public function enqueue_selectwoo_assets()
    {
        ...
        if (!in_array(GFForms::get_page(), ['entry_detail_edit'])) {
            return;
        }
        ...
        wp_enqueue_script('gravityview_selectwoo', plugins_url('assets/lib/selectWoo/selectWoo.full.min.js', GRAVITYVIEW_FILE), [], $version);
        wp_enqueue_style('gravityview_selectwoo', plugins_url('assets/lib/selectWoo/selectWoo.min.css', GRAVITYVIEW_FILE), [], $version);

        wp_enqueue_script('gravityview_entry_creator', plugins_url('assets/js/admin-entry-creator'.$script_debug.'.js', GRAVITYVIEW_FILE), ['jquery', 'gravityview_selectwoo'], $version);
     ...
     */
    public function assign_new_user_to_lead($user_id, $config, $entry = [], $password = '')
    {
        ...
        gravityview()->log->debug('GravityView_Change_Entry_Creator[assign_new_user_to_lead] - {note}', ['note' => $note]);
         ...
         * @since  1.21.5
         *
         * @param bool $disable Disable the Change Entry Creator note. Default: false.
         */
    ...
    public function load()
    {
        ...
        if (!GVCommon::has_cap(['gravityforms_edit_entries', 'gravityview_edit_entries'])) {
            return;
        }
        ...
        add_action('gform_after_update_entry', [&$this, 'update_entry_creator'], 10, 2);
    }
     ...
     * @return void
     */
    public function update_entry_creator($form, $entry_id)
    {
        global $current_user;
    ...
    public function add_select($form_id, $entry)
    {
        ...
        $entry_creator_user = GVCommon::get_users('change_entry_creator', ['include' => $entry_creator_user_id]);
        $entry_creator_user = isset($entry_creator_user[0]) ? $entry_creator_user[0] : [];
        ...
        $all_users = GVCommon::get_users('change_entry_creator', ['number' => self::DEFAULT_NUMBER_OF_USERS]);

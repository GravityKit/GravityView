...
class GVCommon {
	...
	public static function get_all_views( $args = array() ) {
		...
		/**
		 * @hook gravityview/get_all_views/params Modify the parameters sent to get all views.
	...
	public static function get_entry_id_from_slug( $slug ) {
		...
		/**
		 * Filters the form ID used to get the custom entry ID. Change this to avoid collisions with data from other forms with the same values and the same field ID.
		 * @hook gravityview/common/get_entry_id_from_slug/form_id
		 * @since 1.17.2
	...
	public static function get_form_fields( $form = '', $add_default_properties = false, $include_parent_field = true ) {
		...
		/**
		 * Filters the form fields shown in the Add Field field picker.
		 * @hook gravityview/common/get_form_fields
		 * @since 1.17
	...
	public static function calculate_get_entries_criteria( $passed_criteria = array(), $form_ids = array() ) {
		...
		if ( ! empty( $criteria['search_criteria']['field_filters'] ) && is_array( $criteria['search_criteria']['field_filters'] ) ) {
			foreach ( $criteria['search_criteria']['field_filters'] as &$filter ) {
				...
				/**
				 * Filters the search operator for the field (`contains`, `is`, `isnot`, etc.)
				 * @hook gravityview_search_operator
		...
		/**
		 * Filters final search criteria (Used by the Advanced Filter extension)
	...
	public static function get_entries( $form_ids = null, $passed_criteria = null, &$total = null ) {
		...
		if ( is_null( $return ) && class_exists( 'GFAPI' ) && ( is_numeric( $form_ids ) || is_array( $form_ids ) ) ) {
			...
			/**
			 * When filter is used, defines entries to be used before `GFAPI::get_entries()` is called.
			 * @hook gravityview_pre_get_entries
			 * @since 1.14; in 2.1 The `$total` parameter can now be overriden by reference.
		...
		/**
		 * @hook gravityview_entries Modify the array of entries returned to GravityView after it has been fetched from the cache or from `GFAPI::get_entries()`.
	...
	public static function get_entry_id( $entry_id_or_slug = '', $force_allow_ids = false ) {
		...
		/**
		 * @hook gravityview_custom_entry_slug_allow_id When using a custom slug, allow access to the entry using the original slug (the Entry ID).
	...
	public static function get_entry( $entry_slug, $force_allow_ids = false, $check_entry_display = true, $view = null ) {
		...
		/**
		 * @hook gravityview/common/get_entry/check_entry_display Override whether to check entry display rules against filters
		 * @since 1.16.2
	...
	public static function get_sortable_fields_array( $formid, $blacklist = array( 'list', 'textarea' ) ) {
        ...
        /**
         * @hook gravityview/common/sortable_fields Filter the sortable fields
         * @since 1.12
	...
	public static function is_field_numeric(  $form = null, $field = '' ) {
		...
		/**
		 * @hook gravityview/common/numeric_types What types of fields are numeric?
		 * @since 1.5.2
	...
	public static function js_encrypt( $content, $message = '' ) {
		...
		if ( class_exists( 'StandalonePHPEnkoder' ) ) {
			...
			/**
			 * @hook gravityview/phpenkoder/msg Modify the message shown when Javascript is disabled and an encrypted email field is displayed
			 * @since 1.7
	...
	public static function get_link_html( $href = '', $anchor_text = '', $atts = array() ) {
		...
		/**
		 * @hook gravityview/get_link/allowed_atts Modify the attributes that are allowed to be used in generating links
		 * @since 1.6
	...
	public static function get_users( $context = 'change_entry_creator', $args = array() ) {
		...
		/**
		 * @hook gravityview/get_users/{$context} There are issues with too many users using [get_users()](http://codex.wordpress.org/Function_Reference/get_users) where it breaks the select. We try to keep it at a reasonable number. \n

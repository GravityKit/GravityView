...
class GVCommon
{
    ...
    {
        $product_fields = apply_filters('gform_product_field_types', ['option', 'quantity', 'product', 'total', 'shipping', 'calculation', 'price', 'hiddenproduct', 'singleproduct', 'singleshipping']);
    ...
    public static function entry_has_transaction_data($entry = [])
    {
        ...
        $payment_meta = ['payment_status', 'payment_date', 'transaction_id', 'payment_amount', 'payment_method'];
    ...
    public static function get_entry_meta($form_id, $only_default_column = true)
    {
        ...
        foreach ($extra_fields as $key => $field) {
            if (!empty($only_default_column) && !empty($field['is_default_column'])) {
                $fields[$key] = ['label' => $field['label'], 'type' => 'entry_meta'];
            }
        }
        ...
        foreach (['start_date', 'end_date'] as $key) {
            if (!empty($criteria['search_criteria'][$key])) {
                ...
                if ($date) {
                    ...
                    gravityview()->log->error('{key} Date format not valid:', ['key' => $key, $criteria['search_criteria'][$key]]);
    ...
    public static function get_entries($form_ids = null, $passed_criteria = null, &$total = null)
    {
        ...
        gravityview()->log->debug('[gravityview_get_entries] Final Parameters', ['data' => $criteria]);
        ...
        if (is_null($return) && class_exists('GFAPI') && (is_numeric($form_ids) || is_array($form_ids))) {
             ...
             * @deprecated
             */
            $entries = apply_filters_ref_array('gravityview_before_get_entries', [null, $criteria, $passed_criteria, &$total]);
            ...
            if (is_null($entries)) {
                ...
                if (is_wp_error($entries)) {
                    gravityview()->log->error('{error}', ['error' => $entries->get_error_message(), 'data' => $entries]);
         ...
         * @deprecated
         */
        $return = apply_filters_ref_array('gravityview_entries', [$return, $criteria, $passed_criteria, &$total]);
    ...
    public static function get_entry_id($entry_id_or_slug = '', $force_allow_ids = false)
    {
         ...
         *
         * @param bool True: Allow for slugs based on entry values. False: always use entry IDs (default)
         */
     ...
     * @param bool          $force_allow_ids     Force the get_entry() method to allow passed entry IDs, even if the `gravityview_custom_entry_slug_allow_id` filter returns false.
     * @param bool          $check_entry_display Check whether the entry is visible for the current View configuration. Default: true. {@since 1.14}
    ...
    public static function get_entry($entry_slug, $force_allow_ids = false, $check_entry_display = true, $view = null)
    {
        ...
        if (is_wp_error($entry)) {
            gravityview()->log->error('{error}', ['error' => $entry->get_error_message()]);
    ...
    public static function matches_operation($val1, $val2, $operation)
    {
        ...
        if ('context' === $val1) {
            $matching_contexts = [$val2];

            // We allow for non-standard contexts.
            switch ($val2) {
                // Check for either single or edit
                case 'singular':
                    $matching_contexts = ['single', 'edit'];
                    break;
                // Use multiple as alias for directory for consistency
                case 'multiple':
                    $matching_contexts = ['directory'];
                    break;
            }
        ...
        switch ($operation) {
            ...
            case 'not_in':
                ...
                if (!empty($json_val_1) || !empty($json_val_2)) {
                    $json_in = false;
                    $json_val_1 = $json_val_1 ? (array) $json_val_1 : [$val1];
                    $json_val_2 = $json_val_2 ? (array) $json_val_2 : [$val2];
    ...
    public static function check_entry_display($entry, $view = null)
    {
        ...
        add_action('gravityview/view/query', $entry_subset_callback = function (&$query, $view, $request) use ($entry, $view_form_id) {
            $_tmp_query = new \GF_Query($view_form_id, [
                'field_filters' => [
                    'mode' => 'all',
                    [
                        'key'       => 'id',
     ...
     *
     * @return bool True: yep, GravityView; No: not!
     */
     ...
     */
    public static function get_connected_views($form_id, $args = [])
    {
        global $wpdb;
    ...
    public static function get_sortable_fields($formid, $current = '')
    {
        ...
        if (!empty($fields)) {
            $blocklist_field_types = ['list', 'textarea'];

            $blocklist_field_types = apply_filters_deprecated('gravityview_blacklist_field_types', [$blocklist_field_types, null], '2.14', 'gravityview_blocklist_field_types');
    ...
    public static function get_sortable_fields_array($formid, $blocklist = ['list', 'textarea'])
    {
        ...
        foreach ($fields as $id => $field) {
             ...
             */
            if (in_array($field['type'], ['date', 'time']) && !empty($field['parent'])) {
                $fields[intval($id)] = [
    ...
    public static function is_field_numeric($form = null, $field = '')
    {
         ...
         */
        $numeric_types = apply_filters('gravityview/common/numeric_types', ['number', 'time']);

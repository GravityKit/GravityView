...
class GravityView_Bulk_Actions
{
     ...
     */
    private static $bulk_action_prefixes = [
        'approve'    => 'gvapprove',
    ...
    public function __construct()
    {
        if (did_action('gform_loaded')) {
            ...
            add_action('gform_loaded', [$this, 'process_bulk_action']);
        }
    }
    ...
    public function process_bulk_action()
    {
        ...
        if ($bulk_action && \GV\Utils::_POST('gforms_entry_list') && empty($_POST['gforms_update_note'])) {
            ...
            if (empty($form_id)) {
                gravityview()->log->error('Form ID is empty from parsing bulk action.', ['data' => $bulk_action]);

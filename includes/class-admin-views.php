...
class GravityView_Admin_Views
{
    public function __construct()
    {
        ...
        add_action('gravityview_render_directory_active_areas', [$this, 'render_directory_active_areas'], 10, 4);
        ...
        switch ($column_name) {
            case 'gv_template':
                ...
                if (empty($template_id)) {
                    gravityview()->log->error('View ID {view_id} does not have a connected template.', ['view_id' => $post_id]);
                    break;
                }
            ...
            case 'gv_connected_form':
                ...
                // All Views should have a connected form. If it doesn't, that's not right.
                if (empty($form_id)) {
                    gravityview()->log->error('View ID {view_id} does not have a connected GF form.', ['view_id' => $post_id]);
     ...
     * @param bool      $include_form_link Whether to include the bold name of the form in the output
     *
    ...
    public static function get_connected_form_links($form, $include_form_link = true)
    {
        ...
        if (GVCommon::has_cap(['gravityforms_edit_forms', 'gravityforms_create_form', 'gravityforms_preview_forms'])) {
    ...
    public function save_postdata($post_id)
    {
        ...
        if (!GVCommon::has_cap('edit_gravityview', $post_id)) {
            gravityview()->log->error('Current user does not have the capability to edit View {view_id}', ['view_id' => $post_id, 'data' => wp_get_current_user()]);
        ...
        if (false === GVCommon::has_cap('gravityforms_create_form') && empty($statii['form_id'])) {
            gravityview()->log->error('Current user does not have the capability to create a new Form.', ['data' => wp_get_current_user()]);
        ...
        gravityview()->log->debug('[save_postdata] Update Post Meta Statuses (also returns false if nothing changed)', ['data' => array_map('intval', $statii)]);
    }
     ...
     * @return void
     */
    public function render_available_fields($form = 0, $context = 'single')
    {
         ...
         * @deprecated 2.9
         */
        $blocklist_field_types = apply_filters_deprecated('gravityview_blacklist_field_types', [[], $context], '2.14', 'gravityview_blocklist_field_types');
        ...
        if (!is_array($blocklist_field_types)) {
            gravityview()->log->error('$blocklist_field_types is not an array', ['data' => print_r($blocklist_field_types, true)]);
        ...
        if (!empty($fields)) {
            foreach ($fields as $id => $details) {
                ...
                $output .= new GravityView_Admin_View_Field($details['label'], $id, $details, [], $form);
            } // End foreach
        }
    ...
    public function render_additional_fields($form = 0, $context = 'single')
    {
        ...
        foreach ((array) $additional_fields as $item) {
            ...
            echo new GravityView_Admin_View_Field($item['label_text'], $item['field_id'], $item, $settings = [], $form);
        }
    }
    ...
    public function get_entry_default_fields($form, $zone)
    {
        ...
        if (in_array($zone, ['directory', 'single'], true)) {
            $meta_fields = GravityView_Fields::get_all(['meta', 'gravityview', 'add-ons']);
    ...
    public function render_active_areas($template_id, $type, $zone, $rows, $values)
    {
         ...
         * @param array  $atts         {
         *
         *   @var string $type 'widget' or 'field'
         *   @var string $template_id The current slug of the selected View template
         *   @var string $zone Where is this button being shown? Either 'single', 'directory', 'edit', 'header', 'footer'
         * }
         */
        $button_label = apply_filters('gravityview/admin/add_button_label', $button_label, ['type' => $type, 'template_id' => $template_id, 'zone' => $zone]);
				...
				<div class="gv-grid-col-<?php echo esc_attr($column); ?>">
					...
					<?php foreach ($areas as $area) { 	?>
						...
						<div class="gv-droppable-area" data-areaid="<?php echo esc_attr($zone.'_'.$area['areaid']); ?>" data-context="<?php echo esc_attr($zone); ?>">
                            <p class="gv-droppable-area-title" <?php if ('widget' === $type && empty($area['subtitle'])) {
                    echo ' style="margin: 0; padding: 0;"';
                } ?>>
								<strong <?php if ('widget' === $type) {
							...
							<div class="active-drop active-drop-<?php echo esc_attr($type); ?>" data-areaid="<?php echo esc_attr($zone.'_'.$area['areaid']); ?>"><?php // render saved fields
                                if (!empty($values[$zone.'_'.$area['areaid']])) {
                                    foreach ($values[$zone.'_'.$area['areaid']] as $uniqid => $field) {
                                        ...
                                        if (!$original_item) {
                                            gravityview()->log->error('An item was not available when rendering the output; maybe it was added by a plugin that is now de-activated.', [' data' => ['available_items' => $available_items, 'field' => $field]]);
    ...
    public function render_widgets_active_areas($template_id = '', $zone = '', $post_id = '')
    {
        ...
        if (!empty($post_id)) {
            if ('auto-draft' === get_post_status($post_id)) {
                ...
                $widgets = [
                    'header_top' => [
                        substr(md5(microtime(true)), 0, 13) => [
                            'id'            => 'search_bar',
     ...
     */
    public function render_directory_active_areas($template_id = '', $context = 'single', $post_id = '', $echo = false)
    {
        if (empty($template_id)) {
            gravityview()->log->debug('[render_directory_active_areas] {template_id} is empty', ['template_id' => $template_id]);
        ...
        if (empty($template_areas)) {
            gravityview()->log->debug('[render_directory_active_areas] No areas defined. Maybe template {template_id} is disabled.', ['data' => $template_id]);
            $output = '<div>';
    ...
    public static function add_scripts_and_styles($hook)
    {
        ...
        if (gravityview()->plugin->is_GF_25() && gravityview()->request->is_admin('', 'single')) {
            ...
            wp_enqueue_style('gravityview_gf_tooltip', plugins_url('assets/css/gf_tooltip.css', GRAVITYVIEW_FILE), [], \GV\Plugin::$version);
            wp_enqueue_script('gravityview_gf_tooltip', plugins_url('assets/js/gf_tooltip'.$script_debug.'.js', GRAVITYVIEW_FILE), [], \GV\Plugin::$version);
        }

        // Add the GV font (with the Astronaut)
        wp_enqueue_style('gravityview_global', plugins_url('assets/css/admin-global.css', GRAVITYVIEW_FILE), [], \GV\Plugin::$version);
        wp_register_style('gravityview_views_styles', plugins_url('assets/css/admin-views.css', GRAVITYVIEW_FILE), ['dashicons', 'wp-jquery-ui-dialog'], \GV\Plugin::$version);

        wp_register_script('gravityview-jquery-cookie', plugins_url('assets/lib/jquery.cookie/jquery.cookie.min.js', GRAVITYVIEW_FILE), ['jquery'], \GV\Plugin::$version, true);
        ...
        // Enqueue scripts
        wp_enqueue_script('gravityview_views_scripts', plugins_url('assets/js/admin-views'.$script_debug.'.js', GRAVITYVIEW_FILE), ['jquery-ui-tabs', 'jquery-ui-draggable', 'jquery-ui-droppable', 'jquery-ui-sortable', 'jquery-ui-tooltip', 'jquery-ui-dialog', 'gravityview-jquery-cookie', 'jquery-ui-datepicker', 'underscore'], \GV\Plugin::$version);

        wp_localize_script('gravityview_views_scripts', 'gvGlobals', [

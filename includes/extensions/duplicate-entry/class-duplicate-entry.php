...
final class GravityView_Duplicate_Entry
{
    ...
    {
        add_action('wp', [$this, 'process_duplicate'], 10000);

        add_filter('gravityview_entry_default_fields', [$this, 'add_default_field'], 10, 3);

        add_action('gravityview_before', [$this, 'maybe_display_message']);
        ...
        add_filter('gravityview_template_duplicate_link_options', [$this, 'duplicate_link_field_options'], 10, 5);

        // add template path to check for field
        add_filter('gravityview_template_paths', [$this, 'add_template_path']);

        // Entry duplication in the backend
        add_action('gform_entries_first_column_actions', [$this, 'make_duplicate_link_row'], 10, 5);

        // Handle duplicate action in the backend
        add_action('gform_pre_entry_list', [$this, 'maybe_duplicate_list']);

        add_filter('gravityview/sortable/field_blocklist', [$this, '_filter_sortable_fields'], 1);

        add_filter('gravityview/field/is_visible', [$this, 'maybe_not_visible'], 10, 3);

        add_filter('gravityview/api/reserved_query_args', [$this, 'add_reserved_arg']);
    }
    ...
    public function duplicate_link_field_options($field_options, $template_id, $field_id, $context, $input_type)
    {
        ...
        $field_options['allow_duplicate_cap'] = [
            'type'    => 'select',
    ...
    {
        if ('edit' !== $zone) {
            $entry_default_fields['duplicate_link'] = [
     ...
     */
    public function add_available_field($available_fields = [])
    {
        $available_fields['duplicate_link'] = [
     ...
     */
    public function modify_visibility_caps($visibility_caps = [], $template_id = '', $field_id = '', $context = '', $input_type = '')
    {
    ...
    public static function get_duplicate_link($entry, $view_id, $post_id = null)
    {
        ...
        if (empty($base)) {
            gravityview()->log->error('Post ID does not exist: {post_id}', ['post_id' => $post_id]);
    ...
    public function process_duplicate()
    {
        ...
        if ($entry) {
            ...
            if (is_wp_error($has_permission)) {
                ...
                if (is_wp_error($duplicate_response)) {
                    ...
                    gravityview()->log->error('Entry {entry_slug} cannot be duplicated: {error_code} {error_message}', [
            ...
            gravityview()->log->error('Duplicate entry failed: there was no entry with the entry slug {entry_slug}', ['entry_slug' => $entry_slug]);
        ...
        $redirect_to_base = esc_url_raw(remove_query_arg(['action', 'gvid', 'entry_id']));
    ...
    private function duplicate_entry($entry)
    {
        ...
        gravityview()->log->debug('Starting duplicate entry: {entry_id}', ['entry_id' => $entry_id]);
        ...
        $row['source_url'] = esc_url_raw(remove_query_arg(['action', 'gvid', 'result', 'duplicate', 'entry_id']));
        ...
        $reset_meta = ['is_approved', 'gravityview_unique_id', 'workflow_current_status_timestamp'];
        foreach ($reset_meta as $meta_key) {
            $duplicate_meta->filter(['meta_key' => $meta_key], 'NOT');
        }
        ...
        gravityview()->log->debug('Duplicate response: {duplicate_response}', ['duplicate_response' => $duplicate_response]);
     ...
     *
     * @return bool|WP_Error True: can edit form. WP_Error: nope.
     */
    private function user_can_duplicate_entry($entry = [], $view_id = null)
    {
        $error = null;
        ...
        if (!self::check_user_cap_duplicate_entry($entry, [], $view_id)) {
        ...
        gravityview()->log->error('{error}', ['erorr' => $error]);
     ...
     * @return bool
     */
    public static function check_user_cap_duplicate_entry($entry, $field = [], $view_id = 0)
    {
        ...
        if (GVCommon::has_cap(['gravityforms_edit_entries', 'gform_full_access', 'gravityview_full_access'], $entry_id)) {
        ...
        if (!empty($field)) {
            ...
            if (empty($field['allow_duplicate_cap'])) {
                gravityview()->log->error('Cannot read duplicate entry field caps', ['data' => $field]);
            ...
            if (GVCommon::has_cap($field['allow_duplicate_cap'])) {
                ...
                gravityview()->log->debug('User {user_id} is not authorized to view duplicate entry link ', ['user_id' => $current_user->ID]);
        ...
        if (is_user_logged_in() && intval($current_user->ID) === intval($entry['created_by'])) {
            gravityview()->log->debug('User {user_id} created the entry.', ['user_id' => $current_user->ID]);
    ...
    public function maybe_duplicate_list($form_id)
    {
        ...
        if (is_wp_error($is_duplicated)) {
            gravityview()->log->error('Error duplicating {id}: {error}', ['id' => $entry_id, 'error' => $is_duplicated->get_error_message()]);
        }

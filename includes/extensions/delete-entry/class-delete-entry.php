...
final class GravityView_Delete_Entry
{
    ...
    {
        add_action('wp', [$this, 'process_delete'], 10000);
        ...
        add_action('gravityview/edit-entry/publishing-action/after', [$this, 'add_delete_button'], 10, 4);

        add_action('gravityview/delete-entry/deleted', [$this, 'process_connected_posts'], 10, 2);
        add_action('gravityview/delete-entry/trashed', [$this, 'process_connected_posts'], 10, 2);
     ...
     * @return void
     */
    public function add_delete_button($form = [], $entry = [], $view_id = null, $post_id = null)
    {

        // Only show the link to those who are allowed to see it.
        if (!self::check_user_cap_delete_entry($entry, [], $view_id)) {
            return;
        }
         ...
         *
         * @param bool $show_entry Default: true
         */
    ...
    public function process_delete()
    {
        ...
        $delete_redirect_base = esc_url_raw(remove_query_arg(['action', 'gvid', 'entry_id']));

        if (!$entry) {
            gravityview()->log->debug('Delete entry failed: there was no entry with the entry slug {entry_slug}', ['entry_slug' => $entry_slug]);
    ...
    private function delete_or_trash_entry($entry)
    {
        ...
        if ('delete' === $mode) {
            gravityview()->log->debug('Starting delete entry: {entry_id}', ['entry_id' => $entry_id]);
            ...
            gravityview()->log->debug('Delete response: {delete_response}', ['delete_response' => $delete_response]);
        } else {
            gravityview()->log->debug('Starting trash entry: {entry_id}', ['entry_id' => $entry_id]);
            ...
            gravityview()->log->debug(' Trashed? {delete_response}', ['delete_response' => $delete_response]);
        }
     ...
     */
    public function process_connected_posts($entry_id = 0, $entry = [])
    {
     ...
     */
    public function user_can_delete_entry($entry = [], $view_id = null)
    {
        $error = null;
     ...
     * @return bool
     */
    public static function check_user_cap_delete_entry($entry, $field = [], $view = 0)
    {
        if (!$view) {
            /** @deprecated path */
        ...
        if (GVCommon::has_cap(['gravityforms_delete_entries', 'gravityview_delete_others_entries'], $entry_id)) {
        ...
        if (!empty($field)) {
            ...
            if (empty($field['allow_edit_cap'])) {
                gravityview()->log->error('Cannot read delete entry field caps', ['data' => $field]);
            ...
            if (GVCommon::has_cap($field['allow_edit_cap'])) {
                ...
                gravityview()->log->debug('User {user_id} is not authorized to view delete entry link ', ['user_id' => $current_user->ID]);

...
class GravityView_Field_Notes extends GravityView_Field
{
    ...
    public function register_scripts()
    {
        ...
        wp_register_style('gravityview-notes', $css_file, [], GravityView_Plugin::version);
        wp_register_script('gravityview-notes', plugins_url('/assets/js/entry-notes.js', GravityView_Field_Notes::$file), ['jquery'], GravityView_Plugin::version, true);
    }
    ...
    public function enqueue_scripts()
    {
        ...
        if (!wp_script_is('gravityview-notes', 'done')) {
            ...
            wp_localize_script('gravityview-notes', 'GVNotes', [
     ...
     *
     * @var array {
     * @var string $action "gv_note_add"
     * @var string $entry-slug Entry slug or ID to add note to
     * @var string $gv_note_add Nonce with action "gv_note_add_{entry slug}" and name "gv_note_add"
     * @var string $_wp_http_referer Relative URL to submitting page ('/view/example/entry/123/')
     * @var string $gv-note-content Note content
     * @var string $add_note Submit button value ('Add Note')
     *            }
     *
     * @return void
     */
     ...
     *
     * @var array $data {
     * @var string $action "gv_delete_notes"
     * @var string $entry-slug Entry slug or ID to add note to
     * @var string $gv_delete_notes Nonce with action "gv_delete_notes_{entry slug}" and name "gv_delete_notes"
     * @var string $_wp_http_referer Relative URL to submitting page ('/view/example/entry/123/')
     * @var int[]  $note  Array of Note IDs to be deleted
     *            }
     *
     * @return void
     */
    ...
    public static function display_note($note, $show_delete = false, $context = null)
    {
         ...
         * @param object $note         Note object with id, user_id, date_created, value, note_type, user_name, user_email vars
         * @param bool   $show_delete  True: Notes are editable. False: no editing notes.
         *
         * @since 2.0
         *
         * @param \GV\Template_Context $context The context.
    ...
    private function maybe_send_entry_notes($note = false, $entry, $data)
    {
        ...
        if (!empty($data['gv-note-to'])) {
            ...
            if (!GFCommon::is_valid_email_list($to)) {
                gravityview()->log->error('$to not a valid email or email list (CSV of emails): {to}', ['to' => print_r($to, true), 'data' => $email_data]);
            ...
            $form = isset($entry['form_id']) ? GFAPI::get_form($entry['form_id']) : [];

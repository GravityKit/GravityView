...
class GravityView_Field_Notes extends GravityView_Field {
	...
	static public function strings( $key = '' ) {
		...
		/**
		 * @hook gravityview/field/notes/strings Modify the text used in the Entry Notes field. Sanitized by `esc_html` after return.
		 * @since 1.17
	...
	static public function display_note( $note, $show_delete = false, $context = null ) {
		...
		/**
		 * @hook gravityview/field/notes/content Modify the note content before rendering in the template
		 * @since 1.17
	...
	private static function get_note_emails_array() {
		...
		/**
		 * @hook gravityview/field/notes/emails Modify the dropdown values displayed in the "Also email note to" dropdown
		 * @since 1.17
	...
	private static function get_note_email_fields( $entry_slug = '' ) {
		...
		/**
		 * @hook gravityview/field/notes/custom-email Whether to include a Custom Email option for users to define a custom email to mail notes to
		 * @since 1.17
	...
	private function maybe_send_entry_notes( $note = false, $entry, $data ) {
		...
		if ( ! empty( $data['gv-note-to'] ) ) {
			...
			/**
			 * @hook gravityview/field/notes/email_content Modify the values passed when sending a note email
			...
			/**
             * @hook gravityview/field/notes/wpautop_email Should the message content have paragraphs added automatically, if using HTML message format
			 * @since 1.18

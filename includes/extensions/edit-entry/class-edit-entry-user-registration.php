...
class GravityView_Edit_Entry_User_Registration
{
    ...
    {
        add_action('wp', [$this, 'add_hooks'], 10);
    }
    ...
    public function add_hooks()
    {
         ...
         * @since 1.11
         *
         * @param bool $boolean Whether to trigger update on user registration (default: true)
         */
        if (apply_filters('gravityview/edit_entry/user_registration/trigger_update', true)) {
            add_action('gravityview/edit_entry/after_update', [$this, 'update_user'], 10, 2);

            // last resort in case the current user display name don't match any of the defaults
            add_action('gform_user_updated', [$this, 'restore_display_name'], 10, 4);
        }
    }
     ...
     * @return void
     */
    public function update_user($form = [], $entry_id = 0)
    {
        if (!class_exists('GFAPI') || !class_exists('GF_User_Registration')) {
            ...
            gravityview()->log->error('Entry ID is empty [{entry_id}]; not updating the user', ['entry_id' => $entry_id]);
        ...
        remove_action('gravityview/edit_entry/after_update', [$this, 'update_user'], 10);
    }
     ...
     */
    public function restore_display_name($user_id = 0, $config = [], $entry = [], $password = '')
    {
        ...
        if (!$user_after_update) {
            gravityview()->log->error('User not found at $user_id #{user_id}', ['user_id' => $user_id]);
        ...
        if (is_wp_error($updated)) {
            gravityview()->log->error('There was an error updating user #{user_id} details', ['user_id' => $user_id, 'data' => $updated]);
        } else {
            gravityview()->log->debug('User #{user_id} details restored', ['user_id' => $user_id]);
        }

...
class GravityView_Edit_Entry_Render
{
     ...
     * @var array
     */
    public $fields_with_calculation = [];
    ...
    public function __construct(GravityView_Edit_Entry $loader)
    {
        $this->loader = $loader;
    }
    ...
    public function load()
    {
        ...
        add_action('wp_head', [$this, 'prevent_render_form']);
        add_action('wp_footer', [$this, 'prevent_render_form']);

        // Stop Gravity Forms processing what is ours!
        add_action('wp', [$this, 'prevent_maybe_process_form'], 8);
        add_action('admin_init', [$this, 'prevent_maybe_process_form'], 8);

        add_filter('gravityview_is_edit_entry', [$this, 'is_edit_entry']);

        add_action('gravityview_edit_entry', [$this, 'init'], 10, 4);

        // Disable conditional logic if needed (since 1.9)
        add_filter('gform_has_conditional_logic', [$this, 'manage_conditional_logic'], 10, 2);

        // Make sure GF doesn't validate max files (since 1.9)
        add_filter('gform_plupload_settings', [$this, 'modify_fileupload_settings'], 10, 3);

        // Add fields expected by GFFormDisplay::validate()
        add_filter('gform_pre_validation', [$this, 'gform_pre_validation']);

        // Fix multiselect value for GF 2.2
        add_filter('gravityview/edit_entry/field_value_multiselect', [$this, 'fix_multiselect_value_serialization'], 10, 3);
    }
    ...
    public function prevent_maybe_process_form()
    {
        ...
        remove_action('wp', ['RGForms', 'maybe_process_form'], 9);
        remove_action('wp', ['GFForms', 'maybe_process_form'], 9);

        remove_action('admin_init', ['GFForms', 'maybe_process_form'], 9);
        remove_action('admin_init', ['RGForms', 'maybe_process_form'], 9);
    }
    ...
    public function init($gv_data = null, $entry = null, $view = null, $request = null)
    {
        ...
        if (false === $this->user_can_edit_entry(true)) {
            gravityview()->log->error('User is not allowed to edit this entry; returning', ['data' => $this->entry]);
    ...
    private function print_scripts()
    {
        ...
        wp_register_script('gform_gravityforms', GFCommon::get_base_url().'/js/gravityforms.js', ['jquery', 'gform_json', 'gform_placeholder', 'sack', 'plupload-all', 'gravityview-fe-view']);

        GFFormDisplay::enqueue_form_scripts($gravityview_view->getForm(), false);

        wp_localize_script('gravityview-fe-view', 'gvGlobals', ['cookiepath' => COOKIEPATH]);

        // Sack is required for images
        wp_print_scripts(['sack', 'gform_gravityforms', 'gravityview-fe-view']);
    ...
    private function process_save($gv_data)
    {
        ...
        gravityview()->log->debug('$_POSTed data (sanitized): ', ['data' => esc_html(print_r($_POST, true))]);
        ...
        if ($this->is_valid) {
            ...
            gravityview()->log->error('Submission is NOT valid.', ['entry' => $this->entry]);
        }
    } // process_save
    ...
    private function unset_hidden_field_values()
    {
        ...
        $this->unset_hidden_calculations = [];
        ...
        foreach ($this->entry as $input_id => $field_value) {
            ...
            if (GFFormsModel::is_field_hidden($this->form, $field, [], $this->entry)) {
    ...
    private function preset_approval_fields()
    {
        ...
        add_filter('gravityview/approve_entries/update_unapproved_meta', [$this, 'prevent_update_unapproved_meta'], 9, 3);
    }
    ...
    {
        remove_filter('gravityview/approve_entries/update_unapproved_meta', [$this, 'prevent_update_unapproved_meta'], 9);
    ...
    private function process_save_process_files($form_id)
    {
         ...
         * @since 1.20.1
         */
        add_filter("gform_save_field_value_$form_id", [$this, 'save_field_value'], 99, 5);
    ...
    private function maybe_update_post_fields($form)
    {
        ...
        if (false === GVCommon::has_cap('edit_post', $post_id)) {
            gravityview()->log->error('The current user does not have the ability to edit Post #{post_id}', ['post_id' => $post_id]);
        ...
        if ($update_entry) {
            ...
            if (is_wp_error($return_entry)) {
                gravityview()->log->error('Updating the entry post fields failed', ['data' => ['$this->entry' => $this->entry, '$return_entry' => $return_entry]]);
            } else {
                gravityview()->log->debug('Updating the entry post fields for post #{post_id} succeeded', ['post_id' => $post_id]);
            }
        }
        ...
        if (is_wp_error($return_post)) {
            ...
            gravityview()->log->error('Updating the post content failed', ['data' => compact('updated_post', 'return_post')]);
        } else {
            gravityview()->log->debug('Updating the post content for post #{post_id} succeeded', ['post_id' => $post_id, 'data' => $updated_post]);
        }
    }
    ...
    private function after_update()
    {
         ...
         * @since develop
         */
        if ($allowed_feeds = $this->view->settings->get('edit_feeds', [])) {
                ...
                foreach (GFAddOn::get_registered_addons() as $registered_feed) {
                    if (is_subclass_of($registered_feed, 'GFFeedAddOn')) {
                        if (method_exists($registered_feed, 'get_instance')) {
                            $registered_feed = call_user_func([$registered_feed, 'get_instance']);
    ...
    public function edit_entry_form()
    {
         ...
         */
        echo gravityview_strip_whitespace($javascript); ?><h2 class="gv-edit-entry-title">
				<span><?php
        ...
        echo esc_attr($edit_entry_title); ?></span>
			</h2>
    ...
    private function maybe_print_message()
    {
        ...
        $back_link = remove_query_arg(['page', 'view', 'edit']);
    ...
    private function render_edit_form()
    {
        ...
        remove_filter('gform_pre_render', [$this, 'filter_modify_form_fields'], 5000);
        remove_filter('gform_submit_button', [$this, 'render_form_buttons']);
        remove_filter('gform_next_button', [$this, 'render_form_buttons']);
        remove_filter('gform_previous_button', [$this, 'render_form_buttons']);
        remove_filter('gform_disable_view_counter', '__return_true');
        remove_filter('gform_field_input', [$this, 'verify_user_can_edit_post'], 5);
        remove_filter('gform_field_input', [$this, 'modify_edit_field_input'], 10);
     ...
     *
     * @param array        $form
     * @param bool         $ajax         Whether in AJAX mode
    ...
    public function modify_edit_field_input($field_content = '', $field = null, $value = '', $lead_id = 0, $form_id = 0)
    {
        ...
        if (
            ($this->is_edit_entry_submission() && !in_array($field->type, ['fileupload', 'post_image']))
            && false === ($gv_field && is_callable([$gv_field, 'get_field_input']))
            && !GFCommon::is_product_field($field->type)
            || !empty($field_content)
            || in_array($field->type, ['honeypot'])
        ) {
            return $field_content;
        }
        ...
        if ($gv_field && is_callable([$gv_field, 'get_field_input'])) {
        ...
        if (!empty($warnings)) {
            gravityview()->log->error('{warning}', ['warning' => $warnings, 'data' => $field_value]);
        }
    ...
    private function get_field_value($field)
    {
         ...
         *
         * @param bool True: override saved values; False: don't override (default)
        ...
        if (isset($field->inputs) && is_array($field->inputs) && !in_array($field->type, ['time', 'date'])) {
            $field_value = [];
            ...
            $pre_value = $field->get_value_submission([], false);
            ...
            $pre_value = $field->allowsPrepopulate ? GFFormsModel::get_parameter_value($field->inputName, [], $field) : '';
    ...
    public function gform_pre_validation($form)
    {
        ...
        foreach ($form['fields'] as &$field) {
            ...
            foreach (['noDuplicates', 'adminOnly', 'inputType', 'isRequired', 'enablePrice', 'inputs', 'allowedExtensions'] as $key) {
    ...
    private function validate()
    {
        ...
        if (class_exists('GF_User_Registration')) {
            remove_filter('gform_validation', [GF_User_Registration::get_instance(), 'validate']);
        } elseif (class_exists('GFUser')) {
            remove_filter('gform_validation', ['GFUser', 'user_registration_validation']);
        }
         ...
         */
        add_filter('gform_validation_'.$this->form_id, [$this, 'custom_validation'], 10, 4);
        ...
        remove_filter('gform_validation_'.$this->form_id, [$this, 'custom_validation'], 10);
    }
     ...
     *   @var bool $is_valid
     *   @var array $form
     *   @var int $failed_validation_page The page number which has failed validation.
     * }
     *
     * @return array
     */
    ...
    {
        gravityview()->log->debug('GravityView_Edit_Entry[custom_validation] Validation results: ', ['data' => $validation_results]);

        gravityview()->log->debug('GravityView_Edit_Entry[custom_validation] $_POSTed data (sanitized): ', ['data' => esc_html(print_r($_POST, true))]);
        ...
        foreach ($validation_results['form']['fields'] as $key => &$field) {
            ...
            if (!empty($field->failed_validation)) {
                gravityview()->log->debug('GravityView_Edit_Entry[custom_validation] Field is invalid.', ['data' => ['field' => $field, 'value' => $value]]);
                ...
                if (!empty($field->noDuplicates)) {
                    ...
                    if (!empty($entry) && $value == $entry[$field->id]) {
                        ...
                        gravityview()->log->debug('GravityView_Edit_Entry[custom_validation] Field not a duplicate; it is the same entry.', ['data' => $entry]);
    ...
    private function filter_fields($fields, $configured_fields)
    {
        ...
        if (empty($configured_fields) && apply_filters('gravityview/features/paged-edit', false)) {
            $field_type_blocklist = array_diff($field_type_blocklist, ['page']);
        }
     ...
     */
    private function filter_admin_only_fields($fields = [], $edit_fields = null, $form = [], $view_id = 0)
    {
         ...
         * @since 1.9.1
         *
         * @param bool  $use_gf_adminonly_setting True: Hide field if set to Admin Only in GF and the user is not an admin. False: show field based on GV permissions, ignoring GF permissions.
         * @param array $form                     GF Form array
         * @param int   $view_id                  View ID
         */
     ...
     */
    public function prefill_conditional_logic($form)
    {
            ...
            if (!empty($field->conditionalLogic) && !empty($field->conditionalLogic['rules'])) {
                foreach ($field->conditionalLogic['rules'] as $i => $rule) {
                    if (!in_array($rule['fieldId'], $editable_ids)) {
                        ...
                        if ($match) {
                            $remove_conditions_rule[] = [$field['id'], $i];
                        }
                    }
                }
            }
            ...
            foreach ($form['fields'] as &$field) {
                foreach ($remove_conditions_rule as $_remove_conditions_r) {
                    ...
                    if ($field['id'] == $rule_field_id) {
                        ...
                        gravityview()->log->debug('Removed conditional rule #{rule} for field {field_id}', ['rule' => $rule_i, 'field_id' => $field['id']]);
                    }
                }
            }
        }
     ...
     *
     * @param bool $echo Show error messages in the form?
     *
     * @return bool True: can edit form. False: nope.
     */
    private function user_can_edit_entry($echo = false)
    {
        ...
        if ($echo && $error !== true) {
             ...
             */
            if (!empty($this->entry)) {
                $error .= ' '.gravityview_get_link('#', _x('Go back.', 'Link shown when invalid Edit Entry link is clicked', 'gravityview'), ['onclick' => 'window.history.go(-1); return false;']);
            }

            echo GVCommon::generate_notice(wpautop($error), 'gv-error error');
        }

        gravityview()->log->error('{error}', ['error' => $error]);
     ...
     * @param bool  $echo  Whether to show error message telling user they aren't allowed
     *
     * @return bool True: user can edit the current field; False: nope, they can't.
     */

...
abstract class GravityView_Lightbox_Provider {
	 ...
	 * @since 2.45.1 Deprecated in favor of add_inline_scripts_and_styles().
	 * @deprecated 2.45.1 Use add_inline_scripts_and_styles() instead.
	 *
	 * @return void
	 */
	protected function print_scripts() {
		_deprecated_function( __METHOD__, '2.45.1', get_class( $this ) . '::add_inline_scripts_and_styles()' );
		$this->add_inline_scripts_and_styles();
	}

	/**
	 * Add inline scripts and styles for the lightbox after enqueueing.
	 *
	 * @since 2.45.1
	 *
	 * @return void
	 */
	protected function add_inline_scripts_and_styles() {
		// This method is intentionally empty in the base class.
		// Child classes can override to add custom inline scripts and styles after enqueuing.
	}
	...
	public function remove_hooks() {
		...
		remove_action( 'wp_enqueue_scripts', array( $this, 'register_styles' ) );
	...
	public function enqueue_scripts() {
		if ( empty( static::$script_slug ) ) {
			return;
		}

		// Only enqueue if not already enqueued
		if ( wp_script_is( static::$script_slug, 'enqueued' ) ) {
			return;
		}

		wp_enqueue_script( static::$script_slug );

		// Allow child classes to add inline scripts and styles.
		$this->add_inline_scripts_and_styles();
	}
	...
	public function enqueue_styles() {
		if ( empty( static::$style_slug ) ) {
			return;
		}

		// Only enqueue if not already enqueued
		if ( wp_style_is( static::$style_slug, 'enqueued' ) ) {
			return;
		}

		wp_enqueue_style( static::$style_slug );
	}
	...
	public function filter_entry_link_output( $output, $args ) {
		...
		// Enqueue the scripts and styles for the lightbox.
		$this->enqueue_scripts();
		$this->enqueue_styles();

...
class GravityView_Ajax
{
     ...
     * @return void
     */
    public function check_ajax_nonce()
    {
     ...
     */
    public function get_available_fields_html()
    {
     ...
     * @return void
     */
    public function get_preset_fields_config()
    {
        $this->check_ajax_nonce();
     ...
     * @return void
     */
    public function create_preset_form()
    {
        $this->check_ajax_nonce();
        ...
        if (false === $form) {
            // send error to user
            gravityview()->log->error('Error importing form for template id: {template_id}', ['template_id' => (int) $_POST['template_id']]);
     ...
     */
    public function import_form($xml_or_json_path = '')
    {
        ...
        if (empty($xml_or_json_path) || !class_exists('GFExport') || !file_exists($xml_or_json_path)) {
            gravityview()->log->error('Class GFExport or file not found. file: {path}', ['path' => $xml_or_json_path]);
     ...
     */
    public static function pre_get_form_fields($template_id = '')
    {
        if (empty($template_id)) {
            ...
            if (!file_exists($form_file)) {
                gravityview()->log->error('Importing Form Fields for preset [{template_id}]. File not found. file: {path}', ['template_id' => $template_id, 'path' => $form_file]);
        ...
        if (file_exists($xml_parser)) {
            ...
            gravityview()->log->debug(' - Gravity Forms XML Parser not found {path}.', ['path' => $xml_parser]);
        ...
        $options = [
            'page'         => ['unserialize_as_array' => true],
            'form'         => ['unserialize_as_array' => true],
            'field'        => ['unserialize_as_array' => true],
            'rule'         => ['unserialize_as_array' => true],
            'choice'       => ['unserialize_as_array' => true],
            'input'        => ['unserialize_as_array' => true],
            'routing_item' => ['unserialize_as_array' => true],
            'creditCard'   => ['unserialize_as_array' => true],
            'routin'       => ['unserialize_as_array' => true],
            'confirmation' => ['unserialize_as_array' => true],
            'notification' => ['unserialize_as_array' => true],
        ];
        ...
        if (!$forms) {
            gravityview()->log->error('Importing Form Fields for preset [{template_id}]. Error importing file. (File) {path}', ['template_id' => $template_id, 'path' => $form_file]);
        ...
        if (empty($form)) {
            gravityview()->log->error('$form not set.', ['data' => $forms]);

            return false;
        }

        gravityview()->log->debug('[pre_get_available_fields] Importing Form Fields for preset [{template_id}]. (Form)', ['template_id' => $template_id, 'data' => $form]);
     ...
     */
    public function import_fields($file)
    {
        ...
        if (empty($presets['posts'][0]['postmeta']) && !is_array($presets['posts'][0]['postmeta'])) {
            gravityview()->log->error('Importing Preset Fields failed. Meta not found in file. {path}', ['path' => $file]);

...
class GravityView_API {
	...
	public static function entry_link_html( $entry = array(), $anchor_text = '', $passed_tag_atts = array(), $field_settings = array(), $base_id = null ) {
		...
		/**
		 * @hook gravityview_field_entry_link Modify the link HTML
	...
	public static function no_results( $wpautop = true, $context = null ) {
		...
		/**
		 * @hook gravitview_no_entries_text Modify the text displayed when there are no entries.
		...
		/**
		 * @hook gravityview/template/text/no_entries Modify the text displayed when there are no entries.
		 * @since 2.0
		 * @param string $output The existing "No Entries" text
		 * @param boolean $is_search Is the current page a search result, or just a multiple entries screen?
		 * @param \GV\Template_Context $context The context.
	...
	public static function directory_link( $post_id = NULL, $add_query_args = true, $context = null ) {
		...
		/**
		 * @hook gravityview_directory_link Modify the URL to the View "directory" context
		 * @since 1.19.4
		...
		/**
		 * @hook gravityview/view/links/directory Modify the URL to the View "directory" context
		 * @since 2.0
		 * @param string $link URL to the View's "directory" context (Multiple Entries screen)
	...
	private static function get_custom_entry_slug( $id, $entry = array() ) {
		...
		/**
		 * @hook gravityview_entry_slug Modify the unique hash ID generated, if you want to improve usability or change the format. This will allow for custom URLs, such as `{entryid}-{first-name}` or even, if unique, `{first-name}-{last-name}`
	...
	public static function get_entry_slug( $id_or_string, $entry = array() ) {
		...
		/**
		 * @hook gravityview_custom_entry_slug Whether to enable and use custom entry slugs.
    ...
    public static function entry_create_custom_slug( $entry, $form ) {
        /**
         * @hook gravityview_custom_entry_slug On entry creation, check if we are using the custom entry slug feature and update the meta
	...
	public static function entry_link( $entry, $post_id = NULL, $add_directory_args = true, $view_id = 0 ) {
		...
		/**
		 * @hook gravityview/entry_link/add_query_args Modify whether to include passed $_GET parameters to the end of the url
		 * @since 2.10
...
function gv_get_query_args() {
	...
	/**
	 * @hook gravityview/api/reserved_query_args Modify the URL arguments that should not be used because they are internal to GravityView
	 * @since 2.10
...
function gv_container_class( $passed_css_class = '', $echo = true, $context = null ) {
	...
	/**
	 * @hook gravityview/render/container/class Modify the CSS class to be added to the wrapper <div> of a View
	 * @since 1.5.4
...
function gv_value( $entry, $field ) {
	...
	if( $value === '' ) {
		/**
		 * @hook gravityview_empty_value What to display when a field is empty
...
function gravityview_back_link( $context = null ) {
	...
	/**
	 * @hook gravityview_go_back_url Modify the back link URL
	 * @since 1.17.5
	...
	/**
	 * @hook gravityview/template/links/back/url Modify the back link URL
	 * @since 2.0
	 * @see gv_directory_link() Generated the original back link
	 * @param string $href Existing label URL
	...
	/**
	 * @hook gravityview_go_back_label Modify the back link text
	 * @since 1.0.9
	...
	/**
	 * @hook gravityview/template/links/back/label Modify the back link text
	 * @since 2.0
	 * @see gv_directory_link() Generated the original back link
	 * @param string $label Existing label text
	 * @param \GV\Template_Context The context.
	...
	/**
	 * @hook gravityview/template/links/back/atts Modify the attributes used on the back link anchor tag
	 * @since 2.1
...
function gravityview_before() {
	/**
	 * @hook gravityview/template/before Append content to the view.
	...
	if ( count( $args = func_get_args() ) ) {
		...
		if ( $gravityview instanceof \GV\Template_Context ) {
			/**
			 * @hook gravityview/template/before Prepend content to the view.
	...
	/**
	 * @hook gravityview_before Prepend content to the View container `<div>`
	 * @deprecated Use `gravityview/template/before`.
...
function gravityview_header() {
	/**
	 * @hook gravityview/template/header Append content to the view.
	 * @param object $gravityview The $gravityview object available in templates.
	 */
	if ( count( $args = func_get_args() ) ) {
		$gravityview = reset( $args );
		if ( $gravityview instanceof \GV\Template_Context ) {
			/**
			 * @hook gravityview/template/header Prepend content to the view container <div>.
			 * @param \GV\Template_Context $gravityview The $gravityview object available in templates.
			 */
	...
	/**
	 * @hook gravityview_header Prepend content to the View container `<div>`
	 * @deprecated Use `gravityview/template/header`.
	 * @param int $view_id The ID of the View being displayed
	 */
...
function gravityview_footer() {
	/**
	 * @hook gravityview/template/footer Append content to the view.
	 * @param object $gravityview The $gravityview object available in templates.
	 */
	if ( count( $args = func_get_args() ) ) {
		$gravityview = reset( $args );
		if ( $gravityview instanceof \GV\Template_Context ) {
			/**
			 * @hook gravityview/template/footer Prepend outside of the view container <div>.
			 * @param \GV\Template_Context $gravityview The $gravityview object available in templates.
			 */
	...
	/**
	 * @hook gravityview_after Display content after a View. Used to render footer widget areas. Rendered outside the View container `<div>`
	 * @deprecated Use `gravityview/template/footer`.
	 * @param int $view_id The ID of the View being displayed
	 */
...
function gravityview_after() {
	if ( count( $args = func_get_args() ) ) {
		...
		if ( $gravityview instanceof \GV\Template_Context ) {
			/**
			 * @hook gravityview/template/after Append content to the view.
			 * @param \GV\Template_Context $gravityview The $gravityview object available in templates.
			 */
	...
	/**
	 * @hook gravityview_after Append content to the View container `<div>`
	 * @deprecated Use `gravityview/template/after`
	 * @param int $view_id The ID of the View being displayed
	 */
...
function gravityview_get_map_link( $address ) {
	...
	/**
	 * @hook gravityview_map_link Modify the map link generated. You can use a different mapping service, for example.
...
function gravityview_field_output( $passed_args, $context = null ) {
	...
	/**
	 * @hook gravityview/field_output/args Modify the args before generation begins
	 * @since 1.7
	...
	/**
	 * @hook gravityview/template/field_output/context Modify the context before generation begins.
	 * @since 2.0
	 * @param \GV\Template_Context $context The context.
	...
	/**
	 * @hook gravityview/field_output/pre_html Allow Pre filtering of the HTML
	 * @since 1.11
	...
	/**
	 * @hook gravityview/field_output/open_tag Modify the opening tags for the template content placeholders
	 * @since 1.11
	...
	/**
	 * @hook gravityview/field_output/close_tag Modify the closing tags for the template content placeholders
	 * @since 1.11
	...
	/**
	 * @hook gravityview_field_output Modify field HTML output
	...
	/**
	 * @hook gravityview/field_output/html Modify field HTML output
	 * @param string $html Existing HTML output
	 * @param array $args Arguments passed to the function
	 * @since 2.0
	 * @param \GV\Template_Context $context The context.

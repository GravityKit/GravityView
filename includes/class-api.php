...
class GravityView_API
{
     ...
     * @param bool  $force_show_label Whether to always show the label, regardless of field settings
     *
     * @return string
     */
     ...
     *
     * @param bool $wpautop Apply wpautop() to the output?
     *
     * @since 2.0
     *
    ...
    public static function no_results($wpautop = true, $context = null)
    {
        ...
        $output = wp_kses(
            ...
            [
                ...
                'b'      => ['class' => [], 'id' => []],
                'em'     => ['class' => [], 'id' => []],
                'a'      => ['class' => [], 'id' => [], 'href' => [], 'title' => [], 'rel' => [], 'target' => []],
                'div'    => ['class' => [], 'id' => []],
                'br'     => [],
            ]
        );
     ...
     * @param bool $add_query_args Add pagination and sorting arguments
     *
     * @since 2.0
     *
    ...
    public static function get_entry_slug($id_or_string, $entry = [])
    {
        ...
        if ($custom) {
            ...
            if (empty($value) || $value !== $hash) {
                gravityview()->log->debug('Setting hash for entry {entry}: {hash}', ['entry' => $id_or_string, 'hash' => $hash]);
    ...
    public static function entry_create_custom_slug($entry, $form)
    {
         ...
         *
         * @param bool $custom Should we process the custom entry slug?
         */
        $custom = apply_filters('gravityview_custom_entry_slug', false);
        if ($custom) {
            ...
            gravityview()->log->debug('Setting hash for entry {entry_id}: {hash}', ['entry_id' => $entry['id'], 'hash' => $hash]);
     ...
     * @param bool      $add_directory_args True: Add args to help return to directory; False: only include args required to get to entry.
     * @param int       $view_id
     *
...
function gv_class($field, $form = null, $entry = [])
{
...
function gravityview_back_link($context = null)
{
     ...
     * @param \GV\Template_Context The context.
     */
    $atts = apply_filters('gravityview/template/links/back/atts', ['data-viewid' => $view_id], $context);
 ...
 * @param bool   $link     Add links or no?
...
function gravityview_get_current_views()
{
    ...
    if (!is_a($fe->getGvOutputData(), 'GravityView_View_Data')) {
        gravityview()->log->debug('gv_output_data not an object or get_view not callable.', ['data' => $fe->getGvOutputData()]);
...
function gravityview_get_context()
{
    ...
    if ($is_edit_entry) {
        ...
        return 'single';
    } elseif (gravityview()->request->is_view()) {
        return 'directory';
    }
 ...
 */
function gravityview_get_map_link($address, $atts = [])
{
    $address_qs = str_replace(['<br />', "\n"], ' ', $address); // Replace \n with spaces
...
function gravityview_field_output($passed_args, $context = null)
{
    ...
    if ($context instanceof \GV\Template_Context) {
        if (!$context->field || !$context->view || !$context->view->form) {
            gravityview()->log->error('Field or form are empty.', ['data' => [$context->field, $context->view->form]]);
        ...
        if (empty($args['field']) || empty($args['form'])) {
            gravityview()->log->error('Field or form are empty.', ['data' => $args]);
    ...
    if ($context instanceof \GV\Template_Context) {
        $entry = $args['entry'] ?: ($context->entry ? $context->entry->as_entry() : []);
        $field = $args['field'] ?: ($context->field ? $context->field->as_configuration() : []);
        $form = $args['form'] ?: ($context->view->form ? $context->view->form->form : []);
    } else {
        // @deprecated path
        $entry = empty($args['entry']) ? [] : $args['entry'];
        $field = $args['field'];
        $form = $args['form'];
    }
    ...
    $html = str_replace([' style=""', ' class=""', ' id=""'], '', $html);

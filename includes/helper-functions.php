...
function gravityview_css_url($css_file = '', $dir_path = '')
{
    ...
    if (file_exists($template_css_path)) {
        ...
        gravityview()->log->debug('Stylesheet override ({css_file})', ['css_file' => esc_attr($css_file)]);
    } else {
...
function gravityview_ob_include($file_path, $object = null)
{
    if (!file_exists($file_path)) {
        gravityview()->log->error('File path does not exist. {path}', ['path' => $file_path]);
 ...
 * @since 1.13
 *
 * @param int|float|string|float $number    A number to format
 * @param int|string             $decimals  Optional. Precision of the number of decimal places. Default '' (use existing number of decimals)
 * @param bool                   $separator Separate with dots or commas, etc. Default: true.
 *
...
function gravityview_format_link($value = null)
{
     ...
     * @since 1.5.1
     *
     * @param bool $enable Whether to strip the scheme. Return false to show scheme. (default: true)\n
     ...
     * @since 1.5.1
     *
     * @param bool $enable Whether to strip www. Return false to show www. (default: true)\n
     ...
     * @since 1.5.1
     *
     * @param bool $enable Whether to strip subdomains. Return false to show subdomains. (default: true)\n
     ...
     * @since 1.5.1
     *
     * @param bool $enable Whether to enable "root only". Return false to show full path. (default: true)\n
 ...
 * @param bool  $zero_is_empty         Should the number zero be treated as an empty value?
 * @param bool  $allow_string_booleans Whether to check if 'yes', 'true' => `true` and 'no', 'false' => `false`
 *
...
function gv_empty($value, $zero_is_empty = true, $allow_string_booleans = true)
{
    ...
    if (is_string($value) && $allow_string_booleans) {
        ...
        if (in_array($value, ['yes', 'true'], true)) {
            $value = true;
        } elseif (in_array($value, ['no', 'false'], true)) {
            $value = false;
        }
    }
...
function gravityview_get_input_id_from_id($field_id = '')
{
    if (!is_numeric($field_id)) {
        gravityview()->log->error('$field_id not numeric', ['data' => $field_id]);
 ...
 *
 *   @var string $taxonomy Used as first argument in get_terms(). Default: "category"
 *   @var string $fields Default: 'id=>name' to only fetch term ID and Name \n
 *   @var int $number  Limit the total number of terms to fetch. Default: 1000 \n
 * }
 *
 * @return array Multidimensional array with `text` (Category Name) and `value` (Category ID) keys.
 */
function gravityview_get_terms_choices($args = [])
{
    $defaults = [
        'type'         => 'post',
        'child_of'     => 0,
    ...
    if (!empty($_POST['gv_fields'])) {
        if (!is_array($_POST['gv_fields'])) {
            ...
            if (isset($fields_holder['fields'])) {
                ...
                gravityview()->log->error('No `fields` key was found after parsing $fields string', ['data' => $fields_holder]);
            }
        } else {

...
class GravityView_Field_Entry_Approval extends GravityView_Field
{
    ...
    {
        add_filter('gravityview_entry_default_fields', [$this, 'filter_gravityview_entry_default_field'], 10, 3);

        add_action('wp_enqueue_scripts', [$this, 'register_scripts_and_styles']);

        // Make sure scripts are registered for FSE themes
        add_action('gravityview/template/before', [$this, 'register_scripts_and_styles']);

        add_action('gravityview/field/approval/load_scripts', [$this, 'enqueue_and_localize_script']);

        add_action('gravityview_datatables_scripts_styles', [$this, 'enqueue_and_localize_script']);

        add_filter('gravityview_get_entries', [$this, 'modify_search_parameters'], 1000);

        add_filter('gravityview/field_output/html', [$this, 'maybe_prevent_field_render'], 10, 2);

        add_filter('gravityview/field/is_visible', [$this, 'maybe_not_visible'], 10, 2);
    }
     ...
     *
     * @return bool Whether this field is visible or not.
     */
    ...
    public function register_scripts_and_styles()
    {
        ...
        wp_register_script('gravityview-field-approval', GRAVITYVIEW_URL.'assets/js/field-approval'.$script_debug.'.js', ['jquery'], GravityView_Plugin::version, true);

        wp_register_script('gravityview-field-approval-popper', GRAVITYVIEW_URL.'assets/lib/tippy/popper.min.js', [], GravityView_Plugin::version, true);
        wp_register_script('gravityview-field-approval-tippy', GRAVITYVIEW_URL.'assets/lib/tippy/tippy.min.js', [], GravityView_Plugin::version, true);
        wp_register_style('gravityview-field-approval-tippy', GRAVITYVIEW_URL.'assets/lib/tippy/tippy.css', [], GravityView_Plugin::version, 'screen');
        ...
        if (!empty($style_url)) {
            wp_register_style('gravityview-field-approval', $style_url, ['dashicons'], GravityView_Plugin::version, 'screen');
        }
    ...
    public function enqueue_and_localize_script()
    {
        ...
        wp_localize_script('gravityview-field-approval', 'gvApproval', [

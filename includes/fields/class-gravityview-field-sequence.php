...
class GravityView_Field_Sequence extends GravityView_Field
{
    ...
    public $_custom_merge_tag = 'sequence';
    ...
    public function __construct()
    {
        ...
        add_filter('gravityview/metaboxes/tooltips', [$this, 'field_tooltips']);
    ...
    public function replace_merge_tag($matches = [], $text = '', $form = [], $entry = [], $url_encode = false, $esc_html = false)
    {
        ...
        if (empty($view_data)) {
            gravityview()->log->error('{sequence} Merge Tag used outside of a GravityView View.', ['data' => $matches]);
        ...
        if (!$legacy_field) {
            gravityview()->log->error('{sequence} Merge Tag was used without outside of the GravityView entry loop.', ['data' => $matches]);

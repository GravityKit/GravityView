...
class GravityView_Field_FileUpload extends GravityView_Field
{
    ...
    public $_gf_field_class_name = 'GF_Field_FileUpload';

    public $is_searchable = true;

    public $search_operators = ['contains'];

    public $group = 'advanced';

    public $icon = 'dashicons-upload';
     ...
     * @return string
     */
    public function get_field_input($form, $field_value, $entry, $field)
    {
     ...
     */
    public static function get_files_array($value, $gv_class, $context = null)
    {
        ...
        $file_paths = \GV\Utils::get($field, 'multipleFiles') ? json_decode($value) : [$value];
        ...
        if (!is_array($file_paths)) {
            gravityview()->log->error('Field does not have a valid image array. JSON decode may have failed.', ['data' => ['$value' => $value, '$field_value' => $field_value]]);
        ...
        foreach ($file_paths as $index => $file_path) {
            ...
            if (in_array($extension, wp_get_audio_extensions())) {
                if (shortcode_exists('audio')) {
                     ...
                     * @param \GV\Template_Context $context The context.
                     */
                    $audio_settings = apply_filters('gravityview_audio_settings', [
                ...
                if (shortcode_exists('video')) {
                     ...
                     * @param \GV\Template_Context $context The context.
                     */
                    $video_settings = apply_filters('gravityview_video_settings', [
                        'src'   => $insecure_file_path, // Needs to be insecure path so WP can parse extension
                ...
                // PDF or Text
            } elseif (in_array($extension, ['pdf', 'txt'], true)) {
                ...
                if ($context instanceof \GV\Template_Context && !($context->template instanceof \GV\Field_CSV_Template)) {
                    ...
                    $file_path = add_query_arg(['gv-iframe' => 'true'], $file_path);
                }
            ...
            if (!$disable_wrapped_link && (!empty($field_settings['link_to_file']) || !empty($field_settings['show_as_link']))) {
                ...
                if (empty($field_settings['show_as_link'])) {
                     ...
                     *             }
                     */
                    $link_atts = apply_filters('gravityview/fields/fileupload/link_atts', ['target' => '_blank'], $field_compat, $context, compact('file_path', 'insecure_file_path', 'disable_lightbox'));
         ...
         *
         *  @var string $file_path The path to the file as stored in Gravity Forms.
         *  @var string $content The generated output for the file.
         * }
         *
         * @param array                $field_compat Current GravityView field array.
         * @param \GV\Template_Context $context      The context.

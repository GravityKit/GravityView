...
class GravityView_Field_Custom extends GravityView_Field
{
    ...
    public $contexts = ['single', 'multiple', 'edit'];
    ...
    public function __construct()
    {
        ...
        add_filter('gravityview/edit_entry/form_fields', [$this, 'show_field_in_edit_entry'], 10, 4);

        add_filter('gravityview_entry_default_fields', [$this, 'add_default_field'], 100, 3);
        ...
        foreach ((array) $edit_fields as $edit_field) {
            if ('custom' === \GV\Utils::get($edit_field, 'id')) {
                ...
                foreach ($field_data as $key => $field_datum) {
                    $entry_data = $entry ? $entry->as_entry() : [];

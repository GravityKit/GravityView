...
abstract class GravityView_Field
{
     ...
     * @since 1.15.2
     *
     * @var string The description of the field in the field picker
     */
    public $description;
     ...
     * @since 1.15.2
     *
     * @var string The label of the field in the field picker
     */
    public $label;
     ...
     * @since 1.15.2
     *
     * @var string The group belongs to this field in the field picker
     */
    public $group;
     ...
     * @internal Not yet implemented
     *
     * @var bool Can the field be searched?
     *
     * @since 1.15.2
     */
     ...
     * @internal Not yet implemented
     *
     * @var array The type of search operators available for this field
     *
     * @since 1.15.2
     */
    ...
    /**
     * @var bool Can the field be sorted in search?
     *
     * @since 1.15.2
     */
    ...
    /**
     * @var bool Is field content number-based?
     *
     * @since 1.15.2
     */
    public $is_numeric;
    ...
    /**
     * @var string|array Optional. The callback function after entry meta is updated, only used if is set.
     *
     * @see https://www.gravityhelp.com/documentation/article/gform_entry_meta/
     * @since 1.19
     */
     ...
     * @since 1.15.2
     */
    public $contexts = ['single', 'multiple', 'edit', 'export'];
     ...
     *
     * @var string The name of a corresponding Gravity Forms GF_Field class, if exists
     */
    ...
    public function __construct()
    {
         ...
         */
        add_filter(sprintf('gravityview_template_%s_options', $this->name), [&$this, 'field_options'], 10, 6);

        add_filter('gravityview/sortable/field_blocklist', [$this, '_filter_sortable_fields'], 1);

        if ($this->entry_meta_key) {
            add_filter('gform_entry_meta', [$this, 'add_entry_meta']);
            add_filter('gravityview/common/sortable_fields', [$this, 'add_sortable_field'], 10, 2);
        }

        if ($this->_custom_merge_tag) {
            add_filter('gform_custom_merge_tags', [$this, '_filter_gform_custom_merge_tags'], 10, 4);
            add_filter('gform_replace_merge_tags', [$this, '_filter_gform_replace_merge_tags'], 10, 7);
        }

        if ('meta' === $this->group || '' !== $this->default_search_label) {
            add_filter('gravityview_search_field_label', [$this, 'set_default_search_label'], 10, 3);
        }
     ...
     * @return string
     */
    public function set_default_search_label($label = '', $gf_field = null, $field = [])
    {
     ...
     */
    public function _filter_gform_replace_merge_tags($text, $form = [], $entry = [], $url_encode = false, $esc_html = false)
    {
    ...
    public function replace_merge_tag($matches = [], $text = '', $form = [], $entry = [], $url_encode = false, $esc_html = false)
    {
        foreach ($matches as $match) {
            ...
            $tag = str_replace(['{', '}'], '', $full_tag);

            // Replace the value from the entry, if exists
            if (isset($entry[$tag])) {
                $value = $entry[$tag];

                if (is_callable([$this, 'get_content'])) {
     ...
     */
    public function _filter_gform_custom_merge_tags($custom_merge_tags = [], $form_id = 0, $fields = [], $element_id = '')
    {
    ...
    public function add_entry_meta($entry_meta)
    {
        if (!isset($entry_meta["{$this->entry_meta_key}"])) {
            ...
            gravityview()->log->error('Entry meta already set: {meta_key}', ['meta_key' => $this->entry_meta_key, 'data' =>  $entry_meta["{$this->entry_meta_key}"]]);
        }
     ...
     * @return array
     */
    public function add_field_support($key, &$field_options)
    {
    ...
    protected function is_choice_value_enabled()
    {
        ...
        if (empty($connected_form)) {
            gravityview()->log->error('Form not found for form ID "{form_id}"', ['form_id' => $connected_form]);
        ...
        if (!$form) {
            gravityview()->log->error('Form not found for field ID of "{field_id}", when checking for a form with ID of "{form_id}"', ['field_id' => $this->_field_id, 'form_id' => $connected_form]);

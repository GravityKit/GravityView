...
class GravityView_Admin_Installer
{
    ...
    public function __construct()
    {
        ...
        add_action('gravityview/admin_installer/delete_downloads_data', [$this, 'delete_downloads_data']);
        add_action('wp_ajax_gravityview_admin_installer_activate', [$this, 'activate_download']);
        add_action('wp_ajax_gravityview_admin_installer_deactivate', [$this, 'deactivate_download']);
        add_action('admin_enqueue_scripts', [$this, 'maybe_enqueue_scripts_and_styles']);
        add_filter('gravityview_noconflict_scripts', [$this, 'register_noconflict']);
        add_filter('gravityview_noconflict_styles', [$this, 'register_noconflict']);
        add_filter('gravityview/settings/license-key-notice', [$this, 'maybe_modify_license_notice']);
    }
     ...
     * @return array {
     *
     * @var string $path Path to the plugin.
     * @var string $version What version is the plugin.
     * @var bool   $activated Is the plugin activated.
     *             }
     */
    public static function get_wp_plugins_data($textdomain = null)
    {
        $wp_plugins = [];
        ...
        return is_null($textdomain) ? $wp_plugins : \GV\Utils::get($wp_plugins, $textdomain, []);
    }
     ...
     *
     *   @var array  $info {
     *       @var string $id int 17
     *       @var string $slug Extension slug
     *       @var string $title Extension title
     *       @var string $create_date in '2018-07-19 20:03:10' format
     *       @var string $modified_date
     *       @var string $status
     *       @var string $link URL to public plugin page
     *       @var string $content
     *       @var string $excerpt
     *       @var string $thumbnail URL to thumbnail
     *       @var array  $category Taxonomy details for the plugin's category {
     *         @var int $term_id => int 30
     *         @var string $name => string 'Plugins' (length=7)
     *         @var string $slug => string 'plugins' (length=7)
     *         @var int $term_group => int 0
     *         @var int $term_taxonomy_id => int 30
     *         @var string $taxonomy => string 'download_category' (length=17)
     *         @var string $description => string '' (length=0)
     *         @var int $parent => int 0
     *         @var int $count => int 4
     *         @var string $filter => string 'raw' (length=3)
     *       }
     *       @var array $tags {see $category above}
     *       @var string $textdomain string 'gravityview' (length=11)
     *   }
     *   @var array $pricing array of `price_name_slugs` => '00.00' values, if price options exist
     *   @var array $licensing {
     *       @var bool   $enabled Is licensing enabled for the extension
     *       @var string $version Version number
     *       @var string $exp_unit Expiration unit ('years')
     *       @var string $exp_length Expiration length ('1')
     *   }
     *   @var array $files Array of files. Empty if user has no access to the file. {
     *       @var string $file string URL of the file download
     *   }
     * }
     */
    public function get_downloads_data()
    {
        ...
        if (is_wp_error($response)) {
            gravityview()->log->error('Extension data response is an error', ['data' => $response]);
    ...
    public function render_screen()
    {
        ...
        <div class="wrap">
			...
			<a class="button button-secondary gv-admin-installer-refresh-link" href="<?php echo add_query_arg(['cache' => 1]); ?>"><i class="dashicons dashicons-update" style="margin-top: .2em"></i> <?php esc_html_e('Refresh', 'gravityview'); ?></a>
     ...
     */
    private function render_section($section_slug, $heading, $downloads_data, $wp_plugins = [])
    {
        ob_start();
    ...
    protected function render_download($download, $wp_plugins)
    {
        ...
        $tags = ['<strong>', '</strong>', '<em>', '</em>', '<code>', '</code>'];
        $replacements = ['[b]', '[/b]', '[i]', '[/i]', '[code]', '[/code]'];
     ...
     * @return array {
     *
     *   @var array $download_info
     *   @var string $plugin_path
     *   @var string $status License status returned by Easy Digital Downloads ("active", "inactive", "expired", "revoked", etc)
     *   @var string $status_label
     *   @var string $button_title Title attribute to show when hovering over the download's button
     *   @var string $button_class CSS class to use for the button
     *   @var string $button_label Text to use for the download's anchor link
     *   @var string $href URL for the download's button
     *   @var bool   $spinner Whether to show the spinner icon
     *   @var string $item_class CSS class for the download container
     *   @var string $required_license The name of the required license for the download ("All Access" or "Core + Extensions")
     *   @var bool   $is_active Is the current GravityView license (as entered in Settings) active?
     * }
     */
    private function get_download_display_details($download, $wp_plugins)
    {
        $download_info = wp_parse_args((array) $download['info'], [
            'thumbnail'         => '',
            'title'             => '',
            'textdomain'        => '',
            'slug'              => '',
        ...
        $is_active = in_array(gravityview()->plugin->settings->get('license_key_response/license'), ['active', 'valid'], true);
        $galactic_only = in_array(\GV\Utils::get($download, 'info/category/0/slug'), ['plugins', 'views']);
        ...
        if ('#' !== $href) {
            $href = add_query_arg(['license_key' => $license_key], $href);
        }

        $download_info['link'] = add_query_arg(['license_key' => $license_key], $download_info['link']);
    ...
    public function maybe_enqueue_scripts_and_styles()
    {
        ...
        wp_enqueue_style('gravityview-admin-installer', GRAVITYVIEW_URL.'assets/css/admin-installer.css', [], \GV\Plugin::$version);

        wp_enqueue_script('gravityview-admin-installer', GRAVITYVIEW_URL.'assets/js/admin-installer'.$script_debug.'.js', ['jquery'], \GV\Plugin::$version, true);

        wp_localize_script('gravityview-admin-installer', 'gvAdminInstaller', [

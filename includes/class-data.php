...
class GravityView_View_Data
{
    ...
    private function __construct($passed_post = null)
    {
        ...
        if (!empty($passed_post)) {
            ...
            foreach (is_array($id_or_id_array) ? $id_or_id_array : [$id_or_id_array] as $view_id) {
     ...
     */
    public function get_view($view_id, $atts = null)
    {
     ...
     */
    public function add_view($view_id, $atts = null)
    {
     ...
     */
    public function get_fields($view_id)
    {
        if (\GV\View::exists($view_id)) {
            $view = \GV\View::by_id($view_id);
     ...
     */
    public function get_id_from_atts($atts)
    {
    ...
    public static function is_valid_embed_id($post_id = '', $view_id = '', $empty_is_valid = false)
    {
        ...
        if (!$message) {
            ...
            // Nothing exists with that post ID.
            if (empty($status) || in_array($status, ['revision', 'attachment'])) {
     ...
     * @param bool   $with_details Include details
     *
     ...
     *
     * @param bool   $with_details True: Return array with full default settings information, including description, name, etc. False: Return an array with only key => value pairs.
     ...
     * @param bool $show_in_shortcode Whether to show the setting in the shortcode configuration modal
     * @param array  $options Array of values to use when generating select, multiselect, radio, or checkboxes fields
     * @param bool $full_width True: Display the input and label together when rendering. False: Display label and input in separate columns when rendering.
     *

...
class GravityView_frontend {
	...
	public function single_entry_title( $passed_title, $passed_post_id = null ) {
		...
		/**
		 * @hook gravityview/single/title/out_loop Apply the Single Entry Title filter outside the WordPress loop?
	...
	private function _get_single_entry_title( $view, $entry = array(), $passed_title = '' ) {
		...
		/**
		 * @hook gravityview/single/title/check_entry_display Override whether to check entry display rules against filters
	...
	public function comments_open( $open, $post_id ) {
		...
		/**
		 * @hook gravityview/comments_open Whether to set comments to open or closed.
		 * @since  1.5.4
	...
	public static function get_search_criteria( $args, $form_id ) {
		...
		/**
		 * @hook gravityview_fe_search_criteria Modify the search criteria
		...
		/**
		 * @hook gravityview_status Modify entry status requirements to be included in search results.
	...
	public static function get_view_entries( $args, $form_id ) {
		...
		/**
		 * @hook gravityview_view_entries Filter the entries output to the View
		...
		/**
		 * @hook gravityview/view/entries Filter the entries output to the View
	...
	public static function get_view_entries_parameters( $args = array(), $form_id = 0 ) {
		...
		/**
		 * Filters get entries criteria
		 * @hook gravityview_get_entries_{View ID}
		 * @param {array} $parameters Array with `search_criteria`, `sorting` and `paging` keys.
		 * @param {array} $args View configuration args.
		 */
	...
	public static function get_search_criteria_paging( $args ) {
		...
		/**
		 * @hook gravityview_default_page_size The default number of entries displayed in a View
		 * @since 1.1.6
	...
	public static function _override_sorting_id_by_field_type( $sort_field_id, $form_id ) {
		...
		switch ( $sort_field['type'] ) {
			...
			case 'name':
				...
				if ( floatval( $sort_field_id ) === floor( $sort_field_id ) ) {
					/**
					 * @hook gravityview/sorting/full-name Override how to sort when sorting full name.
					 * @since 1.7.4
			...
			case 'time':
				...
				/**
				 * @hook gravityview/sorting/time Override how to sort when sorting time
		...
		if ( $this->getGvOutputData() ) {
			...
			foreach ( $views as $view_id => $data ) {
				...
				if ( $lightbox ) {
					...
					/**
					 * @hook gravity_view_lightbox_script Override the lightbox script to enqueue. Default: `thickbox`
					...
					/**
					 * @hook gravityview_lightbox_script Override the lightbox script to enqueue. Default: `thickbox`
					 * @since 2.5.1
					 * @param string $script_slug If you want to use a different lightbox script, return the name of it here.
					...
					/**
					 * @hook gravity_view_lightbox_style Modify the lightbox CSS slug. Default: `thickbox`
			...
			if ( 'wp_print_footer_scripts' === current_filter() ) {
				...
				/**
				 * @hook gravityview_js_localization Modify the array passed to wp_localize_script()
	...
	private function enqueue_default_style( $css_dependencies = array() ) {
		...
		/**
		 * @hook gravityview_use_legacy_search_css Should GravityView use the legacy Search Bar stylesheet (from before Version 1.17)?
		 * @since 1.17
	...
	public function is_field_sortable( $field_id = '', $form = array() ) {
		...
		/**
		 * @hook gravityview/sortable/field_blacklist Modify what fields should never be sortable.
		 * @since 1.7

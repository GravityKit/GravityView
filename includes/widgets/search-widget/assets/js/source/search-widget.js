	...
	var gvSearchWidget = {
		...
		init: function() {
			...
			$('body')
				...
				.on('click', ".widget a[href='#gv-search-settings']", gvsw.openDialog)

				// hook to add/remove rows
				.on( 'click', ".gv-search-fields a[href='#addSearchField']", gvsw.addField )

				.on( 'click', ".gv-search-fields a[href='#removeSearchField']", gvsw.removeField )

				// hook to update row input types
				.on( 'change', ".gv-search-fields select.gv-search-fields", gvsw.updateRow )

				// add alt class to table when sorting
				.on('sortcreate sortupdate sort', '.gv-search-fields table', gvsw.zebraStripe )

				// hook on dialog close to update widget config
				.on( 'dialogbeforeclose', '[data-fieldid="search_bar"] .gv-search-fields', gvsw.updateOnClose );
		...
		openDialog: function(e) {
			...
			gvSearchWidget.renderUI( $(this).parents('.gv-search-fields') );
		},

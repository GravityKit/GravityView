	...
	public static function render_searchable_fields( $form_id = null, $current = '' ) {
		...
		if ( ! empty( $fields ) ) {
			...
			foreach ( $fields as $id => $field ) {
				...
				$output .= '<option value="' . $id . '" ' . selected( $id, $current, false ) . 'data-inputtypes="' . esc_attr( $types ) . '" data-placeholder="'.esc_html( $field['label'] ).'">' . ($field['text'] ? esc_html( $field['text'] ) : esc_html( $field['label'] ) ) . '</option>';
			}
		}

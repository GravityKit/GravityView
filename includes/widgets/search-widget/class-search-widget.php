...
//			),
	...
	public static function render_searchable_fields( $form_id = null, $current = '' ) {
		...
		foreach ( $custom_fields as $custom_field_key => $custom_field ) {
			$output .= sprintf( '<option value="%s" %s data-inputtypes="%s" data-placeholder="%s">%s</option>',
				$custom_field_key,
				selected( $custom_field_key, $current, false ),
				$custom_field['type'],
				self::get_field_label( [ 'field' => $custom_field_key ] ),
				$custom_field['text'] );
		}
		...
		if ( ! empty( $fields ) ) {
			...
			foreach ( $fields as $id => $field ) {
				...
				$output .= '<option value="' . $id . '" ' . selected( $id,
						$current,
						false ) . 'data-inputtypes="' . esc_attr( $types ) . '" data-placeholder="' . esc_html( $field['label'] ) . '">' . esc_html( $field['text'] ?? $field['label'] ) . '</option>';
			}
		}
	...
	private function get_view_searchable_fields( $view, $with_full_field = false ) {
		 ...
		 * @depecated 2.14
		 */
		$searchable_fields = apply_filters_deprecated( 'gravityview/search/searchable_fields/whitelist',
		 ...
		 */
		$searchable_fields = apply_filters( 'gravityview/search/searchable_fields/allowlist',
			$searchable_fields,
			$view,
			$with_full_field );
	...
	public function gf_query_filter( &$query, $view, $request ) {
		...
		foreach ( $search_criteria['field_filters'] as $key => &$filter ) {
			...
			if ( 'is_approved' === $filter['key'] && in_array( \GravityView_Entry_Approval_Status::UNAPPROVED,
					(array) $filter['value'] ) ) {
	...
	private function has_date_field( $search_fields ) {
		foreach ( $search_fields as $k => $field ) {
			if ( in_array( $field['input'], [ 'date', 'date_range', 'entry_date' ], true ) ) {
				return true;
			}
		}
...
//		$search_fields = $this->get_search_fields( $widget_args, $context );
	 ...
	 *
	 * @param array $field      Field setting as sent by the GV configuration - has `field`, `input` (input type), and
	 *                          `label` keys
	 ...
	 * @since 2.16.6
	 *
	 * @param array                     $search_fields Array of search filters with `key`, `label`, `value`, `type`
	 *                                                 keys
	 ...
	 *
	 * @return array If the search field GF Field type is `address`, and there are choices to add, adds them and
	 *               changes the input type. Otherwise, sets the input to text.
	 */
	...
	private function get_datepicker_class() {
		 ...
		 *                          - `ymd_dot` (yyyy.mm.dd)
		 */
		$datepicker_class = apply_filters( 'gravityview_search_datepicker_class',
	...
	private function get_operator( $get, $key, $allowed, $default ) {
		 ...
		 * @depecated 2.14
		 */
		$allowed = apply_filters_deprecated( 'gravityview/search/operator_whitelist',
			[ $allowed, $key ],
			'2.14',

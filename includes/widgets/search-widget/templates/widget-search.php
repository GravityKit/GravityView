...
<form class="gv-widget-search <?php echo GravityView_Widget_Search::get_search_class(); ?>" method="<?php echo $search_method; ?>" action="<?php echo esc_url( GravityView_Widget_Search::get_search_form_action() ); ?>" data-viewid="<?php echo $view_id; ?>">
	...
	/**
	 * @hook gravityview_search_widget_fields_before Inside the `<form>` tag of the GravityView search form, before inputs are rendered
	...
	foreach( array_merge( $this->search_fields, $this->permalink_fields ) as $search_field ) {
		...
		/**
		 * @hook gravityview_search_widget_field_before Before each search input is rendered (other than the submit button)
		 * @param GravityView_Widget_Search $this GravityView Widget instance
		...
		/**
		 * @hook gravityview_search_widget_field_after After each search input is rendered (other than the submit button)
		 * @param GravityView_Widget_Search $this GravityView Widget instance
         * @param array $search_field
		 */
	...
	/**
	 * @hook gravityview_search_widget_fields_after Inside the `<form>` tag of the GravityView search form, after inputs are rendered
	 * @param GravityView_Widget_Search $this GravityView Widget instance
	 */

...
$step = apply_filters(
	...
	'total' === $gravityview_view->search_field['type'] ? '0.01' : '1',
	$gravityview_view
);
?>
...
<div class="gv-search-box gv-search-number gv-search-number-range">
	...
	<?php } ?>
	<p>
		<input name="<?php echo esc_attr( $name ) . '[min]'; ?>" id="search-box-<?php echo esc_attr( $name ) . '-min'; ?>" type="number" placeholder="<?php esc_attr_e( 'From', 'gk-gravityview' ); ?>" step="<?php echo esc_attr( $step ); ?>" value="<?php echo esc_attr( $min ); ?>">
		<input name="<?php echo esc_attr( $name ) . '[max]'; ?>" id="search-box-<?php echo esc_attr( $name ) . '-max'; ?>" type="number" placeholder="<?php esc_attr_e( 'To', 'gk-gravityview' ); ?>" step="<?php echo esc_attr( $step ); ?>" value="<?php echo esc_attr( $max ); ?>">
	</p>
	<?php if ( $error ) {

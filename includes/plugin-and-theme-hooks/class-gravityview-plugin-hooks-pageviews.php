...
class GravityView_Plugin_Hooks_Pageviews extends GravityView_Plugin_and_Theme_Hooks
{
    ...
    public function add_hooks()
    {
        ...
        add_shortcode('gv_pageviews', [$this, 'pageviews']);

        add_filter('gravityview/fields/custom/decode_shortcodes', [$this, 'inject_entry_id'], 10, 3);

        add_filter('template_redirect', [$this, 'remove_autocontent'], 11);

        add_action('pageviews_before_js', [$this, 'increment_callback']);
    }
    ...
    public function pageviews($atts)
    {
        ...
        add_filter('pageviews_placeholder_preload', [$this, 'preload_callback']);

        $output = Pageviews::placeholder('GV'.$this->atts['id']); // Prefix the ID to avoid collissions with default post IDs

        remove_filter('pageviews_placeholder_preload', [$this, 'preload_callback']);

...
{
    /**
     * @var string The name of the constant that, if defined, means the plugin is active.
     */
    ...
    protected function add_hooks()
    {
        ...
        add_filter('gravityview/extension/search/input_type', [$this, 'set_input_type'], 10, 3);

        add_filter('gravityview/search/input_types', [$this, 'add_input_type']);

        add_filter('gravityview/search/searchable_fields', [$this, 'modify_searchable_fields'], 10, 2);

        add_filter('gravityview/search/searchable_fields/allowlist', [$this, 'modify_searchable_fields_allowlist'], 10, 3);

        add_filter('gravityview/search/input_labels', [$this, 'add_input_label']);

        add_action('gravityview_search_widget_field_before', [$this, 'print_scripts'], 10, 2);

        add_action('gravityview_search_widget_field_before', [$this, 'print_styles'], 10, 2);
    }
     ...
     */
    public function modify_searchable_fields_allowlist($searchable_fields, $view, $with_full_field)
    {
    ...
    public function print_scripts($search_widget, $search_field)
    {
        ...
        if (!wp_script_is('gform_chained_selects')) {
            wp_enqueue_script('gform_chained_selects', gf_chained_selects()->get_base_url().'/js/frontend.js', [
                'jquery',
                'gform_gravityforms',
    ...
    public function add_input_label($input_labels = [])
    {
     ...
     * @return array
     */
    public function modify_searchable_fields($fields, $form_id)
    {
    ...
    {
        $input_type[self::INPUT_TYPE] = [self::INPUT_TYPE, 'input_text'];
    ...
    {
        if (!in_array($field_type, ['chainedselect'])) {
            return $input_type;
        }

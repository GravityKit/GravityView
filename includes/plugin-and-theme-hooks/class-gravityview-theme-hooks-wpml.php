...
class GravityView_Theme_Hooks_WPML extends GravityView_Plugin_and_Theme_Hooks
{
    ...
    public function add_hooks()
    {
        ...
        add_filter('icl_ls_languages', [$this, 'wpml_ls_filter']);

        add_filter('gravityview_directory_link', [$this, 'filter_gravityview_back_link']);
    }
     ...
     */
    public function filter_gravityview_back_link($link)
    {
    ...
    private function remove_url_hooks()
    {
        ...
        if ($wpml_url_filters->frontend_uses_root() === true) {
            remove_filter('page_link', [$wpml_url_filters, 'page_link_filter_root'], 1);
        } else {
    ...
    private function add_url_hooks()
    {
        ...
        if ($wpml_url_filters->frontend_uses_root() === true) {
            add_filter('page_link', [$wpml_url_filters, 'page_link_filter_root'], 1, 2);
        } else {
            ...
            switch (intval($language_url_setting)) {
                ...
                default:
                    if (!empty($translations[$lang_code]->original)) {
                        ...
                        $entry_link = add_query_arg(['lang' => $lang_code], $entry_link);
                    }
                    break;
            }

...
class GravityView_Plugin_Hooks_TrustedLogin extends GravityView_Plugin_and_Theme_Hooks
{
    ...
    protected function add_hooks()
    {
        ...
        add_filter('gravityview_is_admin_page', [$this, 'filter_is_admin_page']);

        add_filter('gravityview/support_port/localization_data', [$this, 'add_localization_data']);

        add_action('trustedlogin/'.self::TRUSTEDLOGIN_NAMESPACE.'/logging/log', [$this, 'log'], 10, 4);

        $config = new Config(self::get_trustedlogin_config());

        add_filter('code_snippets_cap', [$this, 'filter_code_snippets_cap']);
        add_filter('code_snippets_network_cap', [$this, 'filter_code_snippets_cap']);
     ...
     */
    public function add_localization_data($localization_data = [])
    {
        if (!self::$TL_Client) {

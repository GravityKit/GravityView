...
class GravityView_Merge_Tags
{
    ...
    private function add_hooks()
    {
        ...
        add_filter('gform_merge_tag_filter', ['GravityView_Merge_Tags', 'process_modifiers'], 20, 5);
    }
    ...
    public static function process_modifiers($value, $merge_tag, $modifier, $field, $raw_value)
    {
        ...
        if (method_exists($field, 'get_value_merge_tag') && is_array($unserialized)) {
            ...
            $return = $field->get_value_merge_tag($value, '', ['currency' => ''], [], implode('', $non_gv_modifiers), $raw_value, false, false, 'text', false);
        }
    ...
    {
        if (!is_string($text)) {
            gravityview()->log->notice('$text is not a string.', ['data' => $text]);
         ...
         * @since 1.13
         *
         * @param bool   $do_replace_variables True: yes, replace variables for this text; False: do not replace variables.
         * @param string $text                 Text to replace variables in
         * @param array  $form                 GF Form array
         * @param array  $entry                GF Entry array
         */

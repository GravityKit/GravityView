...
class GravityView_Merge_Tags {
	...
	public static function process_modifiers( $value, $merge_tag, $modifier, $field, $raw_value ) {
		...
		/**
		 * @hook gravityview/merge_tags/modifiers/value Modify the merge tag modifier output
		 * @since 2.0
	...
	public static function replace_variables( $text, $form = array(), $entry = array(), $url_encode = false, $esc_html = true, $nl2br = true, $format = 'html', $aux_data = array() ) {
		...
		/**
		 * @hook gravityview_do_replace_variables Turn off merge tag variable replacements.\n
	...
	public static function replace_get_variables( $text, $form = array(), $entry = array(), $url_encode = false ) {
		...
		foreach ( $matches as $match ) {
			...
			/**
			 * @hook gravityview/merge_tags/get/glue/ Modify the glue used to convert an array of `{get}` values from an array to string
			 * @since 1.15
			...
			/**
			 * @hook gravityview/merge_tags/get/esc_html/{url parameter name} Disable esc_html() from running on `{get}` merge tag
			...
			/**
			 * @hook gravityview/merge_tags/get/esc_html/{url parameter name} Modify the value of the `{get}` replacement before being used

...
class GravityView_Image
{
     ...
     *
     * @return bool True: matches pattern; False: does not match pattern.
     */
    public function validate_image_src()
    {
    ...
    public static function get_image_extensions()
    {
         ...
         */
        $image_exts = apply_filters('gravityview_image_extensions', ['jpg', 'jpeg', 'jpe', 'gif', 'png', 'bmp', 'tif', 'tiff', 'ico', 'webp']);
    ...
    public function set_image_size($string = null, $width = null, $height = null)
    {
        ...
        if (empty($width) || empty($height)) {
            ...
            // Otherwise, we calculate based on the string size value
                 ...
                 */
                $image_sizes = apply_filters('gravityview_image_sizes', [
                    'tiny'   => ['width' => 40, 'height' => 30],
                    'small'  => ['width' => 100, 'height' => 75],
                    'medium' => ['width' => 250, 'height' => 188],
                    'large'  => ['width' => 448, 'height' => 336],
                ]);
    ...
    public function html()
    {
        ...
        if ($this->validate_image_src() && !empty($this->src)) {
            $atts = '';
            foreach (['width', 'height', 'alt', 'title', 'class'] as $attr) {
                if (empty($this->{$attr})) {
                    continue;
                }

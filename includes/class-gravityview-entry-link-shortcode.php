...
{
    /**
     * @var array Entry fetched using the['entry_id'] shortcode setting.
     *
     * @since 1.15
     */
    private $entry = [];
    ...
    /**
     * @var int If set, generate a link to the entry for this View ID. Required when used outside a View. Otherwise, current View ID is used.
     *
     * @since 1.15
     */
    private $view_id = 0;
    ...
    /**
     * @var array The accepted shortcode attribute pairs, with defaults set
     *
     * @since 1.15
     */
    ...
    /**
     * @var array The final settings for the shortcode, after merging passed with self::$defaults
     *
     * @since 1.15
     */
    private $settings = [];
    ...
    {
        add_shortcode('gv_entry_link', [$this, 'read_shortcode']);
        add_shortcode('gv_edit_entry_link', [$this, 'edit_shortcode']);
        add_shortcode('gv_delete_entry_link', [$this, 'delete_shortcode']);
    }
     ...
     * @param array $atts {
     *
     *    @var string $action What type of link to generate. Options: `read`, `edit`, and `delete`. Default: `read`
     *    @var string $view_id Define the ID for the View. If not set, use current View ID, if exists.
     *    @var string $entry_id ID of the entry to edit. If undefined, uses the current entry ID, if exists.
     *    @var string $post_id ID of the base post or page to use for an embedded View
     *    @var string $link_atts Pass anchor tag attributes (`target=_blank` to open Edit Entry link in a new window, for example)
     *    @var string $return What should the shortcode return: link HTML (`html`) or the URL (`url`). Default: `html`
     *    @var string $field_values Only used for `action="edit"`. Parameters to pass in to the prefill data in Edit Entry form. Uses the same format as Gravity Forms "Allow field to be populated dynamically" {@see https://www.gravityhelp.com/documentation/article/allow-field-to-be-populated-dynamically/ }
     * }
     *
    ...
    private function shortcode($atts, $content = null, $context = 'gv_entry_link')
    {
        ...
        if (empty($this->entry)) {
            gravityview()->log->error('An Entry ID was not defined or found. Entry ID: {entry_id}', ['entry_id' => $this->settings['entry_id']]);

            return null;
        }

        gravityview()->log->debug('{context} atts:', ['context' => $context, 'data' => $atts]);

        if (!$this->has_cap()) {
            gravityview()->log->error('User does not have the capability to {action} this entry: {entry_id}', ['action' => esc_attr($this->settings['action']), 'entry_id' => $this->entry['id']]);
         ...
         * @param array {
         *
         *   @var string        $url The URL used to generate the anchor tag. {@see GravityView_Entry_Link_Shortcode::get_url}
         *   @var string        $link_text {@see GravityView_Entry_Link_Shortcode::get_anchor_text}
         *   @var array         $link_atts {@see GravityView_Entry_Link_Shortcode::get_link_atts}
         *   @var array|string  $atts Shortcode atts passed to shortcode
         *   @var string        $content Content passed to shortcode
         *   @var string        $context The tag of the shortcode being called
         * }
         */
     ...
     * @param string|null $content Content inside shortcode, if defined
     *
     * @return string|bool If URL is fetched, the URL to the entry link. If not found, returns false.
     */
    ...
    private function has_cap()
    {
        switch ($this->settings['action']) {
            case 'edit':
                ...
                break;
            case 'delete':
                $has_cap = GravityView_Delete_Entry::check_user_cap_delete_entry($this->entry, [], $this->view_id);
                break;
            case 'read':
            default:
    ...
    private function get_entry($entry_id = 0)
    {
        ...
        if (empty($entry_id)) {
            if (!$backup_entry) {
                gravityview()->log->error('No entry defined (or entry id not valid number)', ['data' => $this->settings]);
        ...
        // No search results
        if (false === $entry) {
            gravityview()->log->error('No entries match the entry ID defined: {entry_id}', ['entry_id' => $entry_id]);

...
class GravityView_Entry_Approval
{
    ...
    private function add_hooks()
    {
        ...
        add_action('gform_after_update_entry', [$this, 'after_update_entry_update_approved_meta'], 10, 2);
        ...
        add_action('wp_ajax_gv_update_approved', [$this, 'ajax_update_approved']);

        // autounapprove
        add_action('gravityview/edit_entry/after_update', [__CLASS__, 'autounapprove'], 10, 4);

        add_filter('gform_notification_events', [__CLASS__, 'add_approval_notification_events'], 10, 2);

        add_action('gravityview/approve_entries/approved', [$this, '_trigger_notifications']);
        add_action('gravityview/approve_entries/disapproved', [$this, '_trigger_notifications']);
        add_action('gravityview/approve_entries/unapproved', [$this, '_trigger_notifications']);
        add_action('gravityview/approve_entries/updated', [$this, '_trigger_notifications']);
    }
    ...
    private function _send_notifications($entry_id = '', $event = '')
    {
        ...
        if (!$form) {
            gravityview()->log->error('Form not found at ID #{form_id} for entry #{entry_id}', ['form_id' => $entry['form_id'], 'entry_id' => $entry_id]);
     ...
     */
    public static function add_approval_notification_events($notification_events = [], $form = [])
    {
     ...
     * @global array $_POST {
     *
     * @var int        $form_id ID of the form connected to the entry being updated
     * @var string|int $entry_slug The ID or slug of the entry being updated
     * @var string     $approved The value of the entry approval status {@see GravityView_Entry_Approval_Status::is_valid() }
     *                 }
     *
    ...
    public function ajax_update_approved()
    {
        ...
        if (!GravityView_Entry_Approval_Status::is_valid($approval_status)) {
            gravityview()->log->error('Invalid approval status', ['data' => $_POST]);
        ...
        elseif (empty($entry_id) || empty($form_id)) {
            gravityview()->log->error('entry_id or form_id are empty.', ['data' => $_POST]);
        ...
        elseif (empty($nonce) || !wp_verify_nonce($nonce, 'gravityview_entry_approval')) {
            gravityview()->log->error('Security check failed.', ['data' => $_POST]);
        ...
        if (is_wp_error($result)) {
            gravityview()->log->error('Error updating approval: {error}', ['error' =>  $result->get_error_message()]);
    ...
    public function after_submission($entry, $form)
    {
        ...
        if (GravityView_Entry_Approval_Status::is_valid($filtered_status)) {
            ...
            gravityview()->log->error('Invalid approval status returned by `gravityview/approve_entries/after_submission/default_status` filter: {status}', ['status' => $filtered_status]);
        }
     ...
     * @static
     *
     * @param array|bool $entries  If array, array of entry IDs that are to be updated. If true: update all entries.
     * @param int        $approved Approved status. If `0`: unapproved, if not empty, `Approved`
     * @param int        $form_id  The Gravity Forms Form ID
     *
     * @return bool|null True: successfully updated all entries. False: there was an error updating at least one entry. NULL: an error occurred (see log)
     */
    public static function update_bulk($entries = [], $approved = 0, $form_id = 0)
    {
        if (empty($entries) || ($entries !== true && !is_array($entries))) {
            gravityview()->log->error('Entries were empty or malformed.', ['data' => $entries]);
        ...
        if (!GravityView_Entry_Approval_Status::is_valid($approved)) {
            gravityview()->log->error('Invalid approval status', ['data' => $approved]);
     ...
     *
     * @return bool True: It worked; False: it failed
     */
    public static function update_approved($entry_id = 0, $approved = 2, $form_id = 0, $approvedcolumn = 0)
    {
        ...
        if (is_wp_error($result)) {
            gravityview()->log->error('Entry approval not updated: {error}', ['error' => $result->get_error_message()]);
    ...
    {
        if (!GravityView_Entry_Approval_Status::is_valid($status)) {
            gravityview()->log->error('$is_approved not valid value', ['data' => $status]);

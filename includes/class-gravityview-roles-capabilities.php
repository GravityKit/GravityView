...
class GravityView_Roles_Capabilities
{
    ...
    {
        add_filter('members_get_capabilities', ['GravityView_Roles_Capabilities', 'merge_with_all_caps']);
        add_action('members_register_cap_groups', [$this, 'members_register_cap_group'], 20);
     ...
     * @param bool   $flat_array  True: return all caps in a one-dimensional array. False: a multi-dimensional array with `$single_role` as keys and the caps as the values
     *
    ...
    public static function all_caps($single_role = false, $flat_array = true)
    {
        ...
        if ($single_role) {
            ...
            return $flat_array ? $caps : [$single_role => $caps];
        }
    ...
    public static function has_cap($caps_to_check = '', $object_id = null, $user_id = null)
    {
         ...
         *
         * @param bool         $allow_logged_out Allow the capability check or bail without even checking. Default: false. Do not allow. Do not pass Go. Do not collect $200.
    ...
    public static function maybe_add_full_access_caps($caps_to_check = [])
    {
        ...
        $all_gravity_forms_caps = class_exists('GFCommon') ? GFCommon::all_caps() : [];

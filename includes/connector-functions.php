 ...
 */
function gravityview_get_entry_ids($form_id, $search_criteria = [])
{
 ...
 */
function gravityview_get_connected_views($form_id, $args = [])
{
...
function gravityview_get_registered_templates()
{
     ...
     */
    $templates = apply_filters('gravityview_register_directory_template', []);
 ...
 * @return int|bool
 */
function gravityview_set_directory_widgets($post_id, $widgets = [])
{
 ...
 */
function get_gravityview($view_id = '', $atts = [])
{
    if (!empty($view_id)) {
        $atts['id'] = $view_id;
 ...
 * @return void
 */
function the_gravityview($view_id = '', $atts = [])
{
...
{
    if (class_exists('GFFormsModel') && $form_fields = GFFormsModel::get_fields_by_type($form, ['checkbox', 'radio'])) {

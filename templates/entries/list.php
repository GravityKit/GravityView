...
\GV\Mocks\Legacy_Context::push(['view' => $gravityview->view]);

$entry_slug = GravityView_API::get_entry_slug($entry->ID, $entry->as_entry());

extract($gravityview->template->extract_zone_vars(['title', 'subtitle']));
extract($gravityview->template->extract_zone_vars(['image', 'description', 'content-attributes']));
extract($gravityview->template->extract_zone_vars(['footer-left', 'footer-right']));
				...
				<?php
                    ...
                    if ($has_subtitle) {
                        ...
                        foreach ($subtitle->all() as $i => $field) {
                            ...
                            if ($did_main == 0) {
                                $did_main = 1;
                                $extras = ['markup' => '<h4 id="{{ field_id }}" class="{{ class }}">{{ label }}{{ value }}</h4>'];
                            }
        ...
        if ($has_image || $has_description || $has_content_attributes) {
				...
				<?php
                    if ($has_image) {
                        ...
                        foreach ($image->all() as $i => $field) {
                            echo $gravityview->template->the_field($field, ['zone_id' => 'single_list-image']);
                        } ?></div><?php
                    }

            if ($has_description) {
                ?><div class="gv-list-view-content-description"><?php
                        $extras = ['label_tag' => 'h4', 'wpautop' => true];
                foreach ($description->all() as $i => $field) {
                    $extras = ['wpautop' => true, 'zone_id' => 'single_list-description', 'label_markup'  => '<h4>{{ label }}</h4>'];
                    echo $gravityview->template->the_field($field, $extras);
                } ?></div><?php
            }
            ...
            if ($has_content_attributes) {
                ...
                foreach ($attributes->all() as $i => $field) {
                    $extras = ['zone_id' => 'single_list-content-attributes', 'markup' => '<p id="{{ field_id }}" class="{{ class }}">{{ label }}{{ value }}</p>'];
                    echo $gravityview->template->the_field($field, $extras);
                } ?></div><?php
            } ?>
				...
				<div class="gv-grid-col-1-2 gv-left">
					<?php
                        foreach ($footer_left->all() as $i => $field) {
                            echo $gravityview->template->the_field($field, ['zone_id' => 'single_list-footer-left']);
                        } ?>
				</div>
				...
				<div class="gv-grid-col-1-2 gv-right">
					<?php
                        foreach ($footer_right->all() as $i => $field) {
                            echo $gravityview->template->the_field($field, ['zone_id' => 'directory_list-footer-right']);
                        } ?>
				</div>
			</div>

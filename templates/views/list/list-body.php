...
if (!$gravityview->entries->count()) {
	...
	<?php
        ...
        $template::entry_before($entry, $gravityview);
             ...
             */
            extract($template->extract_zone_vars(['title', 'subtitle']));
		...
		<?php
             ...
             */
            extract($template->extract_zone_vars(['image', 'description', 'content-attributes']));
                ...
                if ($has_image) {
                    ?><div class="gv-grid-col-1-3 gv-list-view-content-image"><?php
                        foreach ($image->all() as $i => $field) {
                            echo $gravityview->template->the_field($field, $entry, ['zone_id' => 'directory_list-image']);
                        } ?></div><?php
                }
                ...
                if ($has_description) {
                    ?><div class="gv-grid-col-2-3 gv-list-view-content-description"><?php
                        foreach ($description->all() as $i => $field) {
                            $extras = ['wpautop' => true, 'zone_id' => 'directory_list-description', 'label_markup'  => '<h4>{{ label }}</h4>'];
                            echo $gravityview->template->the_field($field, $entry, $extras);
                        } ?></div><?php
                }
                ...
                if ($has_content_attributes) {
                    ?><div class="gv-grid-col-3-3 gv-list-view-content-attributes"><?php
                        foreach ($attributes->all() as $i => $field) {
                            $extras = ['zone_id' => 'directory_list-content-attributes', 'markup' => '<p id="{{ field_id }}" class="{{ class }}">{{ label }}{{ value }}</p>'];
                            echo $gravityview->template->the_field($field, $entry, $extras);
                        } ?></div><?php
                }
             ...
             */
            extract($template->extract_zone_vars(['footer-left', 'footer-right']));
				...
				<div class="gv-grid-col-1-2 gv-left">
					<?php
                        foreach ($footer_left->all() as $i => $field) {
                            echo $gravityview->template->the_field($field, $entry, ['zone_id' => 'directory_list-footer-left']);
                        } ?>
				</div>
				...
				<div class="gv-grid-col-1-2 gv-right">
					<?php
                        foreach ($footer_right->all() as $i => $field) {
                            echo $gravityview->template->the_field($field, $entry, ['zone_id' => 'directory_list-footer-right']);
                        } ?>
				</div>
			</div>

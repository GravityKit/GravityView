 ...
 * @since 2.3
 */
class GF_Patched_Query extends GF_Query
{
    /**
     ...
     * @param null      $paging
     */
    public function parse($form_id, $search_criteria = [], $sorting = null, $paging = null)
    {
        ...
        $from = !is_array($form_id) ? [$form_id] : $form_id;
        ...
        if (!empty($property_conditions)) {
            if (count($property_conditions) > 1) {
                $properties_condition = call_user_func_array(['GF_Query_Condition', '_and'], $property_conditions);
            } else {
        ...
        if (!empty($search_criteria['field_filters'])) {
            ...
            foreach ($field_filters as $filter) {
                ...
                if ($key == 'date_created' && $operator == GF_Query_Condition::EQ) {
                    ...
                    $filters[] = new GF_Query_Condition(
                        ...
                        new GF_Query_Series([new GF_Query_Literal($date_create_start_utc), new GF_Query_Literal($date_created_end_utc)])
                    );
            ...
            if (count($filters) > 1) {
                $filters_condition = call_user_func_array(['GF_Query_Condition', $condition_mode], $filters);
            } elseif ($filters) {
        ...
        if (!empty($properties_condition) && !empty($filters_condition)) {
            $where = call_user_func_array(['GF_Query_Condition', '_and'], [$properties_condition, $filters_condition]);
        } else {
    ...
    private function get_global_condition($filter, $form_ids)
    {
        ...
        if (count($choice_filters) > 1) {
            $combine_operator = in_array($operator, [GF_Query_Condition::NEQ, GF_Query_Condition::NIN]) ? '_and' : '_or';

            $condition = call_user_func_array(['GF_Query_Condition', $combine_operator], $choice_filters);
        } else {
    ...
    public function join($on_column, $to_column)
    {
        if ($on_column instanceof GF_Query_Column && $to_column instanceof GF_Query_Column) {
            $this->joins[] = [$on_column, $to_column];
        }
    ...
    public function order($column, $order = null)
    {
        ...
        if ($column instanceof GF_Query_Call) {
            ...
            $this->order[spl_object_hash($column).'()'] = [$column, $order];
            ...
            $this->order["{$source_id}_{$field_id}"] = [$column, $order];
        }
    ...
    private function query()
    {
        ...
        $paginate = implode(' ', array_filter([$limit, $offset], 'strlen'));
        ...
        $this->queries[] = [$this->timer_stop(), $sql];
    ...
    private function find_joins($condition, &$already_joined)
    {
         ...
         */
        if (in_array($condition->operator, [GF_Query_Condition::_AND, GF_Query_Condition::_OR])) {
    ...
    public function _join_infer($condition)
    {
        ...
        if (in_array($condition->operator, [GF_Query_Condition::_AND, GF_Query_Condition::_OR])) {
            ...
            /** Recurse and unwrap. */
            $_joins = array_map([$this, __FUNCTION__], $condition->expressions);
            $this->_inferred_joins = [];
            array_walk_recursive($_joins, [$this, 'merge_joins']);
        ...
        foreach ($condition->columns as $column) {
            ...
            if ($column->is_entry_column()) {
                ...
                if (!$column->alias && ($this->_alias(null, $column->source) !== 't1')) {
                    $this->_inferred_joins[] = sprintf(
                        '`%s` AS `%d` ON `%%s`.`id` = `%%s`.`entry_id`', // the rest is filled in during _priming
            ...
            if (!$column->is_meta_column() && $column->field_id != GF_Query_Column::META) {
                if ($column->field_id == intval($column->field_id) && ($field = GFFormsModel::get_field(GFAPI::get_form($column->source ? $column->source : reset($this->from)), $column->field_id)) && $field->get_entry_inputs()) {
                    ...
                    $this->_inferred_joins[] = sprintf(
                        '`%s` AS `%s` ON (`%s`.`entry_id` = `%s`.`id` AND `%s`.`meta_key` LIKE %s)',
                        GFFormsModel::get_entry_meta_table_name(),
                        $alias,
                        $alias,
                        $this->_alias(null, $column->source),
                        $alias,
                        $literal->sql($this)
                    );
                } else {
                    ...
                    $this->_inferred_joins[] = sprintf(
                        '`%s` AS `%s` ON (`%s`.`entry_id` = `%s`.`id` AND `%s`.`meta_key` = %s)',
                        GFFormsModel::get_entry_meta_table_name(),
                        $alias,
                        $alias,
                        $this->_alias(null, $column->source),
    ...
    {
        $this->_inferred_joins = array_merge($this->_inferred_joins, [$join]);
    }
                ...
                if (!in_array($matches[1], $explicit_join_aliases)) {
                    if (($key = array_search($matches[1], $this->aliases, true)) !== false) {
                        ...
                        if (isset($explicit_join_aliases[$form_id]) && strpos($explicit_join_aliases[$form_id], 't') !== 0) {
                            ...
                            $join = implode(' ON ', [$table, sprintf('`%s`.`entry_id` = `%s`.`entry_id`', $matches[1], $explicit_join_aliases[$form_id])]);
                        }
                    }
                    $remaining_inference_joins[] = $join;
                }
            } elseif (preg_match('#` AS `(\d+)` ON `%s`.`id` = `%s`.`entry_id`$#', $join, $matches)) {
    ...
    public function _where_unwrap($condition)
    {
        ...
        if (in_array($condition->operator, [GF_Query_Condition::_AND, GF_Query_Condition::_OR])) {
            ...
            /** Recurse. */
            $conditions = array_filter(array_map([$this, __FUNCTION__], $condition->expressions), 'strlen');
    ...
    {
        if (!in_array($type, [self::TYPE_SIGNED, self::TYPE_UNSIGNED, self::TYPE_DECIMAL])) {
            return $field_id;
        }
    ...
    public function get_entry($entry_id)
    {
        ...
        $entries = $this->get_entries([$entry_id]);
        if (empty($entries)) {
            return false;
        }
...
", $placeholders, $meta_clauses_str);
        ...
        foreach ($entryset as $entry) {
            ...
            foreach ($form['fields'] as $field) {
                ...
                if (is_array($inputs)) {
                    foreach ($inputs as $input) {
                        $entries[$entry['id']][(string) $input['id']] = gf_apply_filters([
                    ...
                    $entries[$entry['id']][$field->id] = gf_apply_filters([
                        'gform_get_input_value',
                        $form['id'],
                        $field->id,
                    ], $value, $entry, $field, '');
            ...
            if (is_callable(['GFFormsModel', 'hydrate_repeaters'])) {

...
class GravityView_Helper_Functions_Test extends GV_UnitTestCase
{
    ...
    public function test_gv_empty()
    {
        ...
        $this->assertTrue(gv_empty(['a' => '', '', []]));
        $this->assertFalse(gv_empty(['a' => '34', '', []]));
    ...
    public function test_gravityview_sanitize_html_class()
    {
        $classes = [
            ...
            // Implode with spaces
            'example dash bar' => gravityview_sanitize_html_class(['example', 'dash', 'bar']),

            // Again, don't strip spaces and implode
            'example-dash bar' => gravityview_sanitize_html_class(['example-dash', 'bar']),

            // Don't strip numbers or caps
            'Foo Bar0' => gravityview_sanitize_html_class(['Foo', 'Bar0']),

            // Strip whitespace
            'foo bar' => gravityview_sanitize_html_class(['foo    ', '           bar       ']),

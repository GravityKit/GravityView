...
class GVCommon_Test extends GV_UnitTestCase
{
     ...
     */
    public function test_has_gravityview_shortcode()
    {
        $post_without_shortcode = $this->factory->post->create_and_get(['post_content' => '[gravityview is not a shortcode']);
        $post_with_shortcode = $this->factory->post->create_and_get(['post_content' => '[gravityview]']);
     ...
     * @since 1.22.1
     */
    public function test_matches_operation()
    {
        ...
        $this->assertTrue(GVCommon::matches_operation(['json', 'encoded'], '["json","encoded"]', 'equals'));
        ...
        $this->assertTrue(GVCommon::matches_operation(['1'], ['1'], 'in'));
        $this->assertTrue(GVCommon::matches_operation(['1', '2', '3', '4'], [4], 'in')); // Number strings
        $this->assertTrue(GVCommon::matches_operation(['1.12345', '2', '3', '4'], [1.12345], 'in')); // Number strings
        $this->assertTrue(GVCommon::matches_operation(['1.12345', '2', '3', '4'], 1.12345, 'in'), 'Should allow for one to be an array and one not array');
        $this->assertTrue(GVCommon::matches_operation('1.12345', 1.12345, 'in'), 'Should allow for both to not be an array');
        $this->assertTrue(GVCommon::matches_operation([], ['1'], 'not_in'));
    }
    ...
    public function test_format_date()
    {
        ...
        $tests = [
            ...
            GVCommon::format_date($date_created, ['human' => true]) => GFCommon::format_date($date_created, true, '', false),

            // Blog "date at time" format ("%s at %s")
            GVCommon::format_date($date_created, 'time=1') => GFCommon::format_date($date_created, false, '', true),
            GVCommon::format_date($date_created, ['time' => true])=> GFCommon::format_date($date_created, false, '', true),

            // 1 second ago
            GVCommon::format_date($date_created, 'diff=1') => sprintf('%s ago', human_time_diff($entry_gmt_time)),
            GVCommon::format_date($date_created, ['diff' => true]) => sprintf('%s ago', human_time_diff($entry_gmt_time)),
            ...
            GVCommon::format_date($date_created, ['format' => 'm/d/Y']) => date_i18n('m/d/Y', $entry_local_time, true),
            GVCommon::format_date($date_created, ['format' => 'm/d/Y\ \w\i\t\h\ \t\i\m\e\ h\:i\:s']) => date_i18n('m/d/Y\ \w\i\t\h\ \t\i\m\e\ h:i:s', $entry_local_time, true),
        ];
    ...
    public function test_get_forms()
    {
        ...
        $i = 0;
        while ($i < 20) {
            GFAPI::add_form(['title' => rand_long_str(3), 'fields' => []]);
            $i++;
        }
     ...
     */
    public function test_get_connected_views()
    {
        $form_id = $this->factory->form->create();

        $this->factory->view->create_many(20, ['form_id' => $form_id]);
     ...
     */
    public function test_get_meta_form_id()
    {
        $form_id = '1234';
        $view_id = $this->factory->view->create(['form_id' => $form_id]);
     ...
     */
    public function test_get_meta_template_id()
    {
        $form_id = '1234';
     ...
     */
    public function test_get_link_html()
    {
        ...
        $this->assertEquals('<a href="#" title="New Title">Basic</a>', GVCommon::get_link_html('#', 'Basic', ['title' => 'New Title']));
        $this->assertEquals('<a href="#" title="New Title">Basic</a>', GVCommon::get_link_html('#', 'Basic', ['title' => 'New Title']));
        $this->assertEquals('<a href="#" onclick="alert(&quot;Javascript!&quot;);">Basic</a>', GVCommon::get_link_html('#', 'Basic', ['onclick' => 'alert("Javascript!");']));
        ...
        add_filter('gravityview/get_link/allowed_atts', [$this, '_filter_test_get_link_html']);
        $this->assertEquals('<a href="#">Basic</a>', GVCommon::get_link_html('#', 'Basic', ['onclick' => 'alert("Javascript!");']));
        remove_filter('', [$this, '_filter_test_get_link_html']);
    }
    ...
    {
        $this->assertTrue(GVCommon::entry_has_transaction_data(['transaction_id' => 1]));
        $this->assertTrue(GVCommon::entry_has_transaction_data(['transaction_id' => null, 'payment_status' => 'completed']));
        $this->assertFalse(GVCommon::entry_has_transaction_data(['transaction_id' => null, 'payment_status' => null]));
    ...
    public function test_get_product_field_types()
    {
        ...
        $this->assertEquals([], $empty_product_field_types, 'The gform_product_field_types filter did not work');
    }
    ...
    public function test_check_entry_display()
    {
        ...
        $view_cpt = $this->factory->view->create_and_get([
            'form_id'  => $form['id'],
            'settings' => $settings,
        ]);
        ...
        $entry2 = $this->factory->entry->create_and_get(['form_id' => $form2['id']]);

        // Entry is empty
        /** @var WP_Error $empty_array */
        $empty_array = GVCommon::check_entry_display([], $view);
        ...
        $form_id_not_set = GVCommon::check_entry_display(['not_empty' => true, 'doesnt_have_form_id' => true], $view);
        ...
        add_filter('gravityview_search_criteria', function () { return ['search_criteria' => null]; });
        ...
        add_filter('gravityview_search_criteria', function () { return ['search_criteria' => 1]; });
        ...
        add_filter('gravityview_search_criteria', function () {
            return [
                'search_criteria' => ['example' => 'not_empty'],
            ];
        });
    ...
    public function test_check_entry_display_categories()
    {
        ...
        add_filter('gravityview_search_criteria', function () use ($view, $cat_1) {
            return [
                'search_criteria' => [
                    'field_filters' => [
                        'mode' => 'all',
        ...
        add_filter('gravityview_search_criteria', function () use ($view , $cat_2) {
            return [
                'search_criteria' => [
                    'field_filters' => [
                        'mode' => 'all',
     ...
     */
    public function test_check_entry_display_dates()
    {
        $form = $this->factory->form->import_and_get('complete.json');
        $view = $this->factory->view->create_and_get(['form_id' => $form['id']]);
     ...
     * https://github.com/gravityview/GravityView/issues/1185.
     */
    public function test_check_entry_display_date_strings()
    {
        $form = $this->factory->form->import_and_get('complete.json');
        $view = $this->factory->view->create_and_get(['form_id' => $form['id']]);
     ...
     */
    public function test_check_entry_display_any_fields()
    {
     ...
     * https://github.com/gravityview/GravityView/issues/929.
     */
    public function test_check_entry_display_gf_query_filters()
    {
        $form = $this->factory->form->import_and_get('simple.json');
        $view = $this->factory->view->create_and_get(['form_id' => $form['id']]);
     ...
     */
    public function test_calculate_get_entries_criteria()
    {
        ...
        $_field_values_only_mode['search_criteria']['field_filters'] = ['mode' => 'all'];
        $_field_values_only_mode_expected['search_criteria']['field_filters'] = [];
        ...
        $this->assertEquals([], GVCommon::calculate_get_entries_criteria($default_values));
    ...
    private function _calculate_get_entries_criteria_dates($default_values)
    {
        ...
        $_date_create_false_expected['search_criteria'] = [];
        ...
        $_date_create_end_invalid_expected['search_criteria'] = [
    ...
    {
        $search_criteria_without_operator = [
            'search_criteria' => [
                'field_filters' => [
                    'mode' => 'all',
                    0      => [
        ...
        $search_criteria_without_operator_expected['search_criteria'] = [
            'field_filters' => [
                'mode' => 'all',
                0      => [
                    'key'      => 'created_by',
        ...
        $search_criteria_without_operator_with_filter_expected['search_criteria'] = [
            'field_filters' => [
                'mode' => 'all',
                0      => [
                    'key'      => 'created_by',
     ...
     * @group has_shortcode
     */
    public function test_has_shortcode_r()
    {
        ...
        $shortcode_exists = [
            ...
            '[shortcode_one][shortcode_two][gravityview][/shortcode_two][/shortcode_one]'                                                                                                                               => ['[gravityview]'],
            '[shortcode_one] [shortcode_two] [gravityview /] [/shortcode_two] [/shortcode_one]'                                                                                                                         => ['[gravityview /]'],
            '[shortcode_one][gravityview] [gravityview id="12345" attributes="custom"][/shortcode_one]'                                                                                                                 => ['[gravityview]', '[gravityview id="12345" attributes="custom"]'],
            '[shortcode_one][shortcode_two][gravityview /][/shortcode_two][/shortcode_one]'                                                                                                                             => ['[gravityview /]'],
            '[embed_wrapper][embed_level_1][embed_level_2][gravityview id="3416"][/embed_level_2][/embed_level_1][embed_level_1][embed_level_2][gravityview id="3418"][/embed_level_2][/embed_level_1][/embed_wrapper]' => ['[gravityview id="3416"]', '[gravityview id="3418"]'],
        ];

...
class GravityView_Shortcode_Test extends GV_UnitTestCase
{
     ...
     */
    public function test_shortcode_get_view_detail_PAGING()
    {
     ...
     * @covers GravityView_View::setTotalEntries
     */
    public function test_shortcode_get_view_detail_TOTAL_ENTRIES()
    {
        $form = $this->factory->form->import_and_get('complete.json');
    ...
    public function test_parse_and_sanitize_atts()
    {
        ...
        $tests = [
            'Strip nothing, make sure it is working' => [
                'original' => [
                    'id' => 123,
                ],
                'expected' => [
            ...
            'Test {get} merge tag, valid value' => [
                'get'      => ['view_id' => 123],
                'original' => [
                    'id' => '{get:view_id}',
                ],
            ...
            'Test {get} merge tag, invalid value (string, not numeric)' => [
                'get'      => ['view_id' => 'asdasdsd'],
                'original' => [
                    'id' => '{get:view_id}',
                ],
                'expected' => [],
            ...
            'Strip attributes not defined in GravityView_View_Data::get_default_args' => [
                'original' => [
                    'id'                    => 123,
                    'page_size'             => 12,
            ...
            'Number settings that are not numbers should be stripped' => [
                'original' => [
                    'id' => 'asdasd',
                ],
                'expected' => [
            ...
            'Number settings that numbers should not be stripped' => [
                'original' => [
                    'search_field' => '123.10',
                ],
                'expected' => [
            ...
            'Checkbox non-numeric string should be based on gv_empty()' => [
                'original' => [
                    'id'       => 123,
                    'lightbox' => '',
                ],
            ...
            'Select options that exist should not be stripped' => [
                'original' => [
                    'id'             => 123,
                    'sort_direction' => 'ASC',
                ],
            ...
            'Select options that do not exist should be stripped' => [
                'original' => [
                    'id'             => 123,
            ...
            'Search operators that are valid should not be stripped' => [
                'original' => [
                    'id'              => 123,
                    'search_operator' => 'is',
                ],
            ...
            'More search operators that are valid should not be stripped' => [
                'original' => [
                    'id'              => 123,
            ...
            'Search operators that do not exist should be stripped' => [
                'original' => [
                    'id'              => 123,
            ...
            '{get} should not pass unsanitized stuff' => [
                'get' => [
    ...
    public function test_shortcode_search_criteria()
    {
        ...
        add_filter('gravityview_get_entries', function ($parameters) use ($_this) {
            $_this->assertEquals($parameters['paging'], ['page_size' => 13, 'offset' => 7]);
        ...
        add_filter('gravityview_get_entries', function ($parameters) use ($_this) {
            $_this->assertEquals($parameters['paging'], ['page_size' => 5, 'offset' => 5]);
        ...
        add_filter('gravityview_get_entries', function ($parameters) use ($_this) {
            $_this->assertEquals($parameters['paging'], ['page_size' => 5, 'offset' => 23]);
    ...
    public function test_shortcode_abstract()
    {
        ...
        $this->assertEmpty($shortcode->callback([]));
    }
    ...
    public function test_shortcode_single_view_from_directory()
    {
        ...
        $this->assertContains($field, $shorcode->callback(['id' => $view->ID]));
    ...
    public function test_shortcode_search()
    {
        ...
        $content = $shortcode->callback(['id' => $view->ID]);
        $this->assertContains('data-label="Text">abc</td>', $content);
        $this->assertContains('data-label="Text">abcxyz</td>', $content);

        $shortcode = new \GV\Shortcodes\gravityview();
        $content = $shortcode->callback(['id' => $view->ID, 'search_field' => '1', 'search_value' => 'abcxyz']);
        $this->assertNotContains('data-label="Text">abc</td>', $content);
        $this->assertContains('data-label="Text">abcxyz</td>', $content);

        $shortcode = new \GV\Shortcodes\gravityview();
        $content = $shortcode->callback(['id' => $view->ID, 'search_field' => '1', 'search_value' => 'abc', 'search_operator' => 'is']);
        $this->assertContains('data-label="Text">abc</td>', $content);

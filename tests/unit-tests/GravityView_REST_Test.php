...
class GravityView_REST_Test extends GV_RESTUnitTestCase
{
    ...
    public function test_context_param()
    {
        ...
        $this->assertEquals(['page', 'limit', 'post_id'], array_keys($data['endpoints'][0]['args']));
    ...
    public function test_get_items()
    {
        ...
        $view3 = $this->factory->view->create_and_get(['form_id' => $form['id']]);
        ...
        $entry1 = $this->factory->entry->import_and_get('simple_entry.json', [
            'form_id' => $form['id'],
        ...
        $this->assertEqualSets(['id', 1, 'ip'], array_keys($entries['entries'][0]));
    ...
    public function test_get_security()
    {
        ...
        $view = $this->factory->view->create_and_get([
            ...
            'settings' => ['show_only_approved' => true],
        ]);
        $view2 = $this->factory->view->create_and_get(['form_id' => $form['id']]);
        $view3 = $this->factory->view->create_and_get(['form_id' => $form['id']]);
        $view4 = $this->factory->view->create_and_get(['form_id' => $form['id']]);
        $view5 = $this->factory->view->create_and_get(['form_id' => $form['id']]);
        $view6 = $this->factory->view->create_and_get(['form_id' => $form['id']]);
        $view7 = $this->factory->view->create_and_get(['form_id' => $form['id']]);

        // These should not be seen by regular users
        wp_update_post(['ID' => $view4->ID, 'post_password' => '123']);
        wp_update_post(['ID' => $view5->ID, 'post_status' => 'private']);
        wp_update_post(['ID' => $view6->ID, 'post_status' => 'trash']);
        wp_update_post(['ID' => $view7->ID, 'post_status' => 'draft']);
        ...
        $view8 = $this->factory->view->create_and_get(['form_id' => $form['id']]);
        ...
        $view9 = $this->factory->view->create_and_get(['form_id' => $form['id'], 'settings' => ['rest_disable' => '1']]);

        $request = new WP_REST_Request('GET', '/gravityview/v1/views/'.$view9->ID);
        $response = rest_get_server()->dispatch($request);
        $this->assertEquals(500, $response->status);

        $view10 = $this->factory->view->create_and_get(['form_id' => $form['id'], 'settings' => ['rest_disable' => '0']]);
        ...
        /** Enable the REST API */
        gravityview()->plugin->settings->set(['rest_api' => '0']);
        add_action('gravityview/settings/defaults', $callback = function ($defaults) {
            $defaults['rest_api'] = '0';

            return $defaults;
        });
        $view11 = $this->factory->view->create_and_get(['form_id' => $form['id'], 'settings' => ['rest_disable' => '0']]);

        $request = new WP_REST_Request('GET', '/gravityview/v1/views/'.$view11->ID);
        $response = rest_get_server()->dispatch($request);
        $this->assertEquals(500, $response->status);

        $view12 = $this->factory->view->create_and_get(['form_id' => $form['id'], 'settings' => ['rest_enable' => '1']]);

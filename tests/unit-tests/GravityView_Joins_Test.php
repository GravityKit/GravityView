...
class GravityView_Joins_Test extends GV_UnitTestCase
{
    ...
    public function test_basic_table_joins()
    {
        ...
        $entries[] = $this->factory->entry->create_and_get([
            'form_id' => $orders['id'],
            'status'  => 'active',
            '9'       => 4,
            '16'      => 'Shoes',
    ...
    public function test_joins_with_approves()
    {
        ...
        $customer = $this->factory->entry->create_and_get([
            'form_id' => $customers['id'],
            'status'  => 'active',
            '1'       => 'Ann',
            '2'       => 1,
        ...
        $order = $this->factory->entry->create_and_get([
            'form_id' => $orders['id'],
            'status'  => 'active',
            '9'       => 1,
            '16'      => 'Shoes',
    ...
    public function test_search_widget()
    {
        ...
        $this->factory->entry->create_and_get(['form_id' => $chefs['id'], 'status' => 'active', '2' => 1, '1' => 'Maria Henry']);
        $this->factory->entry->create_and_get(['form_id' => $chefs['id'], 'status' => 'active', '2' => 2, '1' => 'Henry Marrek']);

        $this->factory->entry->create_and_get(['form_id' => $souschefs['id'], 'status' => 'active', '2' => 1, '1' => 'Mary Jane']);
        $this->factory->entry->create_and_get(['form_id' => $souschefs['id'], 'status' => 'active', '2' => 1, '1' => 'Jane Henryson']);
        $this->factory->entry->create_and_get(['form_id' => $souschefs['id'], 'status' => 'active', '2' => 2, '1' => 'Marick Bonobo']);
        $this->factory->entry->create_and_get(['form_id' => $souschefs['id'], 'status' => 'active', '2' => 2, '1' => 'Henry Oswald']);
        ...
        $_GET = ['filter_1' => 'Henryson'];
        $this->assertEquals(0, $view->get_entries()->count());

        $_GET = ['filter_1' => 'Mari'];
        $entries = $view->get_entries()->all();
        $entries_chefs = [$entries[0][$chefs['id']]['1'], $entries[1][$chefs['id']]['1']];
        $entries_souschefs = [$entries[0][$souschefs['id']]['1'], $entries[1][$souschefs['id']]['1']];
        ...
        $_GET = ['filter_1:'.$souschefs['id'] => 'Marr'];
        $entries = $view->get_entries()->all();
        ...
        $_GET = ['filter_1:'.$chefs['id'] => 'Bono'];
        $this->assertCount(4, $view->get_entries()->all());

        $_GET = ['filter_1:'.$souschefs['id'] => 'Bono'];
        $entries = $view->get_entries()->all();
    ...
    public function test_search_widget_global_search($mode = 'any')
    {
        ...
        $_GET = ['gv_search' => 'en', 'mode' => $mode];
        $this->assertEquals(4, $view->get_entries()->count());

        $_GET = ['gv_search' => 'rick', 'mode' => $mode];
    ...
    public function test_single_entry_join_permissions()
    {
        ...
        $c1 = $this->factory->entry->create_and_get(['form_id' => $chefs['id'], 'status' => 'active', '2' => 1, '1' => 'Maria Henry']);
        $c2 = $this->factory->entry->create_and_get(['form_id' => $chefs['id'], 'status' => 'active', '2' => 2, '1' => 'Henry Marrek']);

        $s1 = $this->factory->entry->create_and_get(['form_id' => $souschefs['id'], 'status' => 'active', '2' => 1, '1' => 'Mary Jane']);
        $s2 = $this->factory->entry->create_and_get(['form_id' => $souschefs['id'], 'status' => 'active', '2' => 1, '1' => 'Jane Henryson']);
        $s3 = $this->factory->entry->create_and_get(['form_id' => $souschefs['id'], 'status' => 'active', '2' => 2, '1' => 'Marick Bonobo']);
        $s4 = $this->factory->entry->create_and_get(['form_id' => $souschefs['id'], 'status' => 'active', '2' => 2, '1' => 'Henry Oswald']);

        $n1 = $this->factory->entry->create_and_get(['form_id' => $souschefs['id'], 'status' => 'active', '1' => 'None']);
        ...
        $this->assertContains('entry='.implode(',', [$c1['id'], $s1['id']]), $output);
        $this->assertContains('entry='.implode(',', [$c1['id'], $s2['id']]), $output);
        $this->assertContains('entry='.implode(',', [$c2['id'], $s3['id']]), $output);
        $this->assertContains('entry='.implode(',', [$c2['id'], $s4['id']]), $output);
        ...
        gravityview()->request->returns['is_entry'] = \GV\Multi_Entry::from_entries([
    ...
    public function test_union_simple()
    {
        ...
        $this->factory->entry->create_and_get(['form_id' => $form_2['id'], 'status' => 'active', '16' => 'neptune@gravityview.co']);
        $this->factory->entry->create_and_get(['form_id' => $form_1['id'], 'status' => 'active', '1'  => 'earth@gravityview.co']);
        $this->factory->entry->create_and_get(['form_id' => $form_1['id'], 'status' => 'active', '1'  => 'saturn@gravityview.co']);
        $this->factory->entry->create_and_get(['form_id' => $form_2['id'], 'status' => 'active', '16' => 'venus@gravityview.co']);
        $this->factory->entry->create_and_get(['form_id' => $form_2['id'], 'status' => 'active', '16' => 'mars@gravityview.co']);
        $this->factory->entry->create_and_get(['form_id' => $form_1['id'], 'status' => 'active', '1'  => 'uranus@gravityview.co']);
        $this->factory->entry->create_and_get(['form_id' => $form_2['id'], 'status' => 'active', '16' => 'jupiter@gravityview.co']);
        $this->factory->entry->create_and_get(['form_id' => $form_2['id'], 'status' => 'active', '16' => 'mercury@gravityview.co']);
        ...
        $_GET = [];
        $view->settings->update(['page_size' => 25]);
    ...
    public function test_joins_on_entry_columns()
    {
        ...
        $entry1_1 = $this->factory->entry->create_and_get([
            'form_id' => $step1['id'],
            'status'  => 'active',
            '1'       => 'Entry 1',
        ]);

        $entry1_2 = $this->factory->entry->create_and_get([
            'form_id' => $step2['id'],
            'status'  => 'active',
            '1'       => 'After Entry 1',
            '2'       => $entry1_1['id'],
    ...
    public function test_joins_joined_status()
    {
        ...
        $inactive = $this->factory->entry->create_and_get([
            'form_id' => $step2['id'],
            'status'  => 'trash',
            '1'       => 'Not active',
            '2'       => $entry1_1['id'],

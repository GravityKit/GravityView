...
class GravityView_Connector_Functions_Test extends GV_UnitTestCase
{
    ...
    public function test_has_gravityview_shortcode()
    {
        ...
        $shortcode = $this->factory->post->create_and_get(['post_content' => '[gravityview]']);
     ...
     */
    public function test_gravityview_has_shortcode_r()
    {
        $single_view = [
            '[gravityview id=123]',
            null,
        ...
        $this->assertEquals($shortcodes, [$single_view], 'failed unnested test');

        $shortcodes = gravityview_has_shortcode_r('[gravityview id=123][gravityview id=123]');

        $this->assertEquals($shortcodes, [$single_view, $single_view], 'unnested two shortcodes test');
        ...
        $this->assertEquals($shortcodes, [$single_view, $single_view, $single_view], 'nested three shortcodes');
    }
}

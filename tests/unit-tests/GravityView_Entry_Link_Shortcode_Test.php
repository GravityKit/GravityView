...
{
    /** @var GravityView_Entry_Link_Shortcode */
    public $object;
     ...
     * @since 2.0.9
     */
    public function _test_embed($view, $entry, $atts)
    {
        $post_title = new WP_UnitTest_Generator_Sequence(__METHOD__.' %s');
        $post_id = $this->factory->post->create([
            'post_title'   => $post_title->next(),
     ...
     */
    public function _test_read($view, $entry, $atts)
    {
     ...
     */
    public function _test_delete($view, $entry, $atts)
    {

        // NO CAPS
        $this->factory->user->create_and_set(['user_login' => 'zero', 'role' => 'zero']);

        $zero_link = $this->object->delete_shortcode($atts);
        $this->assertNull($zero_link, 'user without caps shouldn\'t see delete link');

        // ADMIN
        $this->factory->user->create_and_set(['user_login' => 'administrator', 'role' => 'administrator']);
     ...
     */
    public function _test_edit($view, $entry, $atts)
    {
        ...
        \GV\Mocks\Legacy_Context::push(['view' => \GV\View::from_post($view), 'entry' => \GV\GF_Entry::from_entry($entry), 'post' => $post]);

        $edit_link_no_atts = $this->object->edit_shortcode([]);
        ...
        $edit_link_no_atts_custom_slug = $this->object->edit_shortcode([]);
        ...
        $zero = $this->factory->user->create_and_set(['role' => 'zero']);

...
class GV_20_Issues_Test extends GV_UnitTestCase
{
     ...
     * @since 2.0.6.2
     */
    public function test_gv_age_shortcode()
    {
        add_shortcode('gv_age_1_x', [$this, '_gv_age_1_x_shortcode']);
        add_shortcode('gv_age_2_0', [$this, '_gv_age_2_0_shortcode']);
     ...
     * @return int|string
     */
    public function _gv_age_1_x_shortcode($atts = [], $content = null)
    {
     ...
     * @return string
     */
    public function _gv_age_2_0_shortcode($atts)
    {
        $defaults = [
            'field_id'    => '',
            'entry_id'    => '',
    ...
    public function test_shortcode_search_value_search_filter()
    {
        $form = $this->factory->form->import_and_get('complete.json');
    ...
    public function test_gravityview_entries_pass_count_by_reference()
    {
        ...
        list($entries, $paging, $count) = \GV\Mocks\GravityView_frontend_get_view_entries(['id' => $view->ID], $form['id'], [
            'paging' => ['current_page' => 1, 'offset' => 0, 'page_size' => 25],
        ], 0);

        $this->assertEquals([
            [], ['offset' => 0, 'page_size' => 25], 0,
    ...
    public function test_fileupload_download_link_index_php_detection()
    {
        ...
        $entry = $this->factory->entry->create_and_get([
            'form_id' => $form['id'],
            '5'       => json_encode($files = [$upload_url.'/one.jpg', $upload_url.'/two.mp3']),
        ]);
        $view = $this->factory->view->create_and_get([
            'form_id'  => $form['id'],
            'settings' => [
        ...
        $field->update_configuration(['link_to_file' => false]);
        $field->update_configuration(['show_as_link' => false]);
    ...
    public function test_entry_by_non_unique_slug()
    {
        ...
        $entry1 = $this->factory->entry->create_and_get(['form_id' => $form1['id']]);

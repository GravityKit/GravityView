...
class GVFuture_Test extends GV_UnitTestCase
{
    ...
    public function test_entry_endpoint_rewrite_name()
    {
        $entry_enpoint = array_filter($GLOBALS['wp_rewrite']->endpoints, function ($endpoint) {
            return $endpoint === [EP_PERMALINK | EP_ROOT | EP_PAGES, 'entry', 'entry'];
        });
    ...
    public function test_view_edit_create_permissions()
    {
        ...
        $contributor = $this->factory->user->create(
            [
                'user_login' => md5(microtime()),
                'user_email' => md5(microtime()).'@gravityview.tests',
                'role'       => 'contributor', ]
    ...
    public function test_entry_by_slug()
    {
        ...
        $view = $this->factory->view->create_and_get(['form_id' => $form['id']]);
    ...
    public function test_entry_get_permalink()
    {
        ...
        $view = $this->factory->view->create_and_get(['form_id' => $form['id']]);
        $another_view = $this->factory->view->create_and_get(['form_id' => $form['id']]);
        $and_another_view = $this->factory->view->create_and_get(['form_id' => $form['id']]);
        ...
        $expected_url = add_query_arg(['entry' => $entry->ID], get_permalink($view->ID));
        $this->assertEquals($expected_url, $entry->get_permalink($view, $request));

        /** With tracking. */
        $_GET = ['pagenum' => 1, 'sort' => '4', 'dir' => 'rand'];
        ...
        /** One embedded View */
        $post = $this->factory->post->create_and_get(['post_content' => '[gravityview id="'.$another_view->ID.'"]']);
        $expected_url = add_query_arg(['entry' => $entry->ID], get_permalink($post->ID));
        $this->assertEquals($expected_url, $entry->get_permalink($view, $request));

        /** Multiple embedded Views */
        $post = $this->factory->post->create_and_get(['post_content' => '[gravityview id="'.$another_view->ID.'"] [gravityview id="'.$view->ID.'"]']);
        $expected_url = add_query_arg(['gvid' => $view->ID, 'entry' => $entry->ID], get_permalink($post->ID));
        $this->assertEquals($expected_url, $entry->get_permalink($view, $request));

        /** Multiple embedded Views, even if they are not the View of the entry */
        $post = $this->factory->post->create_and_get(['post_content' => '[gravityview id="'.$another_view->ID.'"] [gravityview id="'.$and_another_view->ID.'"]']);
        $expected_url = add_query_arg(['gvid' => $view->ID, 'entry' => $entry->ID], get_permalink($post->ID));
        $this->assertEquals($expected_url, $entry->get_permalink($view, $request));
        ...
        $this->assertEquals(add_query_arg(['gvid' => $view->ID, 'entry' => $entry->ID], 'ooh'), $entry->get_permalink($view, $request));
    ...
    public function test_view_data_compat()
    {
        ...
        foreach (['id', 'view_id', 'form_id', 'template_id', 'atts', 'fields', 'widgets', 'form'] as $key) {
    ...
    public function test_data_get_views()
    {
        ...
        $wp_admin_bar->expects($this->exactly(4))->method('add_menu')
                ->withConsecutive(
                    ...
                    [$this->callback(function ($subject) use ($another_view) {
        ...
        $and_another_entry = $this->factory->entry->create_and_get(['form_id' => $and_another_view->form->ID, 'status' => 'active']);

        gravityview()->request->returns['is_view'] = $and_another_view;

        $fe->setIsGravityviewPostType(true);
        $this->assertContains('not allowed to view this content', $fe->render_view([
            'id'         => $and_another_view->ID,
            'embed_only' => true, /** Check propagation of $passed_args */
        ]));

        $this->assertContains('gv-container-'.$and_another_view->ID, $fe->render_view([
            'id'         => $and_another_view->ID,
         ...
         * The back link.
         */
        $this->assertContains(sprintf('data-viewid="%d"', $and_another_view->ID), $fe->render_view([
            'id'    => $and_another_view->ID,
            'debug' => true,
        ]));
    }
    ...
    public function test_view_collection_from_post()
    {
        ...
        $with_shortcodes = $this->factory->post->create_and_get([
        ...
        update_post_meta($with_shortcodes_in_meta->ID, 'json_meta_test', json_encode([['random' => 'json', 'has_shortcode' => sprintf('[gravityview id="%d"][gravityview id="%d"]', $post->ID, $another_post->ID)]]));

        /** And make sure arrays don't break things. */
        update_post_meta($with_shortcodes_in_meta->ID, 'invalid_meta_test', ['do not even try to parse this']);
        ...
        add_filter('gravityview/view_collection/from_post/meta_keys', function ($meta_keys, $post) use ($with_shortcodes_in_meta, $test) {
            ...
            return ['meta_test', 'invalid_meta_test'];
        }, 10, 2);
        ...
        $with_bad_shortcodes = $this->factory->post->create_and_get([
        ...
        $this->assertEquals($data->maybe_get_view_id([$post, $another_post]), [$post->ID, $another_post->ID]);
        $this->assertEquals($data->maybe_get_view_id($with_shortcodes), [$post->ID, $another_post->ID]);
        ...
        $this->_reset_context();
        $form = $this->factory->form->create_and_get();
        $entry = $this->factory->entry->create_and_get(['form_id' => $form['id'], 'status' => 'active']);
        $view = $this->factory->view->create_and_get(['form_id' => $form['id']]);
        $post = $this->factory->post->create_and_get(['post_content' => sprintf('[gravityview id="%d"]', $view->ID)]);
    ...
    public function test_form_gravityforms()
    {
        ...
        $this->assertNull(\GV\GF_Form::from_form([]));
    }
    ...
    public function test_shortcode_parse()
    {
        ...
        $this->assertEquals($shortcodes[0]->atts, ['id' => '1', 'm' => '2']);
        ...
        $this->assertEquals($shortcodes[1]->atts, ['id' => '2', 'one' => 3]);
        ...
        $this->assertEquals([-1, -2], $data->parse_post_content('[gravityview id="-1"][gravityview id="-2"]'));
        /** The above calls have a side-effect on the data state; make sure it's still intact. */
        $this->assertEquals($data->get_views(), []);
    }
    ...
    public function test_default_request_is_admin()
    {
        ...
        $this->assertFalse($entry_link_shortcode->read_shortcode(['view_id' => 1, 'entry_id' => 1]));
        set_current_screen('dashboard');
        $this->assertNull($entry_link_shortcode->read_shortcode(['view_id' => 1, 'entry_id' => 1]));

        /** \GVLogic_Shortcode::shortcode short circuits as well. */
        set_current_screen('front');
        $logic_shortcode = \GVLogic_Shortcode::get_instance();
        $this->assertEquals($logic_shortcode->shortcode(['if' => 'true', 'is' => 'true'], 'sentinel'), 'sentinel');
        set_current_screen('dashboard');
        $this->assertEmpty($logic_shortcode->shortcode(['if' => 'true', 'is' => 'true'], 'sentinel'), 'sentinel');
    ...
    public function test_settings_base()
    {
        ...
        $settings = new \GV\Settings(['one' => 'six']);
    ...
    public function test_view_settings()
    {
        ...
        add_filter('gravityview/view/settings/defaults', function ($defaults) {
            $defaults['test_sentinel'] = ['value' => '456'];
        ...
        $settings->update(['valid_key' => 'this exists', 'test_sentinel' => '789']);
    ...
    public function test_logging()
    {
        ...
        if (version_compare(phpversion(), '5.4', '>=')) {
            add_action('gravityview_log_debug_test', function ($message, $data) use ($_this) {
                ...
                $_this->assertEquals($data, ['a' => 'b']);
            }, 10, 2);
            gravityview()->log->info('Hello, {world}!', ['world' => 'TRAPPIST-1', 'data' => ['a' => 'b']]);
            ...
            gravityview()->log->critical('Hello, {world}!', ['world' => 'TRAPPIST-1', 'data' => ['a' => 'b']]);
    ...
    public function test_field_and_field_collection()
    {
        ...
        $this->assertEquals(
            ['id', 'label', 'show_label', 'custom_label', 'custom_class', 'only_loggedin', 'only_loggedin_cap', 'search_filter', 'show_as_link'],
            array_keys($field->as_configuration())
        );

        /** Field configuration and update configuration. */
        $field = \GV\Field::from_configuration(
            ['id' => 'custom', 'label' => 'Custom', 'content' => 'Wow!']
        );
        ...
        $field->update_configuration(['update' => 'Now!', 'id' => 4]);

        $this->assertEquals(4, $field->ID);
        $this->assertEquals('Wow!', $field->content);

        /** Configuration implementations: \GV\Internal_Field */
        $field = \GV\Field::from_configuration(['id' => 'custom']);
        $this->assertInstanceOf('\GV\Internal_Field', $field);
        $this->assertEquals('custom', $field->ID);

        /** Configuration implementations: \GV\GF_Field */
        $field = \GV\Field::from_configuration(['id' => 499]);
        $this->assertInstanceOf('\GV\Field', $field);
        $form = $this->factory->form->import_and_get('simple.json');
        $field = \GV\Field::from_configuration(['id' => 1, 'form_id' => $form['id']]);
        $this->assertInstanceOf('\GV\GF_Field', $field);
        $this->assertEquals('text', $field->type);
        $field = \GV\Field::from_configuration(['id' => 2, 'form_id' => $form['id']]);
        $this->assertInstanceOf('\GV\GF_Field', $field);
        ...
        $field = \GV\Field::from_configuration(['id' => 1]);
        $this->assertInstanceOf('\GV\Field', $field);
        ...
        /** Mass configuration. */
        $fields = \GV\Field_Collection::from_configuration([
            'directory_list-title' => [
                'ffff0001' => ['id' => 1, 'form_id' => $form['id'], 'label' => 'Hi there :)'],
                'ffff0002' => ['id' => 2, 'form_id' => $form['id'], 'label' => 'Hi there, too :)'],
                'ffff0003' => ['id' => 'custom', 'only_loggedin_cap' => 'read'],
            ],
            'single_list-title' => [
                'ffff0004' => ['id' => 1, 'label' => 'Hi there :)', 'custom_class' => 'red'],
            ],
        ]);
        ...
        $this->assertEquals(['directory_list-title', 'single_list-title'], array_keys($fields->as_configuration()));
        ...
        $fields = \GV\Field_Collection::from_configuration([
            'default' => [
                '000a' => ['only_loggedin' => '1', 'only_loggedin_cap' => 'manage_options'],
                '000b' => ['only_loggedin' => '1', 'only_loggedin_cap' => 'read'],
                '000c' => ['only_loggedin' => '0', 'only_loggedin_cap' => 'read' /** Only valid when only_loggedin is set */],
            ],
        ]);
        ...
        $field->update_configuration(['custom_label' => 'This is a custom label']);
        ...
        $field = \GV\GF_Field::by_id($view->form, '4');
        $field->update_configuration(['custom_label' => 'Hobbies <small>Multiselect</small>']);
        ...
        $field->update_configuration(['custom_label' => 'This is {entry_id}']);
        ...
        $field->update_configuration(['show_label' => '0', 'custom_label' => 'This is {entry_id}']);
        $this->assertEquals('', $field->get_label($view, $view->form, $entry));

        /** Internal fields. */
        $field = \GV\Internal_Field::by_id('id');
        $field->update_configuration(['label' => 'ID <small>Entry</small>']);
        $this->assertEquals('ID <small>Entry</small>', $field->get_label());

        /** Show label false for Internal fields. */
        $field->update_configuration(['show_label' => '0', 'label' => 'Mesa busten wit happiness seein yousa again, Ani']);
        $this->assertEquals('', $field->get_label());
        $this->assertEquals('', $field->get_label($view, $view->form, $entry));

        /** Custom label override and merge tags. */
        $field->update_configuration(['show_label' => '1', 'custom_label' => 'This is {entry_id}']);
        $this->assertEquals('This is '.$entry->ID, $field->get_label($view, $view->form, $entry));
    }
    ...
    public function test_field_value_compat()
    {
        ...
        gravityview()->request = new \GV\Mock_Request();
        gravityview()->request->returns['is_view'] = gravityview()->views->get($this->factory->view->create_and_get(['form_id' => $form['id']]));
        ...
        $form = $this->factory->form->create_and_get();
        $form = \GV\GF_Form::by_id($form['id']);
        $entry = $this->factory->entry->import_and_get('standard_entry.json', [
            'form_id' => $form->ID,
        ]);
        $entry = \GV\GF_Entry::by_id($entry['id']);
    ...
    {
        $this->assertEquals(\GV\View_Template::split_slug('main'), ['', 'main']);
        $this->assertEquals(\GV\View_Template::split_slug('secondary', 'part'), ['', 'secondary-part']);
        $this->assertEquals(\GV\View_Template::split_slug('partial/sub'), ['partial/', 'sub']);
        $this->assertEquals(\GV\View_Template::split_slug('partial/sub', 'part'), ['partial/', 'sub-part']);
        $this->assertEquals(\GV\View_Template::split_slug('partial/fraction/atom', ''), ['partial/fraction/', 'atom']);
        $this->assertEquals(\GV\View_Template::split_slug('partial/fraction/atom', 'quark'), ['partial/fraction/', 'atom-quark']);
    }
    ...
    public function test_filter_entries()
    {
         ...
         */
        add_filter('gravityview/view/entries', $callback = [$this, '_filter_gravityview_view_entries'], 10, 3);
     ...
     */
    public function _filter_gravityview_view_entries($entries, $view, $request)
    {
    ...
    public function test_frontend_view_renderer_table()
    {
        ...
        $view = \GV\View::from_post($post);
        $view->settings->update(['page_size' => 3]);
        $entries = new \GV\Entry_Collection();
        ...
        /** Search */
        $_GET = ['pagenum' => 1, 'gv_search' => 'thisissomemoretext'];
        ...
        $_GET = ['pagenum' => 2, 'gv_search' => 'thisissomemoretext'];
        ...
        $_GET = ['pagenum' => 3, 'gv_search' => 'thisissomemoretext'];
        ...
        /** Hide until searched */
        $view->settings->update(['hide_until_searched' => true]);
        ...
        $view->settings->update(['hide_until_searched' => false, 'show_only_approved' => true]);
    ...
    public function test_frontend_field_html_address()
    {
        ...
        $field->update_configuration(['show_map_link' => true]);
        ...
        $field->update_configuration(['show_map_link' => false]);
    ...
    public function test_frontend_field_html_checkbox()
    {
        ...
        $field = \GV\GF_Field::by_id($form, '2.2');
        $field->update_configuration(['choice_display' => 'value']);
        $this->assertEquals('', $renderer->render($field, $view, $form, $entry, $request));
        $field->update_configuration(['choice_display' => 'label']);
        $this->assertEquals('', $renderer->render($field, $view, $form, $entry, $request));
    ...
    public function test_frontend_field_html_other_entries()
    {
        ...
        $field->update_configuration(['no_entries_text' => 'The user has no other entries, none at all.']);

        $this->assertEquals("<div class=\"gv-no-results\"><p>The user has no other entries, none at all.</p>\n</div>", $renderer->render($field, $view, null, $entry, $request));

        $entry_anon = $this->factory->entry->create_and_get([
            'form_id'    => $form['id'],
            'created_by' => 0,
            'status'     => 'active',
        ]);
        ...
        $field->update_configuration(['no_entries_hide' => true]);
        ...
        $field->update_configuration(['link_format' => 'Entry #{entry_id}', 'after_link' => 'wut']);
        $expected = sprintf(
            '<ul><li><a href="%s">Entry #%d</a><div>wut</div></li><li><a href="%s">Entry #%d</a><div>wut</div></li></ul>',
            esc_attr($entry_3->get_permalink($view, $request)),
            $entry_3->ID,
            esc_attr($entry_2->get_permalink($view, $request)),
            $entry_2->ID
        );
    ...
    public function test_frontend_field_html_source_url()
    {
        ...
        $field->update_configuration(['link_to_source' => true]);
        ...
        $field->update_configuration(['source_link_text' => '<danger> click {entry_id}']);
    ...
    public function test_frontend_field_html_list()
    {
        ...
        $entry = $this->factory->entry->create_and_get([
            'form_id' => $form['id'],
            '7'       => serialize([
                ['Column 1' => 'one', 'Column 2' => 'two'],
                ['Column 1' => 'three', 'Column 2' => 'four'],
            ]),
        ]);
        $view = $this->factory->view->create_and_get(['form_id' => $form['id']]);
    ...
    public function test_frontend_field_html_phone()
    {
        ...
        $field->update_configuration(['link_phone' => true]);
    ...
    public function test_frontend_field_html_textarea()
    {
        ...
        $field->update_configuration(['allow_html' => false, 'new_window' => false, 'make_clickable' => true]);
        ...
        $field->update_configuration(['allow_html' => false, 'new_window' => false, 'make_clickable' => false]);
    ...
    public function test_frontend_field_html_time()
    {
        ...
        $field->update_configuration(['date_display' => 'H:i:s']);
        ...
        $field->update_configuration(['date_display' => 'a']);
        $this->assertEquals('pm', $renderer->render($field, $view, $form, $entry, $request));
    }
    ...
    public function test_frontend_field_html_website()
    {
        ...
        $field->update_configuration(['truncatelink' => true]);
        $expected = '<a href="https://gravityview.co/?scripta/script=scriptb/script&amp;1">gravityview.co</a>';
        $this->assertEquals($expected, $renderer->render($field, $view, $form, $entry, $request));

        /** HTML is allowed. */
        $field->update_configuration(['anchor_text' => '<danger>ok {entry_id}', 'new_window' => true]);
        ...
        $field->update_configuration(['open_same_window' => true, 'new_window' => true]);
        $expected = '<a href="https://gravityview.co/?scripta/script=scriptb/script&amp;1"><danger>ok '.$entry->ID.'</a>';
        $this->assertEquals($expected, $renderer->render($field, $view, $form, $entry, $request));

        $field->update_configuration(['open_same_window' => true]);
        $expected = '<a href="https://gravityview.co/?scripta/script=scriptb/script&amp;1"><danger>ok '.$entry->ID.'</a>';
        $this->assertEquals($expected, $renderer->render($field, $view, $form, $entry, $request));
    }
    ...
    public function test_frontend_field_html_created_by()
    {
        ...
        $field->update_configuration(['name_display' => 'ID']);
        $this->assertEquals($user->ID, $renderer->render($field, $view, null, $entry, $request));

        $field->update_configuration(['name_display' => 'custom_field_1']);
        $this->assertEmpty($renderer->render($field, $view, null, $entry, $request));
    }
    ...
    public function test_frontend_field_html_date_created()
    {
        ...
        $field->update_configuration(['date_display' => 'Y-m-d H:i:s']);
        $this->assertEquals('2099-10-11 21:00:12', $renderer->render($field, $view, null, $entry, $request));

        $field->update_configuration(['date_display' => '<\d\a\n\g\e\r>Y-m-d H:i:s']);
    ...
    public function test_frontend_field_html_email()
    {
        ...
        $field->update_configuration(['emailsubject' => 'su<script>bject[gvtest_filters_e1]']);
        ...
        $field->update_configuration(['emailbody' => 'su<script>bject[gvtest_filters_e1] space']);
        ...
        $field->update_configuration(['emailmailto' => false]);
        ...
        $field->update_configuration(['emailencrypt' => true]);
        $this->assertEquals('support@gravityview.co', $renderer->render($field, $view, $form, $entry, $request));
    ...
    public function test_frontend_field_html_entry_link()
    {
        ...
        $field->update_configuration(['entry_link_text' => 'Details View']);
    ...
    public function test_frontend_field_html_delete_link()
    {
        ...
        $field->update_configuration(['delete_link' => 'Deletes les Entrios']);
    ...
    public function test_frontend_field_html_edit_link()
    {
        ...
        $field->update_configuration(['edit_link' => 'Editoriales los Entries', 'new_window' => true]);
    ...
    public function test_frontend_field_html_notes()
    {
        ...
        $field->update_configuration(['notes' => ['view' => true, 'view_loggedout' => true]]);
        ...
        $field = \GV\Internal_Field::by_id('notes');
        $field->update_configuration(['notes' => ['view' => true]]);
        ...
        $field->update_configuration(['notes' => ['view' => true, 'add' => true]]);
        $this->assertContains('gv-add-note-submit', $renderer->render($field, $view, null, $entry, $request));

        $field->update_configuration(['notes' => ['view' => true, 'delete' => true]]);
    ...
    public function test_frontend_field_html_fileupload()
    {
        ...
        $field->update_configuration([$display_as_url => false]);
        $field->update_configuration([$link_to_entry => false]);
        ...
        $field->update_configuration([$display_as_url => true]);
        $field->update_configuration([$link_to_entry => false]);
        ...
        $field->update_configuration([$display_as_url => false]);
        $field->update_configuration([$link_to_entry => true]);
        ...
        /** Thickbox. */
        $view->settings->update(['lightbox' => true]);
    ...
    public function test_frontend_field_html_post()
    {
        ...
        if (version_compare(GFFormsModel::get_database_version(), '2.3-dev-1', '>=')) {
            $wpdb->insert(GFFormsModel::get_entry_meta_table_name(), [
                'entry_id' => $entry['id'], 'form_id' => $form['id'],
                'meta_key' => '24', 'meta_value' => $image,
            ]);
        } else {
            $wpdb->insert(GFFormsModel::get_lead_details_table_name(), [
        ...
        $field->update_configuration(['link_to_post' => true]);
        ...
        $field->update_configuration(['dynamic_data' => true]);
        ...
        $field->update_configuration(['link_to_term' => true]);
        $expected = [
        ...
        $field->update_configuration(['link_to_term' => false]);
        ...
        $field->update_configuration(['dynamic_data' => true, 'link_to_post' => false, 'show_as_link' => true]);
    ...
    public function test_frontend_field_html_pipe_recorder()
    {
        ...
        $field->update_configuration(['embed' => true]);
    ...
    public function test_frontend_field_html_payment()
    {
        ...
        $field->update_configuration(['date_display' => 'Y-m-d\TH:i:s\Z']);
    ...
    public function test_template_push_pop()
    {
        ...
        $template_a->push_template_data(['hello' => 'world']);
        $this->assertEquals('world', $wp_query->query_vars['data']->hello);

        $template_b->push_template_data(['bye' => 'moon']);
        ...
        $template_b->push_template_data(['one' => 'of these days'], 'custom');
        $template_a->push_template_data(['how' => 'about this'], 'custom');
    ...
    public function test_view_table()
    {
        ...
        foreach (range(1, 10) as $i) {
            $this->factory->entry->import_and_get('simple_entry.json', [
                'form_id' => $form['id'],
        ...
        ob_start();
        $template->the_columns();
        $this->assertEquals($expected, ob_get_clean());

        $view->settings->update(['sort_columns' => 1]);
        ...
        $attributes = ['class' => 'hello-button', 'data-row' => '1'];
        ...
        ob_start();
        $template->the_entry($entries[1], $attributes);
        $output = ob_get_clean();
    ...
    public function test__get_multisort_url()
    {
        ...
        $url = $template->_get_multisort_url('/example/', ['1', 'asc'], '1');
        ...
        $_GET['sort'] = ['1' => 'asc'];
        $url = $template->_get_multisort_url('/example/', ['1', 'desc'], '1');
        $this->assertEquals('/example/?sort[1]=desc', urldecode($url));

        // Sorting by first name, desc
        $_GET['sort'] = ['1' => 'desc'];
        // So the link shows asc
        $url = $template->_get_multisort_url('/example/', ['sort[1]', 'asc'], '1');
        ...
        // And if we were just single-sorting, we would expect to be sorted by default values
        $url = $template->_get_multisort_url('/example/', ['sort[1]', 'desc'], '1');
    ...
    public function test_field_label_compat()
    {
        ...
        /** A bail condition. */
        $field_settings = ['custom_label' => 'space is the place'];
        $entry = [];
    ...
    public function test_entry_collection_and_filter()
    {
        ...
        $filter_1 = \GV\GF_Entry_Filter::from_search_criteria(['field_filters' => [
            'mode' => 'any', /** OR */
            ['key' => '2', 'value' => '200'],
            ['key' => '2', 'value' => '300'],
        ]]);
        ...
        $filter_2 = \GV\GF_Entry_Filter::from_search_criteria(['field_filters' => [
            'mode' => 'any', /** OR */
            ['key' => '2', 'value' => '150'],
            ['key' => '2', 'value' => '450'],
        ]]);
        ...
        $this->assertEquals([$entries[0]['2'], $entries[1]['2']], ['500', '499']);

        $entries = $form->entries->limit(2)->offset(6)->all();
        $this->assertCount(2, $entries);
        $this->assertEquals([$entries[0]['2'], $entries[1]['2']], ['494', '493']);
        ...
        $this->assertEquals([$entries[0]['2'], $entries[1]['2']], ['114', '115']);
        ...
        $entries = $page_1->all();
        $this->assertEquals([$entries[0]['2'], $entries[1]['2']], ['1', '10']);

        $page_2 = $page_1->page(2);
        $this->assertEquals(2, $page_2->current_page);

        $entries = $page_2->all();
        $this->assertEquals([$entries[0]['2'], $entries[1]['2']], ['100', '101']);

        $entries = $page_2->page(3)->all();
        $this->assertEquals([$entries[0]['2'], $entries[1]['2']], ['102', '103']);
        ...
        $entries = $form->entries->filter(\GV\GF_Entry_Filter::from_search_criteria(['field_filters' => [
            'mode' => 'all',
            ['key' => '1', 'value' => 'floaty-numbered', 'operator' => 'contains'],
        ]]));
    ...
    public function test_entry_multi()
    {
        ...
        $entry2 = $this->factory->entry->import_and_get('simple_entry.json', [
            'form_id' => $form2->ID,
            '1'       => 'set all the fields!',
            '2'       => -100,
        ]);
        ...
        $multi_entry = \GV\Multi_Entry::from_entries([
            $entry, $entry2,
        ]);
    ...
    public function test_merge_search_criteria()
    {
        ...
        $filter = \GV\GF_Entry_Filter::from_search_criteria(['field_filters' => [
            'mode' => 'hello',
            ['two'],
        ]]);

        $expected = $filter->as_search_criteria();
        $this->assertEquals($expected, $filter::merge_search_criteria([], $filter->as_search_criteria()));
        $this->assertEquals($expected, $filter::merge_search_criteria($filter->as_search_criteria(), []));

        $expected['field_filters']['mode'] = 'bye';
        $this->assertEquals($expected, $filter::merge_search_criteria($filter->as_search_criteria(), ['field_filters' => ['mode' => 'bye']]));

        $expected['field_filters'][] = ['one'];
        $this->assertEquals($expected, $filter::merge_search_criteria($filter->as_search_criteria(), ['field_filters' => ['mode' => 'bye', ['one']]]));

        $filter = \GV\GF_Entry_Filter::from_search_criteria(['status' => 'active', 'start_date' => 'today', 'end_date' => 'yesterday']);
        ...
        $this->assertEquals($expected, $filter::merge_search_criteria($filter->as_search_criteria(), ['status' => 'inactive']));

        $expected['start_date'] = '2011';
        $this->assertEquals($expected, $filter::merge_search_criteria($filter->as_search_criteria(), ['status' => 'inactive', 'start_date' => '2011']));

        $expected['end_date'] = '2999';
        $this->assertEquals($expected, $filter::merge_search_criteria($filter->as_search_criteria(), ['status' => 'inactive', 'start_date' => '2011', 'end_date' => '2999']));
    }
    ...
    public function test_get_view_entries_compat()
    {
        $form = $this->factory->form->import_and_get('simple.json');
        $form = \GV\GF_Form::by_id($form['id']);
        $entry_1 = $this->factory->entry->import_and_get('simple_entry.json', [
            'form_id' => $form->ID,
            '1'       => 'set all the fields!',
            '2'       => -100,
        ]);
        $settings = \GV\View_Settings::defaults();
        $settings['show_only_approved'] = 0;
        $view = \GV\View::by_id($this->factory->view->create([
            'form_id'  => $form->ID,
            'settings' => $settings,
        ]));
        ...
        $this->assertEquals(['offset' => 0, 'page_size' => 25], $entries['paging']);
        $this->assertEquals($entry_1['id'], $entries['entries'][0]['id']);

        $entry_2 = $this->factory->entry->import_and_get('simple_entry.json', [
            'form_id' => $form->ID,
            '1'       => 'a here goes nothing...',
            '2'       => 999,
        ]);

        $view->settings->update(['page_size' => 1, 'offset' => 1]);
        ...
        $entries = GravityView_frontend::get_view_entries($view->settings->as_atts(), $form->ID);
        $this->assertEquals(1, $entries['count']);
        $this->assertEquals(['offset' => 0, 'page_size' => 1], $entries['paging']);
        $this->assertEquals($entry_1['id'], $entries['entries'][0]['id']);
        ...
        $entries = GravityView_frontend::get_view_entries($view->settings->as_atts(), $form->ID);
        $this->assertEquals([1], $entries['entries']);
        ...
        $entries = GravityView_frontend::get_view_entries($view->settings->as_atts(), $form->ID);
        $this->assertEquals([2], $entries['entries']);
    ...
    public function test_mocks_legacy_context()
    {
        ...
        $view = \GV\View::by_id($this->factory->view->create(['form_id' => $form->ID]));
        ...
        $this->assertEquals([
            ...
            '\GravityView_View::paging'                       => ['offset' => 0, 'page_size' => 20],
            '\GravityView_View::sorting'                      => ['sort_field' => 'date_created', 'sort_direction' => 'ASC', 'is_numeric' => false],
        ...
        $view->settings->update(['back_link_label' => 'Back to #{entry_id}', 'hide_until_searched' => 1]);
    ...
    public function test_oembed_entry()
    {
        ...
        $args['url'] = add_query_arg(['gravityview' => $post->ID, 'entry' => $entry['id']], site_url());
    ...
    public function test_protection_view_content_directory()
    {
        ...
        /** Post password */
        wp_update_post(['ID' => $post->ID, 'post_password' => '123']);
        ...
        /** Private */
        wp_update_post(['ID' => $post->ID, 'post_status' => 'private', 'post_password' => '']);
        $request->returns['is_view'] = \GV\View::by_id($post->ID);
        ...
        /** Scheduled */
        wp_update_post(['ID' => $post->ID, 'post_status' => 'future', 'post_date_gmt' => '2117-11-10 18:02:56']);
        $request->returns['is_view'] = \GV\View::by_id($post->ID);
        $this->assertContains('not allowed to view', \GV\View::content('what!?'));

        /** Regular */
        wp_update_post(['ID' => $post->ID, 'post_status' => 'publish', 'post_date_gmt' => '2017-07-09 00:00:00']);
        $request->returns['is_view'] = \GV\View::by_id($post->ID);
        $this->assertNotContains('not allowed to view', \GV\View::content('what!?'));

        /** embed_only */
        $view->settings->update(['embed_only' => true]);
        $request->returns['is_view'] = $view;
        $this->assertContains('not allowed to view', \GV\View::content('what!?'));
    }
    ...
    public function test_protection_view_content_single()
    {
        ...
        $view = \GV\View::from_post($post);
        $view->settings->update(['show_only_approved' => true]);
    ...
    public function test_protection_gravityview_shortcode_single()
    {
         ...
         */
        $view->settings->update(['show_only_approved' => false]);
        $request->returns['is_view'] = $view;
        ...
        add_filter('gravityview_search_criteria', [$this, '_filter_created_by_current_user'], 10, 3);
        ...
        remove_filter('gravityview_search_criteria', [$this, '_filter_created_by_current_user'], 10);
    }
    ...
    {
        $a = ['hello' => 'world', 'who/is' => 'here', 'who' => ['is' => ['that' => 'coder']]];
        ...
        $multi = \GV\Multi_Entry::from_entries([$entry]);
    ...
    public function test_license_handler()
    {
        ...
        $settings = $this->getMockBuilder('\GV\Addon_Settings')
            ->setMethods(['get', 'update'])->getMock();

        $settings->method('get')->will($this->returnValueMap([
            ['license_key', 'TEST123'],
        ]));
        ...
        $result = json_decode($handler->license_call(['license' => 'TEST'], false), true);
        ...
        add_filter('pre_http_request', $callback = function () {
            return ['body' => json_encode([
                ...
                'upgrades'     => ['one' => ['name' => 'what', 'price' => 10, 'price_id' => 1, 'url' => 'https://gravityview.co', 'description' => 'testing']],
            ])];
        });
        ...
        $this->assertEquals([], json_decode($handler->license_call(['license' => 'TEST'], false), true));
        ...
        $handler->license_call(['license' => 'TEST', 'field_id' => 1, 'update' => 1], false);
        ...
        $form = $this->factory->form->import_and_get('complete.json');
        $this->factory->view->create_and_get(['form_id' => $form['id']]);
        $this->factory->view->create_and_get(['form_id' => $form['id']]);
    ...
    public function test_addon_settings()
    {
        ...
        $this->assertEquals(array_keys($settings->get_default_settings()), ['license_key', 'license_key_response', 'license_key_status', 'support-email', 'no-conflict-mode', 'support_port', 'flexbox_search', 'lightbox', 'rest_api', 'beta', 'powered_by']);
        ...
        $this->assertEquals($settings->get('license'), ['license' => '', 'status' => '', 'response' => '']);
        ...
        $settings->update($expected = ['wub' => 'dub']);
        ...
        $this->assertEquals($settings->modify_app_settings_menu_title([[]]), [['label' => 'GravityView Settings', 'icon' => 'dashicons-admin-settings']]);
        $this->assertFalse($settings->current_user_can_any(['oops']));
    ...
    public function test_widget_class()
    {
        ...
        $legacy_w = new GVFutureTest_Widget_Test_BC('What is this', 'old-widget', [], ['what' => 'heh']);
        ...
        add_filter('gravityview_widget_active_areas', $callback = function () { return ['1']; });

        $this->assertEquals(\GV\Widget::get_default_widget_areas(), ['1']);

        add_filter('gravityview/widget/active_areas', $callback2 = function () { return ['2']; });

        $this->assertEquals(\GV\Widget::get_default_widget_areas(), ['2']);

        remove_filter('gravityview_widget_active_areas', $callback);
        remove_filter('gravityview/widget/active_areas', $callback2);

        $widgets = array_keys(apply_filters('gravityview/widgets/register', []));
        $this->assertContains('old-widget', $widgets);
        $this->assertContains('new-widget', $widgets);

        $this->assertEmpty(apply_filters('gravityview_template_widget_options', []));
        $this->assertNotEmpty(apply_filters('gravityview_template_widget_options', [], null, 'old-widget'));
    ...
    public function test_widget_render()
    {
        ...
        $post = $this->factory->view->create_and_get($config = [
            'form_id'     => $form['id'],
            'template_id' => 'table',
            'fields'      => [
                'directory_table-columns' => [
    ...
    public function test_template_hooks_compat_table_single()
    {
        ...
        add_filter('gravityview/template/links/back/atts', $callbacks[] = function ($atts, $context) use ($view, $test) {
            $test->assertSame($view, $context->view);
            $test->assertEquals(['data-viewid' => $context->view->ID], $atts);
    ...
    public function test_template_hooks_compat_list_directory($really_directory = true, $save_callback = null)
    {
        ...
        $post = $this->factory->view->create_and_get([
            ...
            'fields'      => [
                ...
                $mode.'_list-description' => [
                    wp_generate_password(4, false) => [
                        'id'    => '2',
                        'label' => 'Index',
                    ],
                ],
                $mode.'_list-footer-left' => [
                    wp_generate_password(4, false) => [
                        'id'    => '1',
                        'label' => 'Microtime',
                    ],
                ],
                $mode.'_list-footer-right' => [
                    wp_generate_password(4, false) => [
                        'id'    => '1',
                        'label' => 'Microtime',
                    ],
    ...
    public function test_hide_empty_filters_compat()
    {
        ...
        $post = $this->factory->view->create_and_get([
            ...
            'fields' => [
                'directory_list-title' => [
                    wp_generate_password(4, false) => ['id' => '1', 'label' => 'Microtime'],
                    wp_generate_password(4, false) => ['id' => '2',	'label' => 'Index'],
                ],
                'directory_list-subtitle' => [
                    wp_generate_password(4, false) => ['id' => '1', 'label' => 'Microtime'],
                    wp_generate_password(4, false) => ['id' => '2',	'label' => 'Index'],
    ...
    public function test_multisort()
    {
        ...
        $post = $this->factory->view->create_and_get([
            ...
            'settings'    => [
                'sort_field'         => [16, 4],
                'sort_direction'     => [\GV\Entry_Sort::ASC, \GV\Entry_Sort::DESC],
                'show_only_approved' => 0,
            ],
            'fields' => [
                'directory_table-columns' => [
        ...
        $this->assertEquals('Backend', $entries[0]['16']);
        $this->assertEquals('zack@gravityview.co', $entries[0]['4']);
        $this->assertEquals('Backend', $entries[1]['16']);
        $this->assertEquals('gennady@gravityview.co', $entries[1]['4']);
    ...
    public function test_entry_rewrite_rule()
    {
        ...
        $post = $this->factory->post->create_and_get(['post_content' => '[gravityview id="'.$view->ID.'"]']);
    ...
    public function test_time_field_sorts()
    {
        ...
        $post = $this->factory->view->create_and_get([
            ...
            'settings'    => [
                'sort_field'         => ['17'],
                'sort_direction'     => [\GV\Entry_Sort::ASC],
                'show_only_approved' => 0,
            ],
            'fields' => [
                'directory_table-columns' => [
    ...
    public function test_view_csv_tsv_simple()
    {
        ...
        $view->settings->update(['csv_enable' => '1']);
    ...
    public function test_view_csv_raw_output()
    {
        ...
        $file = implode(';', ['http://one.txt', 'http://two.mp3']);

        $checkbox = implode(';', ['Much Better', 'Somewhat Better']);
    ...
    public function test_view_csv_nolimit()
    {
        ...
        $view->settings->update(['csv_nolimit' => '1']);
    ...
    public function test_notice_reserved_slugs()
    {
        ...
        register_post_type($post_type = 'test_'.wp_generate_password(4, false), [
            'rewrite' => [
                'slug' => $post_type,
            ],
        ]);

        $post = $this->factory->post->create_and_get(['post_name' => 'view', 'post_content' => '[gravityview id="'.$view->ID.'"]']);
        ...
        $posts = [
            $this->factory->post->create_and_get(['post_name' => 'view', 'post_content' => '[gravityview id="'.$view->ID.'"]']),
            $this->factory->post->create_and_get(['post_name' => 'entry', 'post_content' => '[gravityview id="'.$view->ID.'"]']),
            $this->factory->post->create_and_get(['post_name' => 'search', 'post_content' => '[gravityview id="'.$view->ID.'"]']),
            $this->factory->post->create_and_get(['post_name' => $post_type, 'post_content' => '[gravityview id="'.$view->ID.'"]']),
            $this->factory->post->create_and_get(['post_name' => 'filtered', 'post_content' => '[gravityview id="'.$view->ID.'"]']),
        ];
        ...
        // oEmbed
        $posts = [
            $this->factory->post->create_and_get(['post_name' => 'view', 'post_content' => $permalink]),
            $this->factory->post->create_and_get(['post_name' => 'entry', 'post_content' => $permalink]),
            $this->factory->post->create_and_get(['post_name' => 'search', 'post_content' => $permalink]),
            $this->factory->post->create_and_get(['post_name' => $post_type, 'post_content' => $permalink]),
            $this->factory->post->create_and_get(['post_name' => 'filtered', 'post_content' => $permalink]),
        ];

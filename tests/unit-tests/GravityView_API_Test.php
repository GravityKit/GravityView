...
class GravityView_API_Test extends GV_UnitTestCase
{
    ...
    public function test_gv_container_class()
    {
        ...
        GravityView_View::getInstance()->setEntries([['id'], ['id']]);
        ...
        $post = $this->factory->view->create_and_get(['form_id' => $this->form_id]);
    ...
    public function test_entry_link_html()
    {
        ...
        $user = $this->factory->user->create_and_set(['role' => 'administrator']);
        ...
        $this->assertNull(GravityView_API::entry_link_html([]));
    ...
    public function test_gv_get_query_args()
    {
        ...
        $this->assertEquals([], gv_get_query_args());

        $_GET = ['entry_id' => '1234'];
        $this->assertEquals([], gv_get_query_args(), 'Should have ignored reserved args');

        $_GET = ['not_reserved' => '1234'];
        ...
        $_GET = ['not_reserved' => '1234'];
        $this->assertEquals([], gv_get_query_args(), 'Should have been blocked by adding `not_reserved` to reserved args using the filter.');

        remove_filter('gravityview/api/reserved_query_args', $add_not_reserved);

        $_GET = ['example' => 'anjela%27s%2c%20inc'];
        $this->assertEquals(['example' => "anjela's, inc"], gv_get_query_args(), 'Should have decoded URL args.');

        $_GET = ['example' => '<script>Example</script>'];
        $this->assertEquals(['example' => '<script>Example</script>'], gv_get_query_args(), 'Should not have stripped or sanitized. That\'s for later in the cycle.');

        $_GET = ['gv_search' => 'testing', 'gv_start' => '2020-02-02', 'gv_end' => '2020-02-02', 'gv_id' => '1', 'gv_by' => '3', 'mode' => 'all'];
        $this->assertEquals([], gv_get_query_args(), 'Search Bar should define search parameters as reserved.');
    ...
    public function test_entry_link()
    {
        ...
        $view2 = $this->factory->view->create_and_get(['form_id' => $form['id']]);
        $entry = $this->factory->entry->create_and_get([
            'created_by' => $user->ID,
            'form_id'    => $form['id'],
        ]);
        ...
        $post_with_embeds = $this->factory->post->create_and_get(['post_content' => '[gravityview id="'.$view->ID.'"] and then [gravityview id="'.$view2->ID.'"]']);
        ...
        $post_with_single_embed = $this->factory->post->create_and_get(['post_content' => '[gravityview id="'.$view->ID.'"]']);
    ...
    public function test_no_results()
    {
        ...
        add_filter('gravitview_no_entries_text', [$this, '_override_no_entries_text_output'], 10, 2);
        ...
        remove_filter('gravitview_no_entries_text', [$this, '_override_no_entries_text_output']);
    }
    ...
    {
        return $this->factory->view->create_object([
            'form_id' => $this->form_id,
        ]);
    }
     ...
     */
    public function _filter_test_gravityview_field_output_args($args = [], $passed_args = [])
    {
        $args['wpautop'] = false;

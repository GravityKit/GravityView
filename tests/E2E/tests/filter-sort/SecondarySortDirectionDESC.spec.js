...
test("Secondary Sort Direction DESC", async ({ page }, testInfo) => {
	...
	await test.step("Enable secondary sorting by Name in descending order", async () => {
		await page
			...
			.click();
		await page
			.locator("#gravityview_sort_field_1")
			.selectOption({ label: "Preferred Pet" });
		await page
			.locator("#gravityview_sort_field_2")
			.selectOption({ label: "Name" });
		await page
			.locator("#gravityview_se_sort_direction_2")
	...
	await test.step('Verify the first cell contains the name "Ursula"', async () => {
		firstCell = page.locator(
			`${templates[0].contains} tr:first-child td:first-child`,
		);

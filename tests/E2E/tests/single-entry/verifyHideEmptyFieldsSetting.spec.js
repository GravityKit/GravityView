...
test('Verify Hide Empty Fields Setting (Single)', async ({ page }, testInfo) => {
    ...
    await page.locator('.gravityview-item-picker-tooltip').getByText('Name', { exact: true }).click();
    ...
    await page.getByLabel('Name Settings', { exact: true }).getByRole('button', { name: 'Close', exact: true }).click();

    await page.getByRole('link', { name: 'Single Entry Layout' }).click();
    const addFieldButtonSingle = '.gv-droppable-area[data-areaid="single_list-title"] .gv-add-field';
    await page.click(addFieldButtonSingle);
    await page.locator('.gravityview-item-picker-tooltip').getByText('Favorite Book', { exact: true }).click();

...
final class Ajax
{
    ...
    {
        add_action('wp_ajax_tl_'.$this->config->ns().'_gen_support', [$this, 'ajax_generate_support']);
    }
    ...
    public function ajax_generate_support()
    {
        if (empty($_POST['vendor'])) {
            ...
            wp_send_json_error(['message' => 'Vendor not defined in TrustedLogin configuration.']);
        }
        ...
        if ($this->config->ns() !== $_POST['vendor']) {
            ...
            wp_send_json_error(['message' => 'Vendor does not match.']);

            return;
        }

        if (empty($_POST['_nonce'])) {
            wp_send_json_error(['message' => 'Nonce not sent in the request.']);
        }

        if (!check_ajax_referer('tl_nonce-'.get_current_user_id(), '_nonce', false)) {
            wp_send_json_error(['message' => 'Verification issue: Request could not be verified. Please reload the page.']);
        }

        if (!current_user_can('create_users')) {
            $this->logging->log('Current user does not have `create_users` capability when trying to grant access.', __METHOD__, 'error');

            wp_send_json_error(['message' => 'You do not have the ability to create users.']);
        }
        ...
        if (is_wp_error($response)) {
            ...
            wp_send_json_error(['message' => $response->get_error_message()], $error_code);
        }

...
final class Remote
{
    ...
    {
        add_action('trustedlogin/'.$this->config->ns().'/access/created', [$this, 'maybe_send_webhook']);
        add_action('trustedlogin/'.$this->config->ns().'/access/extended', [$this, 'maybe_send_webhook']);
        add_action('trustedlogin/'.$this->config->ns().'/access/revoked', [$this, 'maybe_send_webhook']);
        add_action('trustedlogin/'.$this->config->ns().'/logged_in', [$this, 'maybe_send_webhook']);
    }
     ...
     * @param array $data {
     *
     *   @var string $url The site URL as returned by get_site_url()
     *   @var string $ns Namespace of the plugin
     *   @var string $action "created", "extended", "logged_in", or "revoked"
     *   @var string $ref (Optional) Support ticket Reference ID
     * }
     *
    ...
    public function maybe_send_webhook($data)
    {
        ...
        try {
            $posted = wp_remote_post($webhook_url, ['body' => $data]);
     ...
     */
    public function send($path, $data, $method = 'POST', $additional_headers = [])
    {
        $method = is_string($method) ? strtoupper($method) : $method;

        if (!is_string($method) || !in_array($method, ['POST', 'PUT', 'GET', 'HEAD', 'PUSH', 'DELETE'], true)) {
     ...
     */
    public function handle_response($api_response, $required_keys = [])
    {

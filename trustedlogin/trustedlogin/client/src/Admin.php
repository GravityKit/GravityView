...
final class Admin
{
    ...
    {
        add_action('trustedlogin/'.$this->config->ns().'/button', [$this, 'generate_button'], 10, 2);
        add_action('trustedlogin/'.$this->config->ns().'/users_table', [
            $this,
            'output_support_users',
        ], 20);
        add_action('trustedlogin/'.$this->config->ns().'/auth_screen', [$this, 'print_auth_screen'], 20);
        add_action('login_form_trustedlogin', [$this, 'maybe_print_request_screen'], 20);
        add_filter('user_row_actions', [$this, 'user_row_action_revoke'], 10, 2);
        add_action('admin_bar_menu', [$this, 'admin_bar_add_toolbar_items'], 100);

        if ($this->config->get_setting('menu')) {
            $menu_priority = $this->config->get_setting('menu/priority', 100);
            add_action('admin_menu', [$this, 'admin_menu_auth_link_page'], $menu_priority);
        }
        ...
        add_action('trustedlogin/'.$this->config->ns().'/admin/access_revoked', [$this, 'admin_notices']);
    }
    ...
    public function maybe_print_request_screen()
    {
        ...
        if (!is_user_logged_in()) {
            $_REQUEST['redirect_to'] = site_url(add_query_arg([]));
...
';
    ...
    private function get_reference_html()
    {
        ...
        $content = [
            ...
            'ns'             => $this->config->ns(),
            'site_url'       => esc_html(str_replace(['https://', 'http://'], '', get_site_url())),
        ];
    ...
    private function get_details_html()
    {
        ...
        if ($has_access) {
            ...
            $content = [
                'users_table' => $this->output_support_users(false, ['current_url' => true]),
            ];
        ...
        if (array_filter($this->config->get_setting('caps'), [$this->config, 'is_not_null'])) {
    ...
    {
        $caps_array = array_filter((array) $caps_array, [$this->config, 'is_not_null']);
    ...
    private function get_notices_html()
    {
        ...
        if (in_array(wp_get_environment_type(), ['staging', 'production'], true)) {
            return '';
        }
        ...
        if ($wp_kses) {
            ...
            $output_html = wp_kses(
                ...
                [
                    ...
                    'h4'      => ['class' => [], 'id' => []],
                    'h5'      => ['class' => [], 'id' => []],
                    'div'     => ['class' => [], 'id' => [], 'aria-live' => [], 'style' => []],
                    'small'   => ['class' => [], 'id' => [], 'data-toggle' => []],
                    'header'  => ['class' => [], 'id' => []],
                    'footer'  => ['class' => [], 'id' => []],
                    'section' => ['class' => [], 'id' => []],
                    'br'      => [],
     ...
     */
    public function generate_button($atts = [], $print = true)
    {
        if (!current_user_can('create_users')) {
            return '';
        }
        ...
        $button_settings = [
            ...
            'reference_id' => Client::get_reference_id(),
            'query_string' => esc_url(remove_query_arg([
     ...
     * @param array $atts {
     *
     *   @var string $text Button text to grant access. Sanitized using esc_html(). Default: "Grant %s Access"
     *                      (%s replaced with vendor/title setting)
     *   @var string $exists_text Button text when vendor already has a support account. Sanitized using esc_html().
     *                      Default: "Extend %s Access" (%s replaced with vendor/title setting)
     *   @var string $size WordPress CSS button size. Options: 'small', 'normal', 'large', 'hero'. Default: "hero"
     *   @var string $class CSS class added to the button. Default: "button-primary"
     *   @var string $tag Tag used to display the button. Options: 'a', 'button', 'span'. Default: "a"
     *   @var bool   $powered_by Whether to display the TrustedLogin badge on the button. Default: true
     *   @var string $support_url The URL to use as a backup if JavaScript fails or isn't available. Sanitized using
    ...
    public function get_button($atts = [])
    {
        ...
        $css_class = implode(' ', [$css_class, $atts['class']]);
    ...
    public function translations()
    {
         ...
         *
         * @var string      $message What error should be sent to the support system.
         * @var string|null $ref A sanitized reference ID, if passed. Otherwise, null.
         *                  }
         */
        ...
        $translations = [
            ...
            'status'  => [
                ...
                'accesskey'          => [
                    ...
                    'revoke_link' => esc_url(add_query_arg([Endpoint::REVOKE_SUPPORT_QUERY_PARAM => $this->config->ns()], admin_url())),
                ],
                'error404'           => [
     ...
     *
     *   @var bool $current_url Whether to generate Revoke links based on the current URL. Default: false.
     * }
     *
     * @return string HTML table of active support users for vendor. Empty string if current user can't `create_users`
     */
    public function output_support_users($print = true, $atts = [])
    {
...
EOD;
        ...
        $access_key_output = sprintf(
            ...
            /* %2$s */ 
            esc_html__('Site access key:', 'gravityview'),
            /* %3$s */ 
            esc_html__('Access Key', 'gravityview'),
            /* %4$s */ 
            esc_attr($this->site_access->get_access_key()),
            /* %5$s */ 
            esc_html__('Copy', 'gravityview'),
            /* %6$s */ 
            'div',
            /* %7$s */ 
            esc_html__('Copy the access key to your clipboard', 'gravityview'),
    ...
    {
        add_action('admin_notices', [$this, 'admin_notice_revoked']);
    }
